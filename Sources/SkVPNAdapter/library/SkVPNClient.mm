...
SkVPNClient::SkVPNClient(id<SkVPNClientDelegate> delegate): ClientAPI::SkVPNClient() {
    this->delegate = delegate;
    this->config = nullptr;
}

SkVPNClient::~SkVPNClient() {
    if (this->config != nullptr) { delete this->config; }
}

ClientAPI::EvalConfig SkVPNClient::apply_config(const ClientAPI::Config& config) {
    if (this->config != nullptr) { delete this->config; }
...
bool SkVPNClient::tun_builder_new() {
...
bool SkVPNClient::tun_builder_set_remote_address(const std::string &address, bool ipv6) {
    NSString *remoteAddress = [NSString stringWithUTF8String:address.c_str()];
    return [this->delegate setRemoteAddress:remoteAddress];
}

bool SkVPNClient::tun_builder_add_address(const std::string &address, int prefix_length, const std::string &gateway, bool ipv6, bool net30) {
...
bool SkVPNClient::tun_builder_reroute_gw(bool ipv4, bool ipv6, unsigned int flags) {
...
bool SkVPNClient::tun_builder_add_route(const std::string& address, int prefix_length, int metric, bool ipv6) {
...
bool SkVPNClient::tun_builder_exclude_route(const std::string& address, int prefix_length, int metric, bool ipv6) {
    NSString *routeAddress = [NSString stringWithUTF8String:address.c_str()];
...
bool SkVPNClient::tun_builder_set_proxy_http(const std::string& host, int port) {
    ...
    return [this->delegate setProxyServer:proxyServer protocol:SkVPNProxyServerProtocolHTTP];
}

bool SkVPNClient::tun_builder_set_proxy_https(const std::string& host, int port) {
    NSString *proxyHost = [NSString stringWithUTF8String:host.c_str()];
    NEProxyServer *proxyServer = [[NEProxyServer alloc] initWithAddress:proxyHost port:port];
    return [this->delegate setProxyServer:proxyServer protocol:SkVPNProxyServerProtocolHTTPS];
}

bool SkVPNClient::tun_builder_set_block_ipv6(bool block_ipv6) {
    return block_ipv6;
}

int SkVPNClient::tun_builder_establish() {
    return [this->delegate establishTunnel] ? [this->delegate socketHandle] : INVALID_SOCKET;
}

bool SkVPNClient::tun_builder_persist() {
    return config->tunPersist;
}

void SkVPNClient::tun_builder_teardown(bool disconnect) {
    [this->delegate resetSettings];
    [this->delegate resetTun];
}

bool SkVPNClient::socket_protect(int socket, std::string remote, bool ipv6) {
    return true;
}

bool SkVPNClient::pause_on_connection_timeout() {
    return false;
}

void SkVPNClient::external_pki_cert_request(ClientAPI::ExternalPKICertRequest& certreq) { }
void SkVPNClient::external_pki_sign_request(ClientAPI::ExternalPKISignRequest& signreq) { }

void SkVPNClient::event(const ClientAPI::Event& ev) {
