...
#import "SkVPNPrivateKey.h"

#include <mbedtls/pk.h>

#import "SkVPNKeyType.h"
#import "NSError+SkVPNError.h"

@interface SkVPNPrivateKey ()

@property (nonatomic, assign) mbedtls_pk_context *ctx;

@end

@implementation SkVPNPrivateKey

+ (SkVPNPrivateKey *)keyWithPEM:(NSData *)pemData password:(NSString *)password error:(NSError * __autoreleasing *)error {
    SkVPNPrivateKey *key = [SkVPNPrivateKey new];
...
+ (SkVPNPrivateKey *)keyWithDER:(NSData *)derData password:(NSString *)password error:(NSError * __autoreleasing *)error {
    SkVPNPrivateKey *key = [SkVPNPrivateKey new];
...
- (SkVPNKeyType)type {
    return (SkVPNKeyType)mbedtls_pk_get_type(self.ctx);
}
