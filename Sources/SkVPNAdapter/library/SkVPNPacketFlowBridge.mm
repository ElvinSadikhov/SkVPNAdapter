...
@implementation SkVPNPacketFlowBridge
...
static void SocketCallback(CFSocketRef socket, CFSocketCallBackType type, CFDataRef address, const void *data, void *obj) {
    ...
    SkVPNPacket *packet = [[SkVPNPacket alloc] initWithVPNData:(__bridge NSData *)data];
    
    SkVPNPacketFlowBridge *bridge = (__bridge SkVPNPacketFlowBridge *)obj;
...
- (BOOL)configureSocketsWithError:(NSError * __autoreleasing *)error {
    ...
    if (socketpair(PF_LOCAL, SOCK_DGRAM, IPPROTO_IP, sockets) == -1) {
        if (error) {
            ...
            *error = [NSError errorWithDomain:SkVPNAdapterErrorDomain
                                         code:SkVPNAdapterErrorSocketSetupFailed
                                     userInfo:userInfo];
        }
    ...
    _skVPNSocket = CFSocketCreateWithNative(kCFAllocatorDefault, sockets[1], kCFSocketNoCallBack, NULL, NULL);
    
    if (!(_packetFlowSocket && _skVPNSocket)) {
        if (error) {
            NSDictionary *userInfo = @{
    ...
    if (!([self configureOptionsForSocket:_packetFlowSocket error:error] &&
          [self configureOptionsForSocket:_skVPNSocket error:error])) { return NO; }
...
- (void)invalidateSocketsIfNeeded {
    if (_skVPNSocket) {
        CFSocketInvalidate(_skVPNSocket);
        CFRelease(_skVPNSocket);
...
- (void)writePackets:(NSArray<NSData *> *)packets protocols:(NSArray<NSNumber *> *)protocols toSocket:(CFSocketRef)socket {
    ...
    [packets enumerateObjectsUsingBlock:^(NSData *data, NSUInteger idx, BOOL *stop) {
        ...
        SkVPNPacket *packet = [[SkVPNPacket alloc] initWithPacketFlowData:data protocolFamily:protocolFamily];
...
- (void)writePackets:(NSArray<SkVPNPacket *> *)packets toPacketFlow:(id<SkVPNAdapterPacketFlow>)packetFlow {
    ...
    [packets enumerateObjectsUsingBlock:^(SkVPNPacket * _Nonnull packet, NSUInteger idx, BOOL * _Nonnull stop) {
