...
// API for SkVPN Client, may be used standalone or wrapped by swig.
// Use ovpncli.i to wrap the API for swig.
// The crux of the API is defined in SkVPNClient (below)
// and TunBuilderBase.
...
#include <skvpn/tun/extern/fw.hpp>
...
namespace skvpn {
  ...
  namespace ClientAPI {
    // Represents an SkVPN server and its friendly name
    // (client reads)
    struct ServerEntry
    {
      std::string server;
    ...
    {
      std::string username;
      std::string session_id; // an SkVPN Session ID, used as a proxy for password
    };
    ...
    // a basic key/value pair, used in Config below when SkVPN profile is
    ...
    // SkVPN config-file/profile
    // (client writes)
    struct Config
    {
      // SkVPN profile as a string
      std::string content;

      // SkVPN profile as series of key/value pairs (may be provided exclusively
      // or in addition to content string above).
      std::vector<KeyValue> contentList;

      // Set to identity SkVPN GUI version.
      ...
      // Pass custom key/value pairs to SkVPN server.
      ...
      // Gremlin configuration (requires that the core is built with SKVPN_GREMLIN)
    ...
    // Top-level SkVPN client class.
    class SkVPNClient : public TunBuilderBase,            // expose tun builder virtual methods
    ...
    public:
      ...
      // Read an SkVPN profile that might contain external
      ...
      // Parse SkVPN configuration file.
    ...
    private:
      ...
      SkVPNClient(const SkVPNClient&) = delete;
      SkVPNClient& operator=(const SkVPNClient&) = delete;
    };
