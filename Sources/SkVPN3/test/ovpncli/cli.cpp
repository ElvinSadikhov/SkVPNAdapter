...
#ifdef SKVPN_PLATFORM_MAC
#include <CoreFoundation/CFBundle.h>
#include <ApplicationServices/ApplicationServices.h>
#endif

// If enabled, don't direct ovpn3 core logging to
// ClientAPI::SkVPNClient::log() virtual method.
...
// Also, note that the SKVPN_LOG_GLOBAL setting
// MUST be consistent across all compilation units.
#ifdef SKVPN_USE_LOG_BASE_SIMPLE
#define SKVPN_LOG_GLOBAL // use global rather than thread-local log object pointer
#include <skvpn/log/logbasesimple.hpp>
#endif

// don't export core symbols
#define SKVPN_CORE_API_VISIBILITY_HIDDEN

// use SITNL on Linux by default
#if defined(SKVPN_PLATFORM_LINUX) && !defined(SKVPN_USE_IPROUTE2) && !defined(SKVPN_USE_SITNL)
#define SKVPN_USE_SITNL
#endif

// should be included before other skvpn includes,
// with the exception of skvpn/log includes
#include <client/ovpncli.cpp>

#include <skvpn/common/exception.hpp>
#include <skvpn/common/string.hpp>
#include <skvpn/common/signal.hpp>
#include <skvpn/common/file.hpp>
#include <skvpn/common/getopt.hpp>
#include <skvpn/common/getpw.hpp>
#include <skvpn/common/cleanup.hpp>
#include <skvpn/time/timestr.hpp>
#include <skvpn/ssl/peerinfo.hpp>
#include <skvpn/ssl/sslchoose.hpp>

#ifdef SKVPN_REMOTE_OVERRIDE
#include <skvpn/common/process.hpp>
#endif

#if defined(USE_MBEDTLS)
#include <skvpn/mbedtls/util/pkcs1.hpp>
#endif

#if defined(SKVPN_PLATFORM_WIN)
#include <skvpn/win/console.hpp>
#include <shellapi.h>
#endif
...
#if defined(SKVPN_PLATFORM_LINUX)

#include <skvpn/tun/linux/client/tuncli.hpp>
...
#include <skvpn/tun/mac/client/tuncli.hpp>
...
class ClientBase : public ClientAPI::SkVPNClient
{
public:
  ...
  void tun_builder_teardown(bool disconnect) override
  {
    ...
    auto os_print = Cleanup([&os](){ SKVPN_LOG_STRING(os.str()); });
    tun->destroy(os);
  }
...
private:
  virtual void event(const ClientAPI::Event& ev) override
  {
	...
	if (ClientAPI::SkVPNClient::parse_dynamic_challenge(ev.info, dc)) {
  ...
  virtual void external_pki_sign_request(ClientAPI::ExternalPKISignRequest& signreq) override
  {
	...
	try {
	  ...
	  // log info
	  SKVPN_LOG("SIGN[" << PKCS1::DigestPrefix::MbedTLSParse::to_string(md_alg) << ',' << signdata.size() << "]: " << render_hex_generic(signdata));
...
static std::string read_profile(const char *fn, const std::string* profile_content)
{
  if (!string::strcasecmp(fn, "http") && profile_content && !profile_content->empty())
	...
	SKVPN_THROW_EXCEPTION("merge config error: " << pm.status_string() << " : " << pm.error());
...
static void start_thread(Client& client)
{
  ...
  Win::Console::Title title(ClientAPI::SkVPNClient::platform() + "     " + title_text);
...
#elif defined(SKVPN_OVPNCLI_SINGLE_THREAD)
...
int skvpn_client(int argc, char *argv[], const std::string* profile_content)
{
  ...
  try {
	...
	while ((ch = getopt_long(argc, argv, "BAdeTCxfgjwmvaYu:p:r:D:P:6:s:S:t:c:z:M:h:q:U:W:I:G:k:X:R:Z:", longopts, nullptr)) != -1)
	  {
	      ...
	      case 'k':
		{
		  ...
		  if (arg == "bi" || arg == "bidirectional")
		    ...
		    SKVPN_THROW_EXCEPTION("bad default key-direction: " << arg);
		}
		break;
	      case 'D':
	...
	if (version)
	  {
	    std::cout << "SkVPN cli 1.0" << std::endl;
	    std::cout << ClientAPI::SkVPNClient::platform() << std::endl;
	    std::cout << ClientAPI::SkVPNClient::copyright() << std::endl;
	  }
	else if (self_test)
	  {
	    std::cout << ClientAPI::SkVPNClient::crypto_self_test();
	  }
	else if (merge)
	  {
	    if (argc != 1)
	...
	else
	  {
		...
		{
		  const ClientAPI::EvalConfig eval = ClientAPI::SkVPNClient::eval_config_static(config);
	      ...
	      else
		{
		  ...
		  if (eval.error)
		    SKVPN_THROW_EXCEPTION("eval config error: " << eval.message);
		  if (eval.autologin)
		    {
		    ...
		    {
		      if (username.empty())
			SKVPN_THROW_EXCEPTION("need creds");
			...
			SKVPN_THROW_EXCEPTION("creds error: " << creds_status.message);
		    }
		  ...
		  if (!epki_cert_fn.empty())
			...
			SKVPN_THROW_EXCEPTION("--epki-key must be specified");
#endif
		    }
    ...
    {
      std::cout << "SkVPN Client (ovpncli)" << std::endl;
...
#ifndef SKVPN_OVPNCLI_OMIT_MAIN

int main(int argc, char *argv[])
{
  int ret = 0;

#ifdef SKVPN_LOG_LOGBASE_H
  LogBaseSimple log;
#endif
  ...
  try {
    ret = skvpn_client(argc, argv, nullptr);
  }
  catch (const std::exception& e)
    {
