...
// Unit test for SkVPN Protocol implementation (class ProtoContext)
...
#define SKVPN_INSTRUMENTATION

// Data limits for Blowfish and other 64-bit block-size ciphers
#ifndef BF
#define BF 0
#endif
#define SKVPN_BS64_DATA_LIMIT 50000
#if BF == 1
...
#ifdef VERBOSE
#define SKVPN_DEBUG_PROTO 2
#define SKVPN_LOG_SSL(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_SSL(x) // disable
#endif
...
#include <skvpn/openssl/util/init.hpp>
...
{
public:
  SKVPN_SIMPLE_EXCEPTION(drought_limit_exceeded);
...
// test the SkVPN protocol implementation in ProtoContext
...
public:
  ...
  SKVPN_EXCEPTION(session_invalidated);
...
class TestProtoServer : public TestProto
{
  typedef TestProto Base;
public:
  SKVPN_SIMPLE_EXCEPTION(auth_failed);
...
#endif
#ifdef SKVPN_MAX_DATALIMIT_BYTES
    std::cerr << "------------------------------" << std::endl;
