...
#define DEBUG_PRINT_ROUTES for(auto & rt: tb->routes) std::cout << rt << std::endl;
    ...
    class TunBuilderMock : public skvpn::TunBuilderBase {
    public:
	bool is_ipv6;
		...
		bool ipv6) override
	{
	    ...
	    routes.push_back(rt);
	    routesAddr.push_back(skvpn::IP::Route(rt));
	    return is_ipv6==ipv6;
	}
	...
	std::vector<skvpn::IP::Route> routesAddr;

	bool containsIP(std::string ipaddr)
	{
	    return containsIP(skvpn::IP::Addr(ipaddr));
	}

	bool containsIP(skvpn::IP::Addr ipaddr)
	{
    ...
    protected:
	skvpn::IPVerFlags* ipflags;
	skvpn::EmulateExcludeRoute::Ptr emu;
	TunBuilderMock* tb;
	skvpn::OptionList emptyOptionList;
	...
	void setup(bool ipv6, bool excludeServer, bool keepEmu = false)
	{
	    ...
	    ipflags = new skvpn::IPVerFlags(emptyOptionList,
		    ipv6 ? skvpn::IP::Addr::V6_MASK : skvpn::IP::Addr::V4_MASK);

	    if (!keepEmu)
	    {
		skvpn::EmulateExcludeRouteFactory::Ptr factory(
			new skvpn::EmulateExcludeRouteFactoryImpl(excludeServer));
	...
	void addRoute(bool include, const std::string& route)
	{
	    ...
	    emu->add_route(include, skvpn::IP::Addr(ipstr), std::stoi(mask));
	}

	void doEmulate(std::string serverip = "1.2.3.4")
	{
	    emu->emulate(this->tb, *this->ipflags, skvpn::IP::Addr(serverip));
	}
	...
	emu->add_route(false, skvpn::IP::Addr("192.168.100.0"), 24);
	...
	setup(false, false);
	// include this net
	emu->add_route(true, skvpn::IP::Addr("10.20.0.0"), 16);

	// but not the first half
	emu->add_route(false, skvpn::IP::Addr("10.20.0.0"), 17);
	...
	emu->add_route(true, skvpn::IP::Addr("2500:1000::"), 32);
	// but not the first half
	emu->add_route(false, skvpn::IP::Addr("2500:1000:8000::"), 33);
