...
extern skvpn::LogOutputCollector* testLog;
 ...
 * tests
 */
class FakeSecureRand : public skvpn::RandomAPI
{
public:
  ...
  virtual void rand_bytes(unsigned char *buf, size_t size) override
  {
    ...
    //SKVPN_LOG("RAND: " << skvpn::render_hex(buf, size));
  }
...
try { \
    statement; \
    SKVPN_THROW_EXCEPTION("JY_EXPECT_THROW: no exception was thrown " << __FILE__ << ':' << __LINE__); \
} \
catch (const expected_exception& e) \
{ \
  if (std::string(e.what()).find(expected_text) == std::string::npos) \
    SKVPN_THROW_EXCEPTION("JY_EXPECT_THROW: did not find expected text in exception at " << __FILE__ << ':' << __LINE__); \
}
...
{ \
  if (!(value)) \
    SKVPN_THROW_EXCEPTION("JY_ASSERT_TRUE: failure at " << __FILE__ << ':' << __LINE__); \
}

#define JY_ASSERT_FALSE(value) \
{ \
  if (value) \
    SKVPN_THROW_EXCEPTION("JY_ASSERT_FALSE: failure at " << __FILE__ << ':' << __LINE__); \
}

#define JY_ASSERT_EQ(v1, v2) \
{ \
  if ((v1) != (v2)) \
    SKVPN_THROW_EXCEPTION("JY_ASSERT_EQ: failure at " << __FILE__ << ':' << __LINE__); \
}

#define JY_ASSERT_NE(v1, v2) \
{ \
  if ((v1) == (v2)) \
    SKVPN_THROW_EXCEPTION("JY_ASSERT_NE: failure at " << __FILE__ << ':' << __LINE__); \
}
