# Unit test framework for SkVPN3 #
...
level as the skvpn3 directory unless overridden by the DEP_DIR variable.

The directory for cmake to build a project can be everywhere, but it is recommended to keep
it outside of the source tree. 

Building unit tests (assuming you are in the skvpn3 directory):
...
On a Mac with OpenSSL from [homebrew](brew.sh):
    ...
    ➜ cmake ../skvpn3 -DOPENSSL_ROOT_DIR=/usr/local/opt/openssl@1.1

Using mbed TLS instead of OpenSSL

    ➜ cmake ../skvpn3 -DUSE_MBEDTLS
...
Examplary commands for building and running on Windows:
    ...
    ➜ cmake -DDEP_DIR=C:\o3\deps -DUSE_MBEDTLS=true -DCMAKE_GENERATOR_PLATFORM=x64 C:\o3\skvpn3
    ➜ cmake --build . --target coreUnitTests
...
common skvpn3 library parameter and to set the `SKVPN_EXTERN` define to `extern`. This
is done so header files that define globals can be included multiple times. The only file that
includes headers without the `SKVPN_EXTERN` being set is the `core_tests.cpp` file. All
