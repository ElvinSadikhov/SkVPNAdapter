...
//    Copyright (C) 2019-2020 David Sommerseth <davids@skvpn.net>
//
...
TEST(VerifyX509Name, config_correct_default_type) {
  ...
  std::string config =
      "verify-x509-name \"C=KG, ST=NA, O=SkVPN-TEST, CN=Test-Server, "
...
TEST(VerifyX509Name, test_subject) {
  ...
  ASSERT_TRUE(verify_def.verify(
      "C=KG, ST=NA, O=SkVPN-TEST, CN=Test-Server, "
      "emailAddress=me@myhost.mydomain"));
  ASSERT_FALSE(verify_def.verify(
      "C=KG, ST=NA, O=SkVPN-TEST-FAIL, CN=Wrong-Server, "
      "emailAddress=me@myhost.mydomain"));
