...
#ifndef SKVPN_ADDR_IPV6_H
#define SKVPN_ADDR_IPV6_H
...
namespace skvpn {
  ...
  namespace IPv6 {
    ...
    SKVPN_EXCEPTION(ipv6_exception);
	...
	skvpn_io::ip::address_v6 a = skvpn_io::ip::make_address_v6(ipstr, ec);
	if (ec)
	...
	return from_string(ipstr, nullptr);
      ...
      {
	const skvpn_io::ip::address_v6 a = to_asio();
	...
	throw ipv6_exception("arpa() not implemented");
      ...
      static Addr from_asio(const skvpn_io::ip::address_v6& asio_addr)
      {
	Addr ret;
	union ipv6addr addr;
	...
	return a->u32[3];
      ...
      skvpn_io::ip::address_v6 to_asio() const
      {
	union ipv6addr addr;
	host_to_network_order(&addr, &u);
	return skvpn_io::ip::address_v6(addr.asio_bytes, scope_id_);
      }
...
#define SKVPN_IPV6_OPERATOR_REL(OP)					\
	...
	if (u.u64[Endian::e2(1)] == other.u.u64[Endian::e2(1)])		\
      ...
      SKVPN_IPV6_OPERATOR_REL(<)
      SKVPN_IPV6_OPERATOR_REL(>)
      SKVPN_IPV6_OPERATOR_REL(<=)
      SKVPN_IPV6_OPERATOR_REL(>=)

#undef SKVPN_IPV6_OPERATOR_REL
	...
	*this = *this + delta;
	...
	skvpn_io::ip::address_v6::bytes_type asio_bytes;
      };
...
#ifdef HAVE_CITYHASH
SKVPN_HASH_METHOD(skvpn::IPv6::Addr, hashval);
#endif

#endif // SKVPN_ADDR_IPV6_H
