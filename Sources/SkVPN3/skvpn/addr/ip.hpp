...
#ifndef SKVPN_ADDR_IP_H
#define SKVPN_ADDR_IP_H
...
#include <skvpn/common/hash.hpp>
#include <skvpn/addr/ipv4.hpp>
#include <skvpn/addr/ipv6.hpp>
#include <skvpn/addr/iperr.hpp>
...
namespace skvpn {
  ...
  namespace IP {
	...
	: Addr(from_string(ipstr, nullptr, UNSPEC))
      ...
      {
	skvpn_io::error_code ec;
	skvpn_io::ip::address a = skvpn_io::ip::make_address(ipstr, ec);
	if (ec)
	...
	{
	  skvpn_io::error_code ec;
	  skvpn_io::ip::make_address(ipstr, ec);
	  return !ec;
	}
      }
	...
	if (ver != UNSPEC)
	  {
	    const skvpn_io::ip::address a = to_asio();
	...
	if (ver == V4)
      ...
      static Addr from_asio(const skvpn_io::ip::address& addr)
      {
	if (addr.is_v4())
	  {
	    Addr a;
      ...
      skvpn_io::ip::address to_asio() const
      {
	switch (ver)
	  {
	  case V4:
	    return skvpn_io::ip::address_v4(u.v4.to_asio());
	  case V6:
	    return skvpn_io::ip::address_v6(u.v6.to_asio());
	  default:
	    throw ip_exception("address unspecified");
	  }
      }
...
#define SKVPN_IP_OPERATOR_BINOP(OP)		       \
...
#define SKVPN_IP_OPERATOR_REL(OP)		\
	...
	if (ver == other.ver)			\
      ...
      SKVPN_IP_OPERATOR_REL(<)
      SKVPN_IP_OPERATOR_REL(>)
      SKVPN_IP_OPERATOR_REL(<=)
      SKVPN_IP_OPERATOR_REL(>=)

#undef SKVPN_IP_OPERATOR_REL
    ...
    SKVPN_OSTREAM(Addr, to_string)
  }
}

#ifdef HAVE_CITYHASH
SKVPN_HASH_METHOD(skvpn::IP::Addr, hashval);
#endif
