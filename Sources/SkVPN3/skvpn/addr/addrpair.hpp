...
#ifndef SKVPN_ADDR_ADDRPAIR_H
#define SKVPN_ADDR_ADDRPAIR_H
...
namespace skvpn {
  namespace IP {
    ...
    {
    public:
      SKVPN_EXCEPTION(addr_pair_mask_parse_error);

      class StringPair {
      public:
	SKVPN_SIMPLE_EXCEPTION(addr_pair_string_error);
	...
	if (!title)
	  title = "";
	SKVPN_THROW(addr_pair_mask_parse_error, "AddrMaskPair parse error '" << title << "': " << s << " : " << e.what());
      }
    ...
    SKVPN_OSTREAM(AddrMaskPair, to_string)
  }
}
