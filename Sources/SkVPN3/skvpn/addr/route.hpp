...
#ifndef SKVPN_ADDR_ROUTE_H
#define SKVPN_ADDR_ROUTE_H
...
namespace skvpn {
  namespace IP {
	...
	if (pair.size() >= 2)
	  {
	    ...
	    if (r.prefix_len > r.addr.size())
	      SKVPN_THROW(route_error, (!StringTempl::empty(title) ? title : "route") << " : bad prefix length : " << rtstr);
	  }
	else
	...
	return false;
      ...
      {
	return addr.to_string() + '/' + skvpn::to_string(prefix_len);
      }
    ...
    struct RouteTypeList : public std::vector<RouteType<ADDR>>
    {
      ...
      SKVPN_EXCEPTION(route_list_error);
    ...
    SKVPN_OSTREAM(Route, to_string);
    SKVPN_OSTREAM(Route4, to_string);
    SKVPN_OSTREAM(Route6, to_string);

    SKVPN_OSTREAM(RouteList, to_string);
    SKVPN_OSTREAM(Route4List, to_string);
    SKVPN_OSTREAM(Route6List, to_string);
					  ...
					  const IP::Addr::Version required_version = IP::Addr::UNSPEC)
    {
      ...
      if (r.prefix_len > r.addr.size())
	SKVPN_THROW(Route::route_error, title << " : bad prefix length : " << addrstr);
      return r;
    }
...
#ifdef HAVE_CITYHASH
SKVPN_HASH_METHOD(skvpn::IP::Route, hash_value);
SKVPN_HASH_METHOD(skvpn::IP::Route4, hash_value);
SKVPN_HASH_METHOD(skvpn::IP::Route6, hash_value);
#endif
