...
#ifndef SKVPN_FRAME_FRAME_H
#define SKVPN_FRAME_FRAME_H
...
namespace skvpn {
  ...
  public:
    ...
    SKVPN_SIMPLE_EXCEPTION(frame_context_index);
	...
	if (newcap > adj_capacity_)
      ...
      // asio read methods.
      skvpn_io::mutable_buffer mutable_buffer(Buffer& buf) const
      {
	return skvpn_io::mutable_buffer(buf.data(), remaining_payload(buf));
      }

      // clamped version of mutable_buffer
      skvpn_io::mutable_buffer mutable_buffer_clamp(Buffer& buf) const
      {
	return skvpn_io::mutable_buffer(buf.data(), buf_clamp_read(remaining_payload(buf)));
      }
#endif
