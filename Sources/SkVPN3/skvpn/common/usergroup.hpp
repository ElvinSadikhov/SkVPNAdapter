...
#ifndef SKVPN_COMMON_USERGROUP_H
#define SKVPN_COMMON_USERGROUP_H
...
namespace skvpn {
  ...
  {
  public:
    SKVPN_EXCEPTION(user_group_err);
      ...
      if (user)
	{
	  pw = ::getpwnam(user);
	  if (!pw && strict)
	    SKVPN_THROW(user_group_err, "user lookup failed for '" << user << '\'');
	  user_name = user;
	}
      if (group)
	{
	  gr = ::getgrnam(group);
	  if (!gr && strict)
	    SKVPN_THROW(user_group_err, "group lookup failed for '" << group << '\'');
	  group_name = group;
	}
    }
      ...
      if (gr)
	{
	  if (::setgid(gr->gr_gid))
	    {
	      const int eno = errno;
	      SKVPN_THROW(user_group_err, "setgid failed for group '" << group_name << "': " << strerror_str(eno));
	    }
	  gid_t gr_list[1];
	  gr_list[0] = gr->gr_gid;
	  if (::setgroups(1, gr_list))
	    {
	      const int eno = errno;
	      SKVPN_THROW(user_group_err, "setgroups failed for group '" << group_name << "': " << strerror_str(eno));
	    }
	  SKVPN_LOG("GID set to '" << group_name << '\'');
	}
      if (pw)
	{
	  if (::setuid(pw->pw_uid))
	    {
	      const int eno = errno;
	      SKVPN_THROW(user_group_err, "setuid failed for user '" << user_name << "': " << strerror_str(eno));
	    }
	  SKVPN_LOG("UID set to '" << user_name << '\'');
	}
#ifdef SKVPN_PLATFORM_LINUX
      ...
      if (pw && gr)
	{
	  ...
	  if (status < 0)
	    {
	      const int eno = errno;
	      SKVPN_THROW(user_group_err, "chown " << user_name << '.' << group_name << ' ' << fn << " : " << strerror_str(eno));
	    }
	}
    }
      ...
      if (pw && gr)
	{
	  ...
	  if (status < 0)
	    {
	      const int eno = errno;
	      SKVPN_THROW(user_group_err, "chown " << user_name << '.' << group_name << ' ' << title << " : " << strerror_str(eno));
	    }
	}
    }
