...
#ifndef SKVPN_COMMON_RUNCONTEXT_H
#define SKVPN_COMMON_RUNCONTEXT_H
...
#include <skvpn/common/environ.hpp>
#include <skvpn/common/number.hpp>
#include <skvpn/common/signal_name.hpp>
...
#include <skvpn/common/logsetup.hpp>
...
namespace skvpn {
  ...
  public:
    ...
    {
      exit_sock.reset(new skvpn_io::posix::stream_descriptor(io_context, fd.release()));
      exit_sock->async_read_some(skvpn_io::null_buffers(),
				 [self=Ptr(this)](const skvpn_io::error_code& error, const size_t bytes_recvd)
				 {
				   if (!error)
				     self->cancel();
				 });
	...
	return;
	  ...
	  // stop threads
	  {
	    ...
	    SKVPN_LOG(self->prefix << "Stopping " << stopped << '/' << self->servlist.size() << " thread(s)");
	  }
	});
    }
  ...
  protected:
    virtual void signal(const skvpn_io::error_code& error, int signum)
    {
      if (!error && !halt)
	{
	  SKVPN_LOG("ASIO SIGNAL: " << signal_name(signum));
	  switch (signum)
	    {
	    case SIGINT:
	    case SIGTERM:
  ...
  private:
	...
	return;
      ...
      exit_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
                            {
			      if (error || self->halt)
				return;
			      SKVPN_LOG("DEBUG EXIT");
			      self->cancel();
                            });
    }
...
#ifdef ASIO_HAS_LOCAL_SOCKETS
    std::unique_ptr<skvpn_io::posix::stream_descriptor> exit_sock;
#endif
