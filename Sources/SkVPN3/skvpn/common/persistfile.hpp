...
#ifndef SKVPN_COMMON_PERSISTFILE_H
#define SKVPN_COMMON_PERSISTFILE_H
...
namespace skvpn {
  ...
  private:
    ...
    {
      const int eno = errno;
      SKVPN_THROW_EXCEPTION(fn << " : " << type << " error : " << strerror_str(eno));
    }

    void err(const char *type)
    {
      SKVPN_THROW_EXCEPTION(fn << " : " << type << " error");
    }
