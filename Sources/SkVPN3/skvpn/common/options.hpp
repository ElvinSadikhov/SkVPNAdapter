...
// General-purpose options parser, used to parse the SkVPN configuration
...
//
// The parser understands the general grammar of SkVPN configuration
// files including:
//
...
//      ...
//      </ca>
// 3. and meta-directives such as those used by SkVPN Access Server such as:
...
#ifndef SKVPN_COMMON_OPTIONS_H
#define SKVPN_COMMON_OPTIONS_H
...
#include <skvpn/common/option_error.hpp>
...
namespace skvpn {
  ...
  public:
    ...
    void min_args(const size_t n) const
    {
      ...
      if (s < n)
	SKVPN_THROW(option_error, err_ref() << " must have at least " << n << " arguments");
    }
    ...
    void exact_args(const size_t n) const
    {
      const size_t s = data.size();
      if (s != n)
	SKVPN_THROW(option_error, err_ref() << " must have exactly " << n << " arguments");
    }
      ...
      if (max_len > 0 && index < data.size())
	{
	  ...
	  if (status != STATUS_GOOD)
	    SKVPN_THROW(option_error, err_ref() << " is " << validate_status_description(status));
	}
    }
    ...
    static void validate_string(const std::string& name, const std::string& str, const size_t max_len)
    {
      ...
      if (status != STATUS_GOOD)
	SKVPN_THROW(option_error, name << " is " << validate_status_description(status));
    }
      ...
      if (numstr.length() >= 2 && numstr[0] == '0' && numstr[1] == 'x')
	{
	  if (!parse_hex_number(numstr.substr(2), n))
	    SKVPN_THROW(option_error, err_ref() << '[' << idx << "] expecting a hex number");
	}
      else if (!parse_number<T_nonconst>(numstr, n))
	SKVPN_THROW(option_error, err_ref() << '[' << idx << "] must be a number");
      return n;
    }
  ...
  private:
    ...
    {
      SKVPN_THROW(option_error, err_ref() << '[' << idx << "] must be in the range [" << min_value << ',' << max_value << ']');
    }
  ...
  public:
      ...
      if (e != map_.end() && !e->second.empty())
	{
	  if (e->second.size() == 1)
	    ...
	    SKVPN_THROW(option_error, "more than one instance of option '" << name << '\'');
	}
      else
	return nullptr;
    }
      ...
      if (e != map_.end() && !e->second.empty())
	{
	  ...
	  if (e->second.size() >= 2)
	      ...
	      for (size_t i = 1; i < e->second.size(); ++i)
		{
		  ...
		  other->touch();
		  if (*other != *first)
		    SKVPN_THROW(option_error, "more than one instance of option '" << name << "' with inconsistent argument(s)");
		}
	    }
	  return first;
	}
	...
	SKVPN_THROW(option_error, "option '" << name << "' not found");
    }
      ...
      if (il)
	{
	  ...
	  for (i = il->begin(); i != il->end(); ++i)
		...
		SKVPN_THROW(option_error, "option '" << name << "' (" << o.size() << ") must have exactly one parameter");
	    }
	  ret.reserve(size);
	  for (i = il->begin(); i != il->end(); ++i)
	    {
      ...
      if (n_unused())
	{
	  ...
	  SKVPN_LOG_NTNL(title << std::endl << render(Option::RENDER_TRUNC_64|Option::RENDER_NUMBER|Option::RENDER_BRACKET|Option::RENDER_UNUSED));
	}
    }
  ...
  private:
    ...
    {
      SKVPN_THROW(option_error, "line " << line_num << ": " << type << " <" << opt.printable_directive() << "> is followed by extraneous text");
    }

    static void not_closed_out_err(const char *type, const Option& opt)
    {
      SKVPN_THROW(option_error, type << " <" << opt.printable_directive() << "> was not properly closed out");
    }

    static void line_too_long(const int line_num)
    {
      SKVPN_THROW(option_error, "line " << line_num << " is too long");
    }
