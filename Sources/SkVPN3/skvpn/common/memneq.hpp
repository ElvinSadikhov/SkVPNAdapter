...
#if defined(SKVPN_ARCH_x86_64) || defined(SKVPN_ARCH_i386)
#define SKVPN_HAVE_EFFICIENT_UNALIGNED_ACCESS
#endif
...
#define SKVPN_COMPILER_FENCE __memory_barrier();
...
#define SKVPN_COMPILER_FENCE _ReadWriteBarrier();

#elif defined(__GNUC__)

#define SKVPN_COMPILER_FENCE __asm__ __volatile__( "" : : : "memory" );
...
#define SKVPN_INCR_VOID_PTR(var, incr) (var) = static_cast<const unsigned char*>(var) + (incr)
...
namespace skvpn {
  namespace crypto {
...
#else
#ifdef SKVPN_HAVE_EFFICIENT_UNALIGNED_ACCESS
	  ...
	  while (size >= sizeof(memneq_t))
	    {
	      ...
	      SKVPN_INCR_VOID_PTR(a, sizeof(memneq_t));
	      SKVPN_INCR_VOID_PTR(b, sizeof(memneq_t));
      ...
      while (size > 0)
	{
	  ...
	  SKVPN_INCR_VOID_PTR(a, 1);
	  SKVPN_INCR_VOID_PTR(b, 1);
	  size -= 1;
	}
      SKVPN_COMPILER_FENCE
      return bool(neq);
    }
#endif
  }
