...
#ifndef SKVPN_COMMON_REDIR_H
#define SKVPN_COMMON_REDIR_H
...
#include <skvpn/common/tempfile.hpp>
#include <skvpn/common/pipe.hpp>
...
namespace skvpn {
  ...
  struct RedirectBase
  {
    SKVPN_EXCEPTION(redirect_std_err);
  ...
  public:
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(redirect_std_err, "RedirectNull: error opening /dev/null for input : " << strerror_str(eno));
	}
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(redirect_std_err, "RedirectNull: error opening /dev/null for output : " << strerror_str(eno));
	}
  ...
  protected:
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(redirect_std_err, "error opening input file: " << fn << " : " << strerror_str(eno));
	}
    }
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(redirect_std_err, "error opening output file: " << fn << " : " << strerror_str(eno));
	}
    }
  };
  ...
  public:
      ...
      if (!(flags & IGNORE_IN))
	{
	  if (flags & ENABLE_IN)
		...
		{
		  const int eno = errno;
		  SKVPN_THROW(redirect_std_err, "error opening /dev/null : " << strerror_str(eno));
		}
	    }
	}
    }
  ...
  private:
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(redirect_std_err, "error setting FD_CLOEXEC on pipe : " << strerror_str(eno));
	}
      return fd;
    }
