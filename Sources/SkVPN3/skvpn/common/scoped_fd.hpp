...
#ifndef SKVPN_COMMON_SCOPED_FD_H
#define SKVPN_COMMON_SCOPED_FD_H
...
namespace skvpn {
  ...
  public:
    ...
    {
      //SKVPN_LOG("**** SFD REPLACE " << fd << " -> " << fd_arg);
      fd = fd_arg;
    }
      ...
      if (defined())
	{
	  if (::close(fd) == -1)
	    eno = errno;
	  post_close(eno);
	  //SKVPN_LOG("**** SFD CLOSE fd=" << fd << " errno=" << eno);
	  fd = -1;
	}
      return eno;
    }
...
#endif // SKVPN_COMMON_SCOPED_FD_H
