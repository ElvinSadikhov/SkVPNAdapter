...
#ifndef SKVPN_COMMON_EXCEPTION_H
#define SKVPN_COMMON_EXCEPTION_H
...
#include <skvpn/common/stringize.hpp> // for SKVPN_STRINGIZE
#include <skvpn/common/string.hpp>

#ifdef SKVPN_DEBUG_EXCEPTION
  // well-known preprocessor hack to get __FILE__:__LINE__ rendered as a string
# define SKVPN_FILE_LINE "/" __FILE__ ":" SKVPN_STRINGIZE(__LINE__)
#else
# define SKVPN_FILE_LINE
#endif
...
# define SKVPN_SIMPLE_EXCEPTION(C) \
  class C : public std::exception { \
  public: \
    virtual const char* what() const throw() { return #C SKVPN_FILE_LINE; } \
  }

  // define a simple custom exception class with no extra info that inherits from a custom base
# define SKVPN_SIMPLE_EXCEPTION_INHERIT(B, C)	\
  class C : public B { \
  public: \
    C() : B(#C SKVPN_FILE_LINE) {} \
    virtual const char* what() const throw() { return #C SKVPN_FILE_LINE; } \
  }

  // define a custom exception class that allows extra info
# define SKVPN_EXCEPTION(C) \
  class C : public skvpn::Exception { \
  public: \
    C() : skvpn::Exception(#C SKVPN_FILE_LINE) {} \
    C(const std::string err) : skvpn::Exception(#C SKVPN_FILE_LINE ": " + err) {} \
  }

  // define a custom exception class that allows extra info, but does not emit a tag
# define SKVPN_UNTAGGED_EXCEPTION(C) \
  class C : public skvpn::Exception { \
  public: \
    C(const std::string err) : skvpn::Exception(err) {} \
  }

  // define a custom exception class that allows extra info, and inherits from a custom base
# define SKVPN_EXCEPTION_INHERIT(B, C) \
  class C : public B { \
  public: \
    C() : B(#C SKVPN_FILE_LINE) {} \
    C(const std::string err) : B(#C SKVPN_FILE_LINE ": " + err) {} \
  }

  // define a custom exception class that allows extra info, and inherits from a custom base,
  // but does not emit a tag
# define SKVPN_UNTAGGED_EXCEPTION_INHERIT(B, C) \
  class C : public B { \
  public: \
...
# define SKVPN_THROW_EXCEPTION(stuff) \
  do { \
    std::ostringstream _ovpn_exc; \
    _ovpn_exc << stuff; \
    throw skvpn::Exception(_ovpn_exc.str()); \
  } while (0)

  // throw an SKVPN_EXCEPTION class with stringstream concatenation allowed
# define SKVPN_THROW(exc, stuff) \
  do { \
    std::ostringstream _ovpn_exc; \
    _ovpn_exc << stuff; \
...
#endif // SKVPN_COMMON_EXCEPTION_H
