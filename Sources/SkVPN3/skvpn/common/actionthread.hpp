...
#ifndef SKVPN_COMMON_ACTIONTHREAD_H
#define SKVPN_COMMON_ACTIONTHREAD_H
...
namespace skvpn {
  ...
  public:
    ...
    ActionThread(skvpn_io::io_context& io_context_arg,
  ...
  private:
    ...
    void thread_func()
    {
      ...
      bool status = false;
      try {
	SKVPN_LOG("START THREAD...");
	status = actions->execute();
	SKVPN_LOG("END THREAD");
      }
      catch (const std::exception& e)
	{
	  SKVPN_LOG("ActionThread exception: " << e.what());
	}
      skvpn_io::post(io_context, [self=Ptr(this), status]()
		 {
