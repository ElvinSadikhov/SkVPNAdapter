...
namespace skvpn {
  ...
  {
    SKVPN_THROW_EXCEPTION("write_private('" << path << "') : not implemented on Windows yet");
  }
  ...
  inline void write_private(const std::string& path, const void *buf, ssize_t count)
  {
    ...
    if (!fd.defined())
      {
	const int eno = errno;
	SKVPN_THROW_EXCEPTION(path << " : open error : " << strerror_str(eno));
      }
    if (::ftruncate(fd(), 0) < 0)
      {
	const int eno = errno;
	SKVPN_THROW_EXCEPTION(path << " : truncate error : " << strerror_str(eno));
      }
    const ssize_t len = write_retry(fd(), buf, count);
    if (len == -1)
      {
	const int eno = errno;
	SKVPN_THROW_EXCEPTION(path << " : write error : " << strerror_str(eno));
      }
    else if (len != count)
      SKVPN_THROW_EXCEPTION(path << " : unexpected write size");
    if (!fd.close())
      {
	const int eno = errno;
	SKVPN_THROW_EXCEPTION(path << " : close error : " << strerror_str(eno));
      }
  }
