...
#ifndef SKVPN_COMMON_FILE_H
#define SKVPN_COMMON_FILE_H
...
namespace skvpn {
  ...
  SKVPN_UNTAGGED_EXCEPTION(file_exception);
  SKVPN_UNTAGGED_EXCEPTION_INHERIT(file_exception, open_file_error);
  SKVPN_UNTAGGED_EXCEPTION_INHERIT(file_exception, file_too_large);
  SKVPN_UNTAGGED_EXCEPTION_INHERIT(file_exception, file_is_binary);
  SKVPN_UNTAGGED_EXCEPTION_INHERIT(file_exception, file_not_utf8);
  ...
  inline std::string read_text_simple(const std::string& filename)
  {
    ...
    if (!ifs)
      SKVPN_THROW(open_file_error, "cannot open for read: " << filename);
    const std::string str((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
    if (!ifs)
      SKVPN_THROW(open_file_error, "cannot read: " << filename);
    return str;
  }
			       ...
			       const unsigned int buffer_flags = 0)
  {
    ...
    if (max_size && std::uint64_t(length) > max_size)
      SKVPN_THROW(file_too_large, "file too large [" << length << '/' << max_size << "]: " << filename);
    ...
    if (ifs.gcount() != length)
      SKVPN_THROW(open_file_error, "read length inconsistency: " << filename);
    if (!ifs)
      SKVPN_THROW(open_file_error, "cannot read: " << filename);
	...
	if (size)
	  {
	    ...
	    if (max_size && std::uint64_t(total_size) > max_size)
	      SKVPN_THROW(file_too_large, "file too large [" << total_size << '/' << max_size << "]: " << filename);
  ...
  inline std::string read_text(const std::string& filename, const std::uint64_t max_size = 0)
  {
    ...
    if (bp->contains_null())
      SKVPN_THROW(file_is_binary, "file is binary: " << filename);
  ...
  inline std::string read_text_utf8(const std::string& filename, const std::uint64_t max_size = 0)
  {
    ...
    if (!Unicode::is_valid_utf8_uchar_buf(bp->c_data(), bp->size()))
      SKVPN_THROW(file_not_utf8, "file is not UTF8: " << filename);
  ...
  inline void write_binary(const std::string& filename, const Buffer& buf)
  {
    ...
    if (!ofs)
      SKVPN_THROW(open_file_error, "cannot open for write: " << filename);
    ofs.write((const char *)buf.c_data(), buf.size());
    if (!ofs)
      SKVPN_THROW(open_file_error, "cannot write: " << filename);
  }
