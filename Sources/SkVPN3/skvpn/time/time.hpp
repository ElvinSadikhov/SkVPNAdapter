...
namespace skvpn {
  SKVPN_SIMPLE_EXCEPTION(get_time_error);
...
#     define SKVPN_DURATION_REL(OP) bool operator OP(const Duration& d) const { return duration_ OP d.duration_; }
      SKVPN_DURATION_REL(==)
      SKVPN_DURATION_REL(!=)
      SKVPN_DURATION_REL(>)
      SKVPN_DURATION_REL(<)
...
#     undef SKVPN_DURATION_REL
	...
	return "INF";
      ...
      if (df >= 0.0)
	ret += '+';
      ret += skvpn::to_string(df);
      return ret;
    }

#   define SKVPN_TIME_REL(OP) bool operator OP(const TimeType& t) const { return time_ OP t.time_; }
    SKVPN_TIME_REL(==)
    SKVPN_TIME_REL(!=)
    SKVPN_TIME_REL(>)
    SKVPN_TIME_REL(<)
...
#   undef SKVPN_TIME_REL
    ...
    {
#ifdef SKVPN_TIME_NO_BASE
      static_assert(sizeof(base_type) >= 8, "SKVPN_TIME_NO_BASE requires time_t to be 64 bits");
#else
      base_ = ::time(0);
#ifdef SKVPN_PLATFORM_WIN
