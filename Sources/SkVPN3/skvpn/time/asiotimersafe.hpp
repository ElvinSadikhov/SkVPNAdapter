...
namespace skvpn {
  ...
  {
  public:
    AsioTimerSafe(skvpn_io::io_context& io_context)
      : timer_(io_context),
	epoch_(new Epoch)
    {
    }
    ...
    {
      inc_epoch();
      timer_.async_wait([func=std::move(func), epoch=epoch(), eptr=epoch_](const skvpn_io::error_code& error)
			 {
			   func(epoch == eptr->epoch ? error : skvpn_io::error::operation_aborted);
			 });
    }
