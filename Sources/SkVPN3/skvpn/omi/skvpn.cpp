...
// SkVPN 3 client with Management Interface
...
#include <skvpn/asio/asiosignal.hpp>
#include <skvpn/common/stop.hpp>
#include <skvpn/time/asiotimersafe.hpp>
#include <skvpn/omi/omi.hpp>

using namespace skvpn;

std::string log_version()
{
  return platform_string("SkVPN Management Interface", OMI_VERSION)
...
public:
  ...
  OMI(skvpn_io::io_context& io_context, OptionList opt_arg)
    : OMICore(io_context),
  ...
  void start()
  {
    ...
    // passed by SkVPN GUI to trigger exit
    if (!exit_event_name.empty())
    {
        exit_event.assign(::CreateEvent(NULL, FALSE, FALSE, exit_event_name.c_str()));
        exit_event.async_wait([self = Ptr(this)](const skvpn_io::error_code& error)
			      {
				if (error)
				  return;
				self->stop();
  ...
  virtual void log(const ClientAPI::LogInfo& msg) override
  {
    skvpn_io::post(io_context, [this, msg]() {
	log_msg(msg);
      });
  }
  ...
  {
    skvpn_io::post(io_context, [this, ev]() {
  ...
  {
    skvpn_io::post(io_context, [this, ev, ci]() {
	event_msg(ev, &ci);
      });
  }
...
private:
  ...
  {
    try {
      //SKVPN_LOG("OMI START CONNECTION");
  ...
  void schedule_bytecount_timer()
  {
	...
	bytecount_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
				   {
				     if (!error)
  ...
  void report_bytecount()
  {
	...
	send(">BYTECOUNT:" + skvpn::to_string(ts.bytesIn) + ',' + skvpn::to_string(ts.bytesOut) + "\r\n");
      }
  ...
  void deferred_reconnect(const unsigned int seconds, const std::string& reason)
  {
    ...
    reconnect_timer.async_wait([self=Ptr(this), reason](const skvpn_io::error_code& error)
			       {
				 if (!error)
				   {
  ...
  static std::string event_format(const ClientAPI::Event& ev, const ClientAPI::ConnectionInfo* ci)
  {
    ...
    std::string evstr = skvpn::to_string(now) + ',' + ev.name;
  ...
  void event_msg(const ClientAPI::Event& ev, const ClientAPI::ConnectionInfo* ci)
  {
	...
	stop();
      ...
      {
	echo_line(skvpn::to_string(::time(NULL)) + ',' + ev.info + "\r\n");
      }
  ...
  void signal(const skvpn_io::error_code& error, int signum)
  {
    if (!error && !is_stopping())
      {
	SKVPN_LOG("ASIO SIGNAL " << signum);
	switch (signum)
	  {
	  case SIGINT:
	  case SIGTERM:
  ...
  void signal_rearm()
  {
    signals->register_signals_all([self=Ptr(this)](const skvpn_io::error_code& error, int signal_number)
				  {
  ...
  typedef skvpn_io::windows::object_handle AsioEvent;
...
{
  skvpn_io::io_context io_context(1);
  ...
  try {
	...
	io_context.poll(); // execute completion handlers,
      std::cerr << "skvpn: run loop exception: " << e.what() << std::endl;
      ret = 1;
    }
  return ret;
}
...
int main(int argc, char *argv[])
{
  ...
  try {
	...
	std::cout << "Usage: skvpn [args...]" << std::endl;
	ret = 2;
      }
  }
  catch (const std::exception& e)
    {
      std::cerr << "skvpn: " << e.what() << std::endl;
      ret = 1;
    }
  return ret;
}
