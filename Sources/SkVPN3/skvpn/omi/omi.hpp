...
#include <skvpn/win/logutil.hpp>
#else
#include <skvpn/common/redir.hpp>
#endif
...
namespace skvpn {
  ...
  protected:
    ...
    OMICore(skvpn_io::io_context& io_context_arg)
      ...
      if (opt.exists("management-client"))
	{
	  if (port == "unix")
	    {
	      SKVPN_LOG("OMI Connecting to " << addr << " [unix]");
	      connect_unix(addr);
	    }
	  else
	    {
	      SKVPN_LOG("OMI Connecting to [" << addr << "]:" << port << " [tcp]");
      ...
      else
	{
	  if (port == "unix")
	    {
	      SKVPN_LOG("OMI Listening on " << addr << " [unix]");
	      listen_unix(addr);
	    }
	  else
	    {
	      SKVPN_LOG("OMI Listening on [" << addr << "]:" << port << " [tcp]");
    ...
    void log_full(const std::string& text) // logs to OMI buffer and log file
    {
      ...
      log_line(skvpn::to_string(now) + ",," + textcrlf);
#if defined(SKVPN_PLATFORM_WIN)
    ...
    {
      const std::string textcrlf = string::unix2dos(text, true);
      log_line(skvpn::to_string(timestamp) + ",," + textcrlf);
    }
  ...
  private:
    ...
    void stop_omi_client(const bool eof, const unsigned int milliseconds)
    {
      ...
      stop_timer.async_wait([self=Ptr(this), eof](const skvpn_io::error_code& error)
				 {
				   if (!error)
    ...
    {
      send(">INFO:SkVPN Management Interface Version 1 -- type 'help' for more info\r\n");
    }
    ...
    {
      skvpn_io::ip::tcp::endpoint ep(IP::Addr::from_string(addr).to_asio(),
      ...
      s->socket.async_connect(ep,
			      [self=Ptr(this), sock](const skvpn_io::error_code& error) mutable
			      {
    ...
    {
#ifdef ASIO_HAS_LOCAL_SOCKETS
      skvpn_io::local::stream_protocol::endpoint ep(socket_path);
	...
	return;
      ...
      socket->async_receive(buf->mutable_buffer_clamp(),
			    [self=Ptr(this), sock=socket, buf](const skvpn_io::error_code& error, const size_t bytes_recvd)
			    {
    ...
    void handle_recv(const skvpn_io::error_code& error, const size_t bytes_recvd,
	...
	return;
      if (error)
	{
	  const bool eof = (error == skvpn_io::error::eof);
	  if (!eof)
	    SKVPN_LOG("client socket recv error: " << error.message());
	...
	return;
      ...
      socket->async_send(buf.const_buffer_clamp(),
			 [self=Ptr(this), sock=socket](const skvpn_io::error_code& error, const size_t bytes_sent)
			 {
			   self->handle_send(error, bytes_sent, sock.get());
			 });
    }

    void handle_send(const skvpn_io::error_code& error, const size_t bytes_sent,
	...
	return;
      ...
      if (error)
	{
	  SKVPN_LOG("client socket send error: " << error.message());
	...
	{
	  SKVPN_LOG("client socket unexpected send size: " << bytes_sent << '/' << buf->size());
	  conditional_stop(false);
	  return;
	}
