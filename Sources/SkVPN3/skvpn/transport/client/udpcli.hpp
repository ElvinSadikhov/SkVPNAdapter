...
#ifndef SKVPN_TRANSPORT_CLIENT_UDPCLI_H
#define SKVPN_TRANSPORT_CLIENT_UDPCLI_H
...
#include <skvpn/transport/udplink.hpp>
...
namespace skvpn {
  namespace UDPTransport {
	...
	if (!impl)
	  {
		...
		if (config->synchronous_dns_lookup)
		  {
		    skvpn_io::error_code error;
		    skvpn_io::ip::udp::resolver::results_type results = resolver.resolve(server_host, server_port, error);
	...
	SKVPN_LOG("Contacting " << server_endpoint << " via UDP");
