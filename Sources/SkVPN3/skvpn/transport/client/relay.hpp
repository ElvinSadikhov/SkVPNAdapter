...
#ifndef SKVPN_TRANSPORT_CLIENT_RELAY_H
#define SKVPN_TRANSPORT_CLIENT_RELAY_H
...
namespace skvpn {
  ...
  {
  public:
    TransportRelayFactory(skvpn_io::io_context& io_context,
  ...
  private:
	...
	: is_skvpn_protocol(old_parent->transport_is_skvpn_protocol())
      {
      }
    ...
    private:
      ...
      {
	SKVPN_LOG("TransportRelayFactory: Transport Error in null parent: " << Error::name(fatal_err) << " : " << err_text);
      }

      virtual void proxy_error(const Error::Type fatal_err, const std::string& err_text)
      {
	SKVPN_LOG("TransportRelayFactory: Proxy Error in null parent: " << Error::name(fatal_err) << " : " << err_text);
      }

      // Return true if we are transporting SkVPN protocol
      virtual bool transport_is_skvpn_protocol() { return is_skvpn_protocol; }
    ...
    skvpn_io::io_context& io_context_;                        // only used to verify consistency
