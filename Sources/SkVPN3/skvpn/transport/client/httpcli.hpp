...
#ifndef SKVPN_TRANSPORT_CLIENT_HTTPCLI_H
#define SKVPN_TRANSPORT_CLIENT_HTTPCLI_H
...
#include <skvpn/common/userpass.hpp>
#include <skvpn/buffer/bufstr.hpp>
#include <skvpn/buffer/buflimit.hpp>
...
#include <skvpn/http/status.hpp>
#include <skvpn/http/htmlskip.hpp>
#include <skvpn/proxy/proxyauth.hpp>
#include <skvpn/proxy/httpdigest.hpp>
#include <skvpn/proxy/ntlm.hpp>
...
namespace skvpn {
  namespace HTTPProxyTransport {
	...
	return new ClientConfig;
      ...
      virtual TransportClient::Ptr new_transport_client_obj(skvpn_io::io_context& io_context,
    ...
    class Client : public TransportClient, AsyncResolvableTCP
    {
      ...
      typedef TCPTransport::Link<skvpn_io::ip::tcp, Client*, false> LinkImpl;
	...
	virtual void bytes_exceeded() {
	  SKVPN_THROW_EXCEPTION("HTTP proxy response too large (> " << max_bytes << " bytes)");
	}

	virtual void lines_exceeded() {
	  SKVPN_THROW_EXCEPTION("HTTP proxy response too large (> " << max_lines << " lines)");
	}
      };

      Client(skvpn_io::io_context& io_context_arg,
	...
	if (http_reply_status == HTTP::ReplyParser::pending)
	  {
	    SKVPN_LOG_NTNL("FROM PROXY: " << buf_to_string(buf));
		...
		if (http_reply_status != HTTP::ReplyParser::pending)
		  {
		    ...
		    if (http_reply_status == HTTP::ReplyParser::success)
		      {
			//SKVPN_LOG("*** HTTP header parse complete, resid_size=" << buf.size());
			//SKVPN_LOG(http_reply.to_string());
	...
	if (parent->transport_is_skvpn_protocol())
	  {
	    // switch socket from HTTP proxy handshake mode to SkVPN protocol mode
      ...
      // (if SkVPN protocol) is packetized, but input data is still in
	...
	while (!buf.empty())
	  {
		...
		{
		  SKVPN_LOG("Proxy: Skipped " << html_skip->n_bytes() << " byte(s) of HTML");
	...
	if (http_reply_status == HTTP::ReplyParser::success)
	  {
		...
		if (n_transactions <= 1)
		  {
		    //SKVPN_LOG("*** PROXY AUTHENTICATION REQUIRED");
		...
		proxy_error(Error::UNDEF, "HTTP proxy server could not connect to SkVPN server");
		return;
	      }
	    else if (http_reply.status_code == HTTP::Status::Forbidden)
	      SKVPN_THROW_EXCEPTION("HTTP proxy returned Forbidden status code");
	    else
	      SKVPN_THROW_EXCEPTION("HTTP proxy status code: " << http_reply.status_code);
	  }
      ...
      {
	SKVPN_LOG("Proxy method: Basic" << std::endl << pa.to_string());
	...
	start_connect_();
      ...
      {
	try {
	  SKVPN_LOG("Proxy method: Digest" << std::endl << pa.to_string());
	...
	return "";
      ...
      {
	SKVPN_LOG("Proxy method: NTLM" << std::endl << pa.to_string());
	...
	try {
	  //SKVPN_LOG("NTLM phase 3: " << phase_2_response);
	...
	if (!halt)
      ...
      // do DNS resolve
      void resolve_callback(const skvpn_io::error_code& error,
		            skvpn_io::ip::tcp::resolver::results_type results) override
      {
	...
	SKVPN_LOG("Contacting " << server_endpoint << " via HTTP Proxy");
	...
	socket.async_connect(server_endpoint, [self=Ptr(this)](const skvpn_io::error_code& error)
                                              {
                                                SKVPN_ASYNC_HANDLER;
      ...
      void start_impl_(const skvpn_io::error_code& error)
      {
	if (!halt)
	  {
	    if (!error)
		...
		// tell proxy to connect through to SkVPN server
		http_proxy_send();
	      }
	    else
	      {
    ...
    inline TransportClient::Ptr ClientConfig::new_transport_client_obj(skvpn_io::io_context& io_context, TransportClientParent* parent)
    {
