...
#define SKVPN_LOG_MSSFIX(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_MSSFIX(x)
#endif
...
namespace skvpn {
  ...
  public:
      ...
      case IPCommon::IPv6:
	{
	   ...
	   * fragment, mobility) are very unlikely to be seen inside an SkVPN
  ...
  private:
	...
	if (*opt == TCPHeader::OPT_EOL)
	...
	else
	  {
		...
		if (mssval > max_mss)
		  {
		    SKVPN_LOG_MSSFIX("MTU MSS " << mssval << " -> " << max_mss);
