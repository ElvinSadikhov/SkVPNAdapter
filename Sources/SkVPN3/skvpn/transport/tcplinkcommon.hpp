...
#ifndef SKVPN_TRANSPORT_COMMONLINK_H
#define SKVPN_TRANSPORT_COMMONLINK_H
...
#include <skvpn/transport/tcplinkbase.hpp>
#include <skvpn/transport/pktstream.hpp>
#include <skvpn/transport/mutate.hpp>

#ifdef SKVPN_GREMLIN
#include <skvpn/transport/gremlin.hpp>
#endif

#if defined(SKVPN_DEBUG_TCPLINK) && SKVPN_DEBUG_TCPLINK >= 1
#define SKVPN_LOG_TCPLINK_ERROR(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_TCPLINK_ERROR(x)
#endif

#if defined(SKVPN_DEBUG_TCPLINK) && SKVPN_DEBUG_TCPLINK >= 3
#define SKVPN_LOG_TCPLINK_VERBOSE(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_TCPLINK_VERBOSE(x)
#endif
...
namespace skvpn {
  namespace TCPTransport {
    ...
    public:
      ...
      // onto each packet.  The SkVPN protocol runs in non-raw mode, while other
	...
	SKVPN_LOG_TCPLINK_VERBOSE("TCP inject size=" << size);
	...
	return true;
      ...
      {
	SKVPN_LOG_TCPLINK_VERBOSE("TLSLink::queue_recv");
	if (!tcpfrom)
	...
	socket.async_receive(frame_context.mutable_buffer_clamp(tcpfrom->buf),
			     [self=Ptr(this), tcpfrom=PacketFrom::SPtr(tcpfrom)](const skvpn_io::error_code& error, const size_t bytes_recvd) mutable
			     {
			       SKVPN_ASYNC_HANDLER;
			       try
			       {
				 ...
				 // to return status string
				 {
				   ...
				   if (ec && ec->code_defined())
				   {
			         ...
			         SKVPN_LOG_TCPLINK_ERROR("TCP packet extract exception: " << e.what());
	...
	socket.async_send(buf.const_buffer_clamp(),
			  [self=Ptr(this)](const skvpn_io::error_code& error, const size_t bytes_sent)
			  {
			    SKVPN_ASYNC_HANDLER;
	...
	if (!halt)
	  {
	    if (!error)
	      {
		SKVPN_LOG_TCPLINK_VERBOSE("TLS-TCP send raw=" << raw_mode_write << " size=" << bytes_sent);
		...
		if (bytes_sent == buf->size())
	      ...
	      {
		SKVPN_LOG_TCPLINK_ERROR("TLS-TCP send error: " << error.message());
	...
	SKVPN_LOG_TCPLINK_VERBOSE("TLSLink::process_recv_buffer: size=" << buf.size());
	...
	if (!is_raw_mode_read())
	{
	  ...
	  {
	    SKVPN_LOG_TCPLINK_ERROR("TLS-TCP packet extract error: " << e.what());
	...
	return requeue;
      ...
      void handle_recv(PacketFrom::SPtr pfp, const skvpn_io::error_code& error, const size_t bytes_recvd)
      {
	SKVPN_LOG_TCPLINK_VERBOSE("Link::handle_recv: " << error.message());
	if (!halt)
	{
	  if (!error)
	  {
	  ...
	  else if (error == skvpn_io::error::eof)
	  {
	    SKVPN_LOG_TCPLINK_ERROR("TCP recv EOF");
	    read_handler->tcp_eof_handler();
	  }
	  else
	  {
	    SKVPN_LOG_TCPLINK_ERROR("TCP recv error: " << error.message());
