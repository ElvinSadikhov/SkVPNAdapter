...
#ifndef SKVPN_COMPRESS_LZ4_H
#define SKVPN_COMPRESS_LZ4_H
...
namespace skvpn {
  ...
  protected:
      ...
      if (decomp_size < 0)
	{
      ...
      SKVPN_LOG_COMPRESS_VERBOSE("LZ4 uncompress " << buf.size() << " -> " << decomp_size);
      ...
      if (comp_size < buf.size())
	{
	  ...
	  SKVPN_LOG_COMPRESS_VERBOSE("LZ4 compress " << buf.size() << " -> " << comp_size);
  ...
  public:
	...
	asym(asym_arg)
    {
      SKVPN_LOG_COMPRESS("LZ4 init asym=" << asym_arg);
    }
  ...
  public:
	...
	asym(asym_arg)
    {
      SKVPN_LOG_COMPRESS("LZ4v2 init asym=" << asym_arg);
    }
