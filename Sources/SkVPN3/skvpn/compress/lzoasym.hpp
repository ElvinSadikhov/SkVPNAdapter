...
#ifndef SKVPN_COMPRESS_LZOASYM_H
#define SKVPN_COMPRESS_LZOASYM_H

#include <skvpn/compress/lzoasym_impl.hpp>
...
namespace skvpn {
  ...
  public:
    ...
    SKVPN_SIMPLE_EXCEPTION(lzo_init_failed);
    ...
    {
      SKVPN_LOG_COMPRESS("LZO-ASYM init swap=" << support_swap_arg << " asym=" << asym_arg);
    }
      ...
      if (err != lzo_asym_impl::LZOASYM_E_OK)
	{
      ...
      SKVPN_LOG_COMPRESS_VERBOSE("LZO-ASYM uncompress " << buf.size() << " -> " << zlen);
      work.set_size(zlen);
      buf.swap(work);
    }
...
#endif // SKVPN_COMPRESS_LZOASYM_H
