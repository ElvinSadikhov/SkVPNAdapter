...
// the SkVPN protocol, but it always sends packets with NO_COMPRESS
...
#ifndef SKVPN_COMPRESS_COMPSTUB_H
#define SKVPN_COMPRESS_COMPSTUB_H
...
namespace skvpn {
  ...
  public:
...
#endif
    {
      SKVPN_LOG_COMPRESS("Comp-stub init swap=" << support_swap_arg);
    }
	...
	case CompressLZO::LZO_COMPRESS:
	  SKVPN_LOG_COMPRESS_VERBOSE("CompressStub: handled unsolicited LZO packet");
	  lzo.decompress_work(buf);
	  break;
#endif
	default: 
	  SKVPN_LOG_COMPRESS_VERBOSE("CompressStub: unable to handle op=" << int(c));
	  error(buf);
	}
    }
  ...
  public:
    CompressStubV2(const Frame::Ptr& frame, const SessionStats::Ptr& stats)
      : Compress(frame, stats)
    {
      SKVPN_LOG_COMPRESS("Comp-stubV2 init");
    }
	...
	return;
      ...
      if (cop)
	{
	  SKVPN_LOG_COMPRESS_VERBOSE("CompressStubV2: unable to handle op=" << c);
	  error(buf);
	}
    }
  };
...
#endif // SKVPN_COMPRESS_COMPSTUB_H
