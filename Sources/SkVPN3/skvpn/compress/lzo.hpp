...
#ifndef SKVPN_COMPRESS_LZO_H
#define SKVPN_COMPRESS_LZO_H
...
namespace skvpn {
  ...
  public:
	...
	asym(asym_arg)
    {
      SKVPN_LOG_COMPRESS("LZO init swap=" << support_swap_arg << " asym=" << asym_arg);
      ...
      if (err != LZO_E_OK)
	{
      ...
      SKVPN_LOG_COMPRESS_VERBOSE("LZO uncompress " << buf.size() << " -> " << zlen);
      work.set_size(zlen);
      buf.swap(work);
    }
      ...
      if (hint && !asym)
	{
	  ...
	  if (zlen < buf.size())
	    {
	      SKVPN_LOG_COMPRESS_VERBOSE("LZO compress " << buf.size() << " -> " << zlen);
	      work.set_size(zlen);
	      if (support_swap)
