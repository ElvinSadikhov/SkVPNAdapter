...
#ifndef SKVPN_COMPRESS_SNAPPY_H
#define SKVPN_COMPRESS_SNAPPY_H
...
namespace skvpn {
  ...
  public:
	...
	asym(asym_arg)
    {
      SKVPN_LOG_COMPRESS("SNAPPY init asym=" << asym_arg);
    }
      ...
      if (hint && !asym)
	{
	  ...
	  if (comp_size < buf.size())
	    {
	      SKVPN_LOG_COMPRESS_VERBOSE("SNAPPY compress " << buf.size() << " -> " << comp_size);
	...
	case SNAPPY_COMPRESS:
	  {
	    ...
	    SKVPN_LOG_COMPRESS_VERBOSE("SNAPPY uncompress " << buf.size() << " -> " << decomp_size);
