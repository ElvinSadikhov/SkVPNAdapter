...
namespace skvpn {
  namespace Acceptor {
	...
	acceptor.async_accept(sock->socket, [listener=ListenerBase::Ptr(listener), sock](const skvpn_io::error_code& error) mutable
			      {
	...
	SKVPN_LOG("ACCEPTOR CLOSE " << local_endpoint);
#endif
	acceptor.close();
      }
	...
	if (!(flags & DISABLE_REUSE_ADDR))
	  acceptor.set_option(skvpn_io::ip::tcp::acceptor::reuse_address(true));
#else
	...
	return flags & (DISABLE_REUSE_ADDR|REUSE_PORT);
      ...
      skvpn_io::ip::tcp::endpoint local_endpoint;
      skvpn_io::ip::tcp::acceptor acceptor;
    };
