...
namespace skvpn {
  namespace Acceptor {
    ...
    {
    public:
      SKVPN_EXCEPTION(named_pipe_acceptor_error);

      typedef RCPtr<NamedPipe> Ptr;

      NamedPipe(skvpn_io::io_context& io_context,
	...
	: name(name_arg),
      ...
      virtual void async_accept(ListenerBase* listener,
				...
				skvpn_io::io_context& io_context) override
      {
	...
	if (!Win::Handle::defined(h))
	  {
	    ...
	    SKVPN_THROW(named_pipe_acceptor_error, "failed to create named pipe: " << name << " : " << err.message());
	  }

	// wait for connection (asynchronously)
	{
	  handle.assign(h);
	  skvpn_io::windows::overlapped_ptr over(
	      io_context,
	      [self=Ptr(this), listener=ListenerBase::Ptr(listener), acceptor_index]
	      (const skvpn_io::error_code& ec, size_t bytes_transferred) {
	  ...
	  if (!ok && err != ERROR_IO_PENDING)
	    {
	      ...
	      const skvpn_io::error_code ec(err, skvpn_io::error::get_system_category());
	      over.complete(ec, 0);
	    }
