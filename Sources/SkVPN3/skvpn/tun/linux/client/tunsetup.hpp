...
#ifndef SKVPN_TUN_LINUX_CLIENT_TUNSETUP_H
#define SKVPN_TUN_LINUX_CLIENT_TUNSETUP_H
...
#include <skvpn/netconf/linux/gw.hpp>

namespace skvpn {
  namespace TunLinuxSetup {

    SKVPN_EXCEPTION(tun_linux_error);
    SKVPN_EXCEPTION(tun_open_error);
    SKVPN_EXCEPTION(tun_layer_error);
    SKVPN_EXCEPTION(tun_ioctl_error);
    SKVPN_EXCEPTION(tun_fcntl_error);
    SKVPN_EXCEPTION(tun_name_error);
    SKVPN_EXCEPTION(tun_tx_queue_len_error);
    SKVPN_EXCEPTION(tun_ifconfig_error);
	...
	if (!fd.defined())
	  SKVPN_THROW(tun_open_error, "error opening tun device " << node << ": " << errinfo(errno));
	...
	if (!name.empty())
	  {
	    ...
	    const int eno = errno;
	    SKVPN_THROW(tun_ioctl_error, "failed to open tun device '" << name << "' after trying " << max_units << " units : " << errinfo(eno));
	  }
	else
	  {
	    if (ioctl (fd(), TUNSETIFF, (void *) &ifr) < 0)
	      {
		const int eno = errno;
		SKVPN_THROW(tun_ioctl_error, "failed to open tun device '" << name << "' : " << errinfo(eno));
	      }
	  }
      }
