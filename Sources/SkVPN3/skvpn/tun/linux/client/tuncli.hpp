...
#ifndef SKVPN_TUN_LINUX_CLIENT_TUNCLI_H
#define SKVPN_TUN_LINUX_CLIENT_TUNCLI_H

#include <skvpn/asio/asioerr.hpp>
#include <skvpn/common/cleanup.hpp>
#include <skvpn/common/scoped_fd.hpp>
#include <skvpn/tun/builder/setup.hpp>
#include <skvpn/tun/tunio.hpp>
#include <skvpn/tun/persist/tunpersist.hpp>
...
namespace skvpn {
  namespace TunLinux {
    ...
    class Tun : public TunIO<ReadHandler, PacketFrom, skvpn_io::posix::stream_descriptor>
    {
      typedef TunIO<ReadHandler, PacketFrom, skvpn_io::posix::stream_descriptor> Base;

    public:
      typedef RCPtr<Tun> Ptr;

      Tun(skvpn_io::io_context& io_context,
	...
	Base::stream = new skvpn_io::posix::stream_descriptor(io_context, socket);
	SKVPN_LOG_TUN(Base::name_ << " opened");
      }
    ...
    class Client : public TunClient
    {
      ...
      friend class TunIO<Client*, PacketFrom, skvpn_io::posix::stream_descriptor>;  // calls tun_read_handler
	...
	if (!impl)
	  {
	    ...
	    if (config->tun_persist)
	      {
		SKVPN_LOG("TunPersist: long-term session scope");
		tun_persist = config->tun_persist; // long-term persistent
	      }
	    else
	      {
		SKVPN_LOG("TunPersist: short-term connection scope");
	      ...
	      if (tun_persist->use_persisted_tun(server_addr, config->tun_prop, opt))
		{
		  ...
		  sd = tun_persist->obj();
		  state = tun_persist->state();
		  SKVPN_LOG("TunPersist: reused tun context");
		}
	      else
	        {
		  // notify parent
		  ...
		  SKVPN_LOG("CAPTURED OPTIONS:" << std::endl << po->to_string());
	...
	parent.tun_recv(pfp->buf);
      ...
      void tun_error_handler(const Error::Type errtype, // called by TunImpl
			     const skvpn_io::error_code* error)
      {
      }
...
#endif // SKVPN_TUN_LINUX_CLIENT_TUNCLI_H
