...
#include <skvpn/tun/client/tunprop.hpp>
#include <skvpn/tun/linux/client/tunsetup.hpp>
#include <skvpn/netconf/linux/gw.hpp>

namespace skvpn {
  namespace TunIPRoute {

    using namespace skvpn::TunLinuxSetup;
	  ...
	  if (flags & R_ADD_SYS)
	    {
	      ...
	      add->argv.push_back(net.to_string() + '/' + skvpn::to_string(prefix_len));
	...
	if (mtu > 0)
	  {
	    ...
	    add->argv.push_back(skvpn::to_string(mtu));
	  }
	create.add(add);
      ...
      if (local4)
	{
	  ...
	  add->argv.push_back(local4->address + '/' + skvpn::to_string(local4->prefix_length));
	  ...
	  if (unit >= 0)
	    {
	      ...
	      add->argv.push_back(iface_name + ':' + skvpn::to_string(unit));
	    }
	  create.add(add);
	...
	add->argv.push_back("addr");
	add->argv.push_back("add");
	add->argv.push_back(local6->address + '/' + skvpn::to_string(local6->prefix_length));
	add->argv.push_back("dev");
	add->argv.push_back(iface_name);
	create.add(add);
	  ...
	  for (const auto &route : pull.add_routes)
	      ...
	      else
		{
		  if (local4 && !local4->gateway.empty())
		    ...
		    SKVPN_LOG("ERROR: IPv4 route pushed without IPv4 ifconfig and/or route-gateway");
		}
	    }
	}
	  ...
	  for (const auto &route : pull.exclude_routes)
	    {
	      if (route.ipv6)
		{
		  SKVPN_LOG("NOTE: exclude IPv6 routes not supported yet"); // fixme
		}
	      else
		{
		  if (gw.v4.defined())
		    ...
		    SKVPN_LOG("NOTE: cannot determine gateway for exclude IPv4 routes");
		}
	    }
	}
