...
//    Copyright (C) 2018-2020 Antonio Quartulli <antonio@skvpn.net>
//
...
#include <skvpn/addr/ipv4.hpp>
#include <skvpn/addr/ipv6.hpp>
#include <skvpn/addr/route.hpp>


#ifdef DEBUG_RTNL
#define SKVPN_LOG_RTNL(_x) SKVPN_LOG(_x)
#else
#define SKVPN_LOG_RTNL(_x)
#endif
...
#define NLMSG_TAIL(nmsg) \
	...
	{
	  SKVPN_LOG(__func__ << ": rtnl: message exceeded bound of " << maxlen);
	  return -EMSGSIZE;
	}
	...
	if (fd < 0)
	{
	  SKVPN_LOG(__func__ << ": cannot open netlink socket");
	  return fd;
	}

	if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf)) < 0)
	{
	  SKVPN_LOG(__func__ << ": SO_SNDBUF");
	  close(fd);
	  return -1;
	}
	...
	{
	  SKVPN_LOG(__func__ << ": cannot bind netlink socket");
	  return -errno;
	}
	...
	{
	  SKVPN_LOG(__func__ << ": cannot getsockname");
	  return -errno;
	}

	if (addr_len != sizeof(local))
	{
	  SKVPN_LOG(__func__ << ": wrong address length " << addr_len);
	  return -EINVAL;
	}

	if (local.nl_family != AF_NETLINK)
	{
	  SKVPN_LOG(__func__ << ": wrong address family " << local.nl_family);
	  return -EINVAL;
	}
	...
	if (fd < 0)
	{
	  SKVPN_LOG(__func__ << ": can't open rtnl socket");
	  return -errno;
	}
	...
	if (ret < 0)
	{
	  SKVPN_LOG(__func__ << ": can't bind rtnl socket");
	  ret = -errno;
	  goto out;
	}
	...
	if (ret < 0)
	{
	  SKVPN_LOG(__func__ << ": rtnl: error on sendmsg()");
	  ret = -errno;
	  goto out;
	}
	...
	while (1)
	{
	   ...
	   * using it again
	   */
	  SKVPN_LOG_RTNL(__func__ << ": checking for received messages");
	  iov.iov_len = sizeof(buf);
	  rcv_len = recvmsg(fd, &nlmsg, 0);
	  SKVPN_LOG_RTNL(__func__ << ": rtnl: received " << rcv_len << " bytes");
	  if (rcv_len < 0)
	  {
	    if ((errno == EINTR) || (errno == EAGAIN))
	    {
	      SKVPN_LOG(__func__ << ": interrupted call");
	      continue;
	    }
	    SKVPN_LOG(__func__ << ": rtnl: error on recvmsg()");
	    ret = -errno;
	    goto out;
	  }
	  ...
	  if (rcv_len == 0)
	  {
	    SKVPN_LOG(__func__ << ": rtnl: socket reached unexpected EOF");
	    ret = -EIO;
	    goto out;
	  }
	  ...
	  {
	    SKVPN_LOG(__func__ << ": sender address length: "
	  ...
	  while (rcv_len >= (int)sizeof(*h))
	  {
	    ...
	    if ((rem_len < 0) || (len > rcv_len))
	    {
	      ...
	      {
		SKVPN_LOG(__func__ << ": truncated message");
		ret = -EIO;
		goto out;
	      }
	      SKVPN_LOG(__func__ << ": malformed message: len=" << len);
	      ret = -EIO;
	      goto out;
	    }
	    ...
	    if (h->nlmsg_type == NLMSG_ERROR)
	    {
	      ...
	      {
		SKVPN_LOG(__func__ << ": ERROR truncated");
		ret = -EIO;
	      }
	      else
	      {
		...
		{
		  SKVPN_LOG(__func__ << ": rtnl: generic error: "
			      << strerror(-err->error)
			      << " (" << err->error << ")");
		  ret = err->error;
		}
	    ...
	    {
	      SKVPN_LOG(__func__ << ": RTNL: unexpected reply");
	    }
	  ...
	  if (nlmsg.msg_flags & MSG_TRUNC)
	  {
	    SKVPN_LOG(__func__ << ": message truncated");
	    continue;
	  }

	  if (rcv_len)
	  {
	    SKVPN_LOG(__func__ << ": rtnl: " << rcv_len
			<< " not parsed bytes");
	    ret = -1;
	    goto out;
	  }
...
out:
	...
	{
	  SKVPN_LOG_RTNL(__func__ << ": RTA_GATEWAY " << gw.to_string());
	}

	if (!route.contains(res->dst))
	{
	  SKVPN_LOG_RTNL(__func__ << ": Ignore gw for unmatched route " << route.to_string());
	  return 0;
	}
	...
	{
	  SKVPN_LOG(__func__ << ": rtnl: can't get ifname for index "
		      << ifindex);
	  return -1;
	}
	...
	{
	  SKVPN_LOG_RTNL(__func__ << ": Ignore gw " << gw.to_string() << " on " << iface);
	  return 0;
	}
	...
	{
	  SKVPN_LOG_RTNL(__func__ << ": Ignore gw " << gw.to_string() << " with shorter route prefix " << route.to_string());
	  return 0;
	}
	...
	{
	  SKVPN_LOG_RTNL(__func__ << ": Ignore gw " << gw.to_string() << " with higher metrics " << metric);
	  return 0;
	}
	...
	SKVPN_LOG_RTNL(__func__ << ": Use gw " << gw.to_string() << " route " << route.to_string() << " metric " << metric);
	...
	if (ret >= 0)
	{
	  ...
	  SKVPN_LOG(__func__ << " result: via " << best_gw << " dev " << best_iface);
	}
	else
	{
	  SKVPN_LOG(__func__ << ": failed to retrieve route, err=" << ret);
	}
...
err:
	...
	if (iface.empty())
	{
	  SKVPN_LOG(__func__ << ": passed empty interface");
	  return -EINVAL;
	}

	if (local.unspecified())
	{
	  SKVPN_LOG(__func__ << ": passed zero IP address");
	  return -EINVAL;
	}
	...
	{
	  SKVPN_LOG(__func__ << ": cannot get ifindex for " << iface << " "
		      << strerror(errno));
	  return -ENOENT;
	}
...
err:
	...
	if (!iface.empty())
	{
	  ...
	  if (ifindex == 0)
	  {
	    SKVPN_LOG(__func__ << ": rtnl: cannot get ifindex for " << iface);
	    return -ENOENT;
	  }
...
err:
	...
	SKVPN_LOG(__func__ << " query IPv6: " << route);
	...
	SKVPN_LOG(__func__ << " query IPv4: " << route);
	...
	SKVPN_LOG(__func__ << ": add " << iface << " type " << type);
...
err:
	...
	SKVPN_LOG(__func__ << ": set " << iface << " " << (up ? "up" : "down"));
	...
	SKVPN_LOG(__func__ << ": mtu " << mtu << " for " << iface);
...
err:
	return sitnl_send(&req.n, 0, 0, NULL, NULL);
      ...
      {
	SKVPN_LOG(__func__ << ": " << addr << "/" << prefixlen << " brd "
		    << broadcast << " dev " << iface);
	...
	return sitnl_addr_add(iface, IP::Addr::from_ipv4(addr), prefixlen,
      ...
      net_addr_add(const std::string& iface, const IPv6::Addr& addr,
		   const int prefixlen)
      {
	SKVPN_LOG(__func__ << ": " << addr << "/" << prefixlen << " dev " << iface);
	...
	return sitnl_addr_del(iface, IP::Addr::from_ipv6(addr), prefixlen);
      ...
      {
	SKVPN_LOG(__func__ << ": " << local << " peer " << remote << " dev " << iface);
	...
	return sitnl_addr_ptp_add(iface, IP::Addr::from_ipv4(local),
      ...
      net_addr_ptp_del(const std::string& iface, const IPv4::Addr& local,
		       const IPv4::Addr& remote)
      {
	SKVPN_LOG(__func__ << ": " << local << " dev " << iface);
	...
	return sitnl_addr_ptp_del(iface, IP::Addr::from_ipv4(local));
      ...
      net_route_add(const IP::Route4& route, const IPv4::Addr& gw,
		    ...
		    const int metric)
      {
	SKVPN_LOG(__func__ << ": " << route << " via " << gw << " dev " << iface
