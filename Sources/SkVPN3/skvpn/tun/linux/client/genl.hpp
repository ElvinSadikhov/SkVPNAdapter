...
//    Copyright (C) 2020-2020 Lev Stipakov <lev@skvpn.net>
//
...
public:
   ...
   */
  explicit GeNL(skvpn_io::io_context &io_context, unsigned int ifindex_arg,
    ...
    if (ret)
      SKVPN_THROW(netlink_error, "failed to join mcast group: " << ret);
    ...
    stream.reset(new skvpn_io::posix::stream_descriptor(
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " start_vpn() nla_put_failure");
  }
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " new_peer() nla_put_failure");
  }
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " send_data() nla_put_failure");
  }
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " set_keys() nla_put_failure");
  }
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " del_key() nla_put_failure");
  }
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, " set_peer() nla_put_failure");
  }
...
private:
  ...
  nla_put_failure:
    SKVPN_THROW(netlink_error, "get_mcast_id() nla_put_failure");
  }

  void handle_read(const skvpn_io::error_code &error) {
    if (halt)
      return;
  ...
  void queue_genl_read() {
    stream->async_wait(skvpn_io::posix::stream_descriptor::wait_read,
  ...
  void read_netlink_message() {
    ...
    if (ovpn_dco_err != 0)
      SKVPN_THROW(netlink_error, "ovpn-dco error on receiving message: "
                                       << strerror(-ovpn_dco_err) << ", "
                                       << ovpn_dco_err);

    if (netlink_err < 0)
      SKVPN_THROW(netlink_error, "netlink error on receiving message: "
  ...
  static int message_received(struct nl_msg *msg, void *arg) {
    ...
    case OVPN_CMD_DEL_PEER:
      if (!attrs[OVPN_ATTR_DEL_PEER_REASON])
        SKVPN_THROW(netlink_error, "missing OVPN_ATTR_DEL_PEER_REASON "
    ...
    default:
      SKVPN_LOG(__func__ << " unknown netlink command: " << (int)gnlh->cmd);
    }
  ...
  static int ovpn_nl_cb_error(struct sockaddr_nl* /*nla*/,
    ...
    *ret = err->error;
    ...
    if (tb_msg[NLMSGERR_ATTR_MSG]) {
      ...
      SKVPN_LOG(__func__ << " kernel error "
  ...
  void send_netlink_message(struct nl_msg *msg) {
    ...
    if (netlink_err < 0)
      SKVPN_THROW(netlink_error, "netlink error on sending message: "
                                       << nl_geterror(netlink_err) << ", "
                                       << netlink_err);
  }
  ...
  std::unique_ptr<skvpn_io::posix::stream_descriptor> stream;
};
} // namespace skvpn