...
#ifndef SKVPN_TUN_CLIENT_TUNPROP_H
#define SKVPN_TUN_CLIENT_TUNPROP_H
...
#include <skvpn/addr/addrpair.hpp>
#include <skvpn/client/remotelist.hpp>
#include <skvpn/client/ipverflags.hpp>
#include <skvpn/tun/client/emuexr.hpp>
...
namespace skvpn {
  ...
  public:
    SKVPN_EXCEPTION(tun_prop_error);
    SKVPN_EXCEPTION(tun_prop_route_error);
    SKVPN_EXCEPTION(tun_prop_dhcp_option_error);
      ...
      if (ipv.rgv4() && !(dhcp_option_flags & F_ADD_DNS))
	{
	  if (config.google_dns_fallback)
	    {
	      if (!quiet)
		SKVPN_LOG("Google DNS fallback enabled");
	      add_google_dns(tb);
	    }
  ...
  private:
	...
	{
	  if (!quiet)
	    SKVPN_LOG("exception processing route-metric: " << e.what());
	}
    }
	  ...
	  if (dopt != opt.map().end())
	      ...
	      for (OptionList::IndexList::const_iterator i = dopt->second.begin(); i != dopt->second.end(); ++i)
		{
		  ...
		  try {
		    ...
		    {
		      if (!quiet)
			SKVPN_LOG("exception parsing IPv4 route: " << o.render(OPT_RENDER_FLAGS) << " : " << e.what());
		    }
		}
	    }
	}
	  ...
	  if (dopt != opt.map().end())
	      ...
	      for (OptionList::IndexList::const_iterator i = dopt->second.begin(); i != dopt->second.end(); ++i)
		{
		  ...
		  try {
		    ...
		    {
		      if (!quiet)
			SKVPN_LOG("exception parsing IPv6 route: " << o.render(OPT_RENDER_FLAGS) << " : " << e.what());
		    }
		}
	    }
	}
      ...
      for (IP::AddrList::const_iterator i = addrlist.begin(); i != addrlist.end(); ++i)
	{
	  ...
	  if (addr != server_addr)
		...
		{
		  if (!quiet)
		    SKVPN_LOG("exception adding remote bypass route: " << addr.to_string() << " : " << e.what());
		}
	    }
	}
    }
      ...
      if (dopt != opt.map().end())
	{
	  ...
	  for (OptionList::IndexList::const_iterator i = dopt->second.begin(); i != dopt->second.end(); ++i)
		...
		if (type == "DNS" || type == "DNS6")
		  ...
		  SKVPN_LOG("Unknown pushed DHCP option: " << o.render(OPT_RENDER_FLAGS));
	      }
	      catch (const std::exception& e)
		{
		  if (!quiet)
		    SKVPN_LOG("exception parsing dhcp-option: " << o.render(OPT_RENDER_FLAGS) << " : " << e.what());
		}
	    }
	  try {
	    if (!http_host.empty())
	    ...
	    {
	      if (!quiet)
		SKVPN_LOG("exception setting dhcp-option for proxy: " << e.what());
	    }
	}
      return flags;
    }
...
#endif // SKVPN_TUN_CLIENT_TUNPROP_H
