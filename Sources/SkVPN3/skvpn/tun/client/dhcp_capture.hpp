...
#ifndef SKVPN_TUN_CLIENT_DHCP_CAPTURE_H
#define SKVPN_TUN_CLIENT_DHCP_CAPTURE_H
...
namespace skvpn {
  ...
  public:
	...
	SKVPN_LOG("NOTE: pushed ifconfig directive is ignored in layer 2 mode");
      if (!props->dns_servers.empty())
	SKVPN_LOG("NOTE: pushed DNS servers are ignored in layer 2 mode");
      reset();
    }
	  ...
	  && dhcp->dhcp.op == DHCP::BOOTREPLY)
	{
	  ...
	  if (message_type == DHCP::DHCPACK || message_type == DHCP::DHCPOFFER)
	      ...
	      if (message_type == DHCP::DHCPACK && !configured)
		{
		  ...
		  if (host.unspecified())
		    {
		      SKVPN_LOG("NOTE: failed to obtain host address via DHCP");
		      complete = false;
		    }
		  if (netmask.unspecified())
		    {
		      SKVPN_LOG("NOTE: failed to obtain netmask via DHCP");
		      complete = false;
		    }
		  if (prefix_len < 0)
		    {
		      SKVPN_LOG("NOTE: bad netmask obtained via DHCP: " << netmask);
		      complete = false;
		    }
		  if (router.unspecified())
		    {
		      SKVPN_LOG("NOTE: failed to obtain router via DHCP");
		      complete = false;
		    }
		  if (complete)
		    {
			...
			SKVPN_LOG("NOTE: failed to obtain DNS servers via DHCP");
		      else
			{
