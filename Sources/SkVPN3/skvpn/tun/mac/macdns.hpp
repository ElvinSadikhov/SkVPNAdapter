...
#ifndef SKVPN_TUN_MAC_MACDNS_H
#define SKVPN_TUN_MAC_MACDNS_H
...
namespace skvpn {
  ...
  public:
	...
	prev = info;
	if (mod)
	  SKVPN_LOG("MacDNS: SETDNS " << ver.to_string() << std::endl << info->to_string());
      }
      catch (const std::exception& e)
	{
	  SKVPN_LOG("MacDNS: setdns exception: " << e.what());
	}
      return mod;
    }
	...
	if (mod)
	  SKVPN_LOG("MacDNS: RESETDNS " << ver.to_string() << std::endl << info->to_string());
      }
      catch (const std::exception& e)
	{
	  SKVPN_LOG("MacDNS: resetdns exception: " << e.what());
	}
      return mod;
    }
