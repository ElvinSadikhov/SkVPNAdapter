...
#ifndef SKVPN_TUN_MAC_GWV4_H
#define SKVPN_TUN_MAC_GWV4_H
...
#include <skvpn/addr/macaddr.hpp>
...
#   define SKVPN_ROUNDUP(a)			\
      ((a) > 0 ? (1 + (((a) - 1) | (sizeof(std::uint32_t) - 1))) : sizeof(std::uint32_t))

#   define SKVPN_NEXTADDR(w, u)		\
      if (rtm_addrs & (w)) {			\
        l = SKVPN_ROUNDUP(u.sa_len);		\
        std::memmove(cp, &(u), l);		\
        cp += l;				\
      }

#   define SKVPN_ADVANCE(x, n)		\
      (x += SKVPN_ROUNDUP((n)->sa_len))

  public:
    SKVPN_EXCEPTION(route_gateway_error);
      ...
      : flags_(0)
    {
      ...
      SKVPN_NEXTADDR(RTA_DST, so_dst);
      SKVPN_NEXTADDR(RTA_NETMASK, so_mask);
