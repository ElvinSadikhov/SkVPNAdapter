...
#ifndef SKVPN_TUN_MAC_MACDNS_WATCHDOG_H
#define SKVPN_TUN_MAC_MACDNS_WATCHDOG_H
...
#include <skvpn/tun/mac/macdns.hpp>

namespace skvpn {
  SKVPN_EXCEPTION(macdns_watchdog_error);
  ...
  public:
    ...
    MacDNSWatchdog()
      : macdns(new MacDNS("SkVPNConnect")),
	thread(nullptr)
    {
    }
  ...
  private:
	  ...
	  if (flags & ENABLE_WATCHDOG)
	      ...
	      else
		{
		  if (runloop.defined())
		    ...
		    SKVPN_LOG("MacDNSWatchdog::setdns: runloop undefined");
		}
	    }
	  else
	    {
	...
	{
	  SKVPN_LOG("MacDNSWatchdog::thread_func exception: " << e.what());
	}
      cancel_push_timer();
    }
	...
	SKVPN_LOG("MacDNSWatchdog::schedule_push_timer: failed to create timer");
    }
	...
	if (macdns->setdns(*config))
	  SKVPN_LOG("MacDNSWatchdog: updated DNS settings");
      }
      catch (const std::exception& e)
	{
	  SKVPN_LOG("MacDNSWatchdog::push_timer_callback exception: " << e.what());
	}
    }
