...
namespace skvpn {
  class DSDict {
	...
	if (dirty())
	  {
	    ...
	    if (SCDynamicStoreSetValue(sc(), keystr(), mod()))
	      {
		SKVPN_LOG("DSDict: updated " << dskey);
		return true;
	      }
	    else
	      SKVPN_LOG("DSDict: ERROR updating " << dskey);
	  }
	return false;
      }
      ...
      bool remove_from_store()
      {
        ...
        if (SCDynamicStoreRemoveValue(sc(), keystr()))
          {
            SKVPN_LOG("DSDict: removed " << dskey);
            return true;
          }
        else
          {
            SKVPN_LOG("DSDict: ERROR removing " << dskey);
            return false;
          }
      }
	...
	SKVPN_LOG("DSDict: SCDynamicStoreNotifyValue " << key);
