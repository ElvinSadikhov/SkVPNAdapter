...
#ifndef SKVPN_TUN_BUILDER_CAPTURE_H
#define SKVPN_TUN_BUILDER_CAPTURE_H
...
#include <skvpn/client/rgopt.hpp>
...
namespace skvpn {
  ...
  public:
	...
	if (require_canonical && !route.is_canonical())
	  SKVPN_THROW_EXCEPTION(title << " : not a canonical route: " << route);
	if (!gateway.empty())
	  IP::Addr(gateway, title + ".gateway", ver);
	if (net30 && route.prefix_len != 30)
	  SKVPN_THROW_EXCEPTION(title << " : not a net30 route: " << route);
      }
    };
	...
	try {
	  ...
	  {
	    SKVPN_THROW_EXCEPTION(title << " : error parsing ProxyAutoConfigURL: " << e.what());
	  }
      }
  ...
  private:
    ...
    static void validate_list(const LIST& list, const std::string& title)
    {
      ...
      for (auto &e : list)
	{
	  e.validate(title + '[' + skvpn::to_string(i) + ']');
	  ++i;
	}
    }
	...
	SKVPN_THROW_EXCEPTION(title << ".tunnel_address_index_ipv4 : IPv4 tunnel address index out of range: " << tunnel_address_index_ipv4);
      if (!validate_tunnel_index(tunnel_address_index_ipv6))
	SKVPN_THROW_EXCEPTION(title << ".tunnel_address_index_ipv6 : IPv6 tunnel address index out of range: " << tunnel_address_index_ipv6);
      const RouteAddress* r4 = vpn_ipv4();
      if (r4 && r4->ipv6)
	SKVPN_THROW_EXCEPTION(title << ".tunnel_address_index_ipv4 : IPv4 tunnel address index points to wrong address type: " << r4->to_string());
      const RouteAddress* r6 = vpn_ipv6();
      if (r6 && !r6->ipv6)
	SKVPN_THROW_EXCEPTION(title << ".tunnel_address_index_ipv6 : IPv6 tunnel address index points to wrong address type: " << r6->to_string());
    }

    void validate_mtu(const std::string& title) const
    {
      if (mtu < 0 || mtu > 65536)
	SKVPN_THROW_EXCEPTION(title << ".mtu : MTU out of range: " << mtu);
    }

    void validate_layer(const std::string& title) const
    {
      if (!layer.defined())
	SKVPN_THROW_EXCEPTION(title << ": layer undefined");
    }
  };
}
