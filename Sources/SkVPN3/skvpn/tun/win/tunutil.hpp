...
#ifndef SKVPN_TUN_WIN_TUNUTIL_H
#define SKVPN_TUN_WIN_TUNUTIL_H

#include <skvpn/common/socktypes.hpp> // prevent winsock multiple def errors
...
#include <skvpn/win/unicode.hpp>
#include <skvpn/win/cmd.hpp>
#include <skvpn/win/winerr.hpp>

namespace skvpn {
  namespace TunWin {
    namespace Util {
      SKVPN_EXCEPTION(tun_win_util);
	...
	const char COMPONENT_ID[] = SKVPN_STRINGIZE(TAP_WIN_COMPONENT_ID); // CONST GLOBAL
	const char WINTUN_COMPONENT_ID[] = "wintun"; // CONST GLOBAL

	const char ROOT_COMPONENT_ID[] = "root\\" SKVPN_STRINGIZE(TAP_WIN_COMPONENT_ID);
	...
	if (status != ERROR_SUCCESS)
	  {
	    ...
	    SKVPN_THROW(tun_win_util, "tap_guids: error opening adapter registry key: " << ADAPTER << " : " << err.message());
	  }
	...
	for (int i = 0;; ++i)
	  {
	    ...
	    if (status == ERROR_NO_MORE_ITEMS)
	      ...
	      SKVPN_THROW(tun_win_util, "tap_guids: error enumerating registry subkeys of key: " << ADAPTER);
	    strbuf[len] = '\0';
	...
	std::string index_or_name() const
	{
	  if (index_defined())
	    ...
	    SKVPN_THROW(tun_win_util, "TapNameGuidPair: TAP interface " << guid << " has no name or interface index");
	}
	...
	TapNameGuidPairList(bool wintun)
	{
	  ...
	  // next, match up control panel interface names with GUIDs
	  {
	    ...
	    if (status != ERROR_SUCCESS)
	      {
		const Win::Error err(status);
		SKVPN_THROW(tun_win_util, "TapNameGuidPairList: error opening network connections registry key: " << NETWORK_CONNECTIONS << " : " << err.message());
	      }
		...
		if (status == ERROR_NO_MORE_ITEMS)
		  ...
		  SKVPN_THROW(tun_win_util, "TapNameGuidPairList: error enumerating registry subkeys of key: " << NETWORK_CONNECTIONS);
		strbuf[len] = '\0';
	...
	while (::DeviceIoControl(th, TAP_WIN_IOCTL_GET_LOG_LINE,
				 ...
				 &len, nullptr))
	  {
	    SKVPN_LOG("TAP-Windows: " << line.get());
	  }
      }
	...
	InterfaceInfoList()
	{
	  ...
	  if (::GetInterfaceInfo(nullptr, &size) != ERROR_INSUFFICIENT_BUFFER)
	    SKVPN_THROW(tun_win_util, "InterfaceInfoList: GetInterfaceInfo #1");
	  list.reset((IP_INTERFACE_INFO*)new unsigned char[size]);
	  if (::GetInterfaceInfo(list.get(), &size) != NO_ERROR)
	    SKVPN_THROW(tun_win_util, "InterfaceInfoList: GetInterfaceInfo #2");
	}
	...
	IPAdaptersInfo()
	{
	  ...
	  if (::GetAdaptersInfo(nullptr, &size) != ERROR_BUFFER_OVERFLOW)
	    SKVPN_THROW(tun_win_util, "IPAdaptersInfo: GetAdaptersInfo #1");
	  list.reset((IP_ADAPTER_INFO*)new unsigned char[size]);
	  if (::GetAdaptersInfo(list.get(), &size) != NO_ERROR)
	    SKVPN_THROW(tun_win_util, "IPAdaptersInfo: GetAdaptersInfo #2");
	}
				   ...
				   const std::string& str)
	{
	  ...
	  if (len)
	    {
	      if (len > 255)
		SKVPN_THROW(dhcp_masq, "string '" << str << "' must be > 0 bytes and <= 255 bytes");
	      buf.push_back(type);
	  ...
	  if (!addr_list.empty())
	    {
	      ...
	      if (size < 1 || size > 255)
		SKVPN_THROW(dhcp_masq, "array size=" << size << " must be > 0 bytes and <= 255 bytes");
	      buf.push_back(type);
	...
	virtual void execute(std::ostream& os) override
	{
	  ...
	  if (status != ERROR_SUCCESS)
	    {
	      const Win::Error err(status);
	      SKVPN_THROW(tun_win_util, "ActionSetAdapterDomainSuffix: error opening registry key: " << reg_key_name << " : " << err.message());
	    }
	  ...
	  if (status != ERROR_SUCCESS)
	    SKVPN_THROW(tun_win_util, "ActionSetAdapterDomainSuffix: error writing Domain registry key: " << reg_key_name);
	}
	...
	if (status == ERROR_INSUFFICIENT_BUFFER)
	  {
	    ...
	    if (status != NO_ERROR)
	      {
		SKVPN_LOG("windows_routing_table: GetIpForwardTable failed");
		return nullptr;
	      }
	  }
	return rt.release();
	...
	BestGateway(const std::string& dest, DWORD vpn_interface_index)
	{
	  ...
	  switch (res)
	    {
	    case -1:
	      SKVPN_THROW(tun_win_util, "GetBestGateway: error converting IPv4 address " << dest << " to int: " << ::WSAGetLastError());

	    case 0:
	      SKVPN_THROW(tun_win_util, "GetBestGateway: " << dest << " is not a valid IPv4 address");
	    }

	  {
	    ...
	    if (res2 != NO_ERROR)
	      {
		SKVPN_THROW(tun_win_util, "GetBestGateway: error retrieving the best route for " << dest << ": " << res2);
	      }
	  ...
	  if (rt)
	      ...
	      for (size_t i = 0; i < rt->dwNumEntries; ++i)
		{
		  ...
		  if ((dest_addr & row->dwForwardMask) == (row->dwForwardDest & row->dwForwardMask))
			...
			{
			  SKVPN_LOG("GetBestGateway: skip gateway " <<
	      ...
	      if (gw)
		{
		  index = gw->dwForwardIfIndex;
		  addr = IPv4::Addr::from_uint32(ntohl(gw->dwForwardNextHop)).to_string();
		  SKVPN_LOG("GetBestGateway: selected gateway " << addr << " on adapter " << index << " for destination " << dest);
		}
	    }
	}
	      ...
	      for (size_t i = 0; i < rt->dwNumEntries; ++i)
		{
		  ...
		  if (row->dwForwardIfIndex == index)
			...
			continue;
		      ...
		      actions.add(new WinCmd("netsh interface ip delete route " + net_str + '/' + skvpn::to_string(pl) + ' ' + skvpn::to_string(index) + " store=active"));
		    }
		}
	    }
	}
		  ...
		  if (row->InterfaceIndex == index)
		      ...
		      if (row->DestinationPrefix.Prefix.si_family == AF_INET6)
			{
			  ...
			  if ((net & ll_mask) == ll_net && pl >= 64)
			    continue;
			  actions.add(new WinCmd("netsh interface ipv6 delete route " + net_str + '/' + skvpn::to_string(pl) + ' ' + skvpn::to_string(index) + " store=active"));
			}
		    }
		}
	    }
	...
	public:
		       ...
		       bool add) : add(add)
	  {
		...
		if (err)
		  SKVPN_THROW(tun_win_util, "Cannot convert interface name " << tap.name << " to LUID");
