...
#ifndef SKVPN_TUN_WIN_CLIENT_TUNCLI_H
#define SKVPN_TUN_WIN_CLIENT_TUNCLI_H
...
#include <skvpn/asio/scoped_asio_stream.hpp>
...
#include <skvpn/tun/client/dhcp_capture.hpp>
#include <skvpn/tun/persist/tunpersist.hpp>
#include <skvpn/tun/persist/tunwrapasio.hpp>
#include <skvpn/tun/tunio.hpp>
#include <skvpn/tun/win/client/clientconfig.hpp>
#include <skvpn/tun/win/client/tunsetup.hpp>
#include <skvpn/win/modname.hpp>
#include <skvpn/tun/win/client/wintun.hpp>

namespace skvpn {
  namespace TunWin {

    SKVPN_EXCEPTION(tun_win_error);
	...
	if (!impl)
	  {
	      ...
	      else
		{
		  ...
		  if (tun_persist->persist_tun_state(ts, { state, nullptr }))
		    SKVPN_LOG("TunPersist: saving tun context:" << std::endl << tun_persist->options());
	...
	try {
	  if (dhcp_capture->mod_reply(buf))
	    {
	      SKVPN_LOG("DHCP PROPS:" << std::endl << dhcp_capture->get_props().to_string());
	...
	l2_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
			    {
			      SKVPN_ASYNC_HANDLER;
			      if (!error && !self->halt)
	...
	try {
	  if (dhcp_capture && tun_setup)
		...
		{
		  SKVPN_LOG("L2: Waiting for DHCP handshake...");
		  layer_2_schedule_timer(1);
		}
	    }
	}
