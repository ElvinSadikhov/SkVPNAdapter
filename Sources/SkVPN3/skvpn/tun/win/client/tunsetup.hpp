...
#ifndef SKVPN_TUN_WIN_CLIENT_TUNSETUP_H
#define SKVPN_TUN_WIN_CLIENT_TUNSETUP_H
...
#include <skvpn/tun/proxy.hpp>
#include <skvpn/tun/win/tunutil.hpp>
#include <skvpn/tun/win/winproxy.hpp>
#include <skvpn/tun/win/tunutil.hpp>
#include <skvpn/tun/win/client/setupbase.hpp>
#include <skvpn/win/scoped_handle.hpp>
#include <skvpn/win/cmd.hpp>

#if _WIN32_WINNT >= 0x0600 // Vista+
#include <skvpn/tun/win/nrpt.hpp>
#include <skvpn/tun/win/wfp.hpp>
#endif

#include <versionhelpers.h>

// use IP Helper on Windows by default
#ifdef SKVPN_USE_NETSH
...
namespace skvpn {
  namespace TunWin {
    ...
    public:
      ...
      Setup(skvpn_io::io_context& io_context_arg, bool wintun_arg=false)
	...
	switch (pull.layer())
	  ...
	  case Layer::OSI_LAYER_3:
	    adapter_config(th(), skvpn_app_path, tap, pull, false, *add_cmds, *remove_cmds, os);
	    break;
	  case Layer::OSI_LAYER_2:
	    adapter_config_l2(th(), skvpn_app_path, tap, pull, *add_cmds, *remove_cmds, os);
	    break;
	  default:
	...
	if (pull.layer() == Layer::OSI_LAYER_2)
	  l2_state.reset(new L2State(tap, skvpn_app_path));
	...
	if (l2s)
	  {
	    Win::ScopedHANDLE nh;
	    ActionList::Ptr add_cmds(new ActionList());
	    adapter_config(nh(), l2s->skvpn_app_path, l2s->tap, pull, true, *add_cmds, *remove_cmds, os);
	    add_cmds->execute(os);
	  }
      }
	...
	if (!ipv6)
	  {
		...
		remove_cmds_bypass_gw.add(new WinCmd("netsh interface ip delete route " + route + "/32 " + to_string(gw.interface_index()) + ' ' + gw.gateway_address() + " store=active"));
	      ...
	      {
		SKVPN_LOG("Skip bypass route to " << route << ", route is local");
	      }
	  }
      }
	...
	L2State(const Util::TapNameGuidPair& tap_arg,
		const std::wstring& skvpn_app_path_arg)
	  : tap(tap_arg),
	...
	if (local4)
	  {
		...
		delete_route_timer.async_wait([self=Ptr(this), cmd=std::move(cmd)](const skvpn_io::error_code& error)
					      {
						if (!error)
						  {
						    std::ostringstream os;
	...
	// Usage: delete dnsservers [name=]<string> [[address=]<IPv6 address>|all] [[validate=]yes|no]
	{
	  ...
	  // the TAP adapter.
	  if (use_wfp && !split_dns && !skvpn_app_path.empty() && (dns.ipv4() || dns.ipv6()))
	    {
	      create.add(new ActionWFP(skvpn_app_path, tap.index, true, wfp));
	      destroy.add(new ActionWFP(skvpn_app_path, tap.index, false, wfp));
	    }
	}
	...
	SKVPN_LOG("proxy_auto_config_url " << pull.proxy_auto_config_url.url);
