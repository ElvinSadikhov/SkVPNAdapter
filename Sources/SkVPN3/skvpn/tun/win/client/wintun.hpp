...
namespace skvpn {
  namespace TunWin {
    ...
    public:
      WintunClient(skvpn_io::io_context& io_context_arg,
	...
	while (true)
	  {
	    ...
	    if (head == tail)
	      {
		ring_buffer->send_tail_moved_asio_event().async_wait([self = Ptr(this)](const skvpn_io::error_code& error) {
		  if (!error)
		    self->read();
		  else
		    {
