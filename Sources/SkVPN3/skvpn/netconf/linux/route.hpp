...
#ifndef SKVPN_NETCONF_LINUX_ROUTE_H
#define SKVPN_NETCONF_LINUX_ROUTE_H
...
namespace skvpn {
  ...
  {
  public:
    SKVPN_EXCEPTION(linux_route_error);
	...
	{
	  const int eno = errno;
	  SKVPN_THROW(linux_route_error, "add_delete: sendmsg failed: " << strerror_str(eno));
	}
    }
    ...
    static int if_index(const std::string& iface)
    {
      ...
      if (!ret)
	SKVPN_THROW(linux_route_error, "if_index: no such interface: " << iface);
      return ret;
    }
