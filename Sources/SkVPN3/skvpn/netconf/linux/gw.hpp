...
#ifndef SKVPN_NETCONF_LINUX_GW_H
#define SKVPN_NETCONF_LINUX_GW_H
...
namespace skvpn {
  ...
  {
  public:
    SKVPN_EXCEPTION(linux_gw_error);
      ...
      while (sl())
	{
	  ...
	  try {
	    ...
	    {
	      if (!ignore_errors)
		SKVPN_THROW(linux_gw_error, "error parsing line: " << line << " : " << e.what());
	    }
	}
    }
	...
	argv.emplace_back("-6");
      ...
      if (status != 0)
	SKVPN_THROW(linux_gw_error, "command returned error status " << status << " : " << argv.to_string());
      return pipe.out;
    }
  ...
  private:
    ...
    {
      if (dev.empty())
	SKVPN_THROW_EXCEPTION("dev is empty");
      return dev;
    }
