...
#ifndef SKVPN_CRYPTO_CRYPTOALGS_H
#define SKVPN_CRYPTO_CRYPTOALGS_H
...
namespace skvpn {
  namespace CryptoAlgs {
    ...
    SKVPN_EXCEPTION(crypto_alg);
    SKVPN_SIMPLE_EXCEPTION(crypto_alg_index);
    ...
    enum AlgFlags {       // bits below must start after Mode bits
      ...
      F_ALLOW_DC=(1<<4),  // alg may be used in SkVPN data channel
      ...
      for (size_t i = 0; i < SIZE; ++i)
	{
      ...
      SKVPN_THROW(crypto_alg, name << ": not found");
    }
	...
	SKVPN_THROW(crypto_alg, alg.name() << ": bad cipher for data channel use");
      return type;
    }
	...
	SKVPN_THROW(crypto_alg, alg.name() << ": bad digest for data channel use");
      return type;
    }
