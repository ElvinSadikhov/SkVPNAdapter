...
#ifndef SKVPN_APPLE_MACLIFE_H
#define SKVPN_APPLE_MACLIFE_H
...
#include <skvpn/apple/cf/cftimer.hpp>
#include <skvpn/apple/cf/cfhelper.hpp>
#include <skvpn/apple/cf/cfrunloop.hpp>
#include <skvpn/apple/reachable.hpp>
#include <skvpn/client/clilife.hpp>
#include <skvpn/apple/macsleep.hpp>
#include <skvpn/apple/scdynstore.hpp>

namespace skvpn {
  class MacLifeCycle : public ClientLifeCycle, MacSleep, ReachabilityTracker
  {
  public:
    SKVPN_EXCEPTION(mac_lifecycle_error);
  ...
  private:
	...
	{
	  SKVPN_LOG("MacLifeCycle exception: " << e.what());
	}
    ...
    void iface_watch()
    {
      ...
      CF::DynamicStore ds(SCDynamicStoreCreate(kCFAllocatorDefault,
					       CFSTR("SkVPN_MacLifeCycle_iface_watch"),
    ...
    void iface_watch_callback(SCDynamicStoreRef store, CFArrayRef changedKeys)
    {
      ...
      SKVPN_LOG("MacLifeCycle NET_IFACE " << state.iface);
      schedule_action_timer(1);
    }

    virtual void notify_sleep()
    {
      SKVPN_LOG("MacLifeCycle SLEEP");
      state.sleep = true;
      ...
      if (rb.vtype() == ReachabilityBase::Internet)
	{
	  ...
	  SKVPN_LOG("MacLifeCycle NET_STATE " << state.net_up << " status=" << ReachabilityBase::render_status(status) << " flags=" << ReachabilityBase::render_flags(flags));
	  schedule_action_timer(1);
	}
    }
      ...
      if (seconds)
	{
	  ...
	  if (action_timer.defined())
	    ...
	    SKVPN_LOG("MacLifeCycle::schedule_action_timer: failed to create timer");
	}
      else
	...
	if (state != prev_state)
	  {
	    SKVPN_LOG("MacLifeCycle ACTION pause=" << paused << " state=" << state.to_string() << " prev=" << prev_state.to_string());
	    if (paused)
	      {
	...
	{
	  SKVPN_LOG("MacLifeCycle::action_timer_callback exception: " << e.what());
	}
    }
