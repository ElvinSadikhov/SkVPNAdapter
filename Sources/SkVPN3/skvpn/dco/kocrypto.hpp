  ...
  namespace KoRekey {
	...
	switch (cipher)
	  ...
	  case CryptoAlgs::NONE:
	    ...
	    break;
	  default:
	    SKVPN_THROW(korekey_error, "cipher alg " << calg.name() << " is not currently supported by kovpn");
	  }
	...
	if (calg.mode() == CryptoAlgs::CBC_HMAC)
	  {
		...
		break;
	      default:
		SKVPN_THROW(korekey_error, "HMAC alg " << halg.name() << " is not currently supported by kovpn");
	      }
	  }
      }
	...
	if (sk.size() < size_required)
	  SKVPN_THROW(korekey_error, title << ": insufficient key material, provided=" << sk.size() << " required=" << size_required);
	return sk.data();
      }
	...
	if (dest_size < NONCE_TAIL_SIZE)
	  SKVPN_THROW(korekey_error, title << ": cannot set");
...
#ifdef ENABLE_KOVPN
#include <skvpn/kovpn/kovpnkocrypto.hpp>
#elif ENABLE_OVPNDCO
#include <skvpn/dco/ovpndcokocrypto.hpp>
#else
