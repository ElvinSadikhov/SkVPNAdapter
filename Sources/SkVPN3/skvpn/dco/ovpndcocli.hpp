...
public:
  ...
  virtual void start_impl_udp(const skvpn_io::error_code &error) override {
    if (halt)
      return;
    ...
    if (!error) {
      ...
      if (tb) {
        ...
        pipe.reset(new skvpn_io::posix::stream_descriptor(io_context, fd));
  ...
  virtual void stop_() override {
    if (!halt) {
      ...
      if (config->builder) {
        ...
        if (res != 0) {
          SKVPN_LOG("ovpndcocli: error deleting iface ovpn:" << os.str());
        }
      }
    }
  }
  ...
  void rekey_impl(const CryptoDCInstance::RekeyType rktype,
    ...
    case CryptoDCInstance::DEACTIVATE_ALL:
      ...
      SKVPN_LOG("ovpndcocli: deactivate all keys");
      break;

    default:
      SKVPN_LOG("ovpndcocli: unknown rekey type: " << rktype);
      break;
    }
  }
  ...
  bool tun_read_handler(BufferAllocated &buf) {
    ...
    default:
      SKVPN_LOG("Unknown ovpn-dco cmd " << cmd);
      break;
    }
...
private:
  OvpnDcoClient(skvpn_io::io_context &io_context_arg,
  ...
  void queue_read_pipe(PacketFrom *pkt) {
    ...
    pipe->async_read_some(
        ...
        [self = Ptr(this),
         pkt = PacketFrom::SPtr(pkt)](const skvpn_io::error_code &error,
          ...
          if (!error) {
            ...
            if (!self->halt) {
              SKVPN_LOG("ovpn-dco pipe read error: " << error.message());
              self->stop_();
  ...
  std::unique_ptr<skvpn_io::posix::stream_descriptor> pipe;
