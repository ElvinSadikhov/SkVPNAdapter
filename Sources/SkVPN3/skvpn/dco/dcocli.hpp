...
#include <skvpn/buffer/asiobuf.hpp>
...
#include <skvpn/tun/linux/client/tunmethods.hpp>
#include <skvpn/transport/dco.hpp>

#ifdef ENABLE_KOVPN
#include <skvpn/kovpn/kovpn.hpp>
#include <skvpn/kovpn/kostats.hpp>
#include <skvpn/kovpn/rps_xps.hpp>
#include <skvpn/kovpn/kodevtun.hpp>
#elif ENABLE_OVPNDCO
#include <skvpn/dco/key.hpp>
#include <skvpn/tun/linux/client/sitnl.hpp>
#include <skvpn/common/uniqueptr.hpp>
#include <skvpn/tun/linux/client/genl.hpp>
#include <skvpn/buffer/buffer.hpp>
#else
#error either ENABLE_KOVPN or ENABLE_OVPNDCO must be defined
#endif

#include <skvpn/dco/korekey.hpp>
...
namespace skvpn {
  namespace DCOTransport {
	...
	return TransportClientFactory::Ptr(this);
      ...
      virtual TunClient::Ptr new_tun_client_obj(skvpn_io::io_context& io_context,
	...
	ProtoBase& operator=(const ProtoBase&) = delete;
      ...
      {
	explicit UDP(skvpn_io::io_context& io_context)
	  : resolver(io_context),
	    socket(io_context)
	{
	}
	...
	skvpn_io::ip::udp::resolver resolver;
	...
	if (config->transport.remote_list->endpoint_available(&server_host, &server_port, nullptr))
      ...
      void resolve_callback(const skvpn_io::error_code& error,
			    skvpn_io::ip::udp::resolver::results_type results) override
      {
	if (!halt)
	  {
	    if (!error)
	...
	SKVPN_LOG("Contacting " << udp().server_endpoint << " via UDP");
	...
	udp().socket.async_connect(udp().server_endpoint, [self=Ptr(this)](const skvpn_io::error_code& error)
                                                          {
      ...
      virtual void start_impl_udp(const skvpn_io::error_code& error) = 0;

      void transport_start_tcp()
      {
	SKVPN_THROW(dco_error, "TCP not implemented yet"); // fixme for DCO
      }
...
#ifdef ENABLE_KOVPN
    #include <skvpn/kovpn/kovpncli.hpp>
    inline DCO::Ptr new_controller()
    {
      return KovpnClientConfig::new_controller();
    }
    inline TransportClient::Ptr ClientConfig::new_transport_client_obj(skvpn_io::io_context& io_context,
								       TransportClientParent* parent)
    {
      return TransportClient::Ptr(new KovpnClient(io_context, this, parent));
    }
#elif ENABLE_OVPNDCO
    #include <skvpn/dco/ovpndcocli.hpp>
    inline DCO::Ptr new_controller()
    {
...
#error either ENABLE_KOVPN or ENABLE_OVPNDCO must be defined
    ...
    inline TunClient::Ptr ClientConfig::new_tun_client_obj(skvpn_io::io_context& io_context,
							   TunClientParent& parent,
