...
#ifndef SKVPN_ASIO_ASIOBOUNDSOCK_H
#define SKVPN_ASIO_ASIOBOUNDSOCK_H
...
namespace skvpn {
  namespace AsioBoundSocket {
    ...
    typedef skvpn_io::basic_stream_socket<skvpn_io::ip::tcp> SocketBase;

    class Socket : public SocketBase
    {
    public:
      explicit Socket(skvpn_io::io_context& io_context)
	...
	if (bind_local_addr.defined())
	  {
	    ...
	    ret += "]:";
	    ret += skvpn::to_string(bind_local_port);
	  }
	try {
	  const std::string re = skvpn::to_string(remote_endpoint());
	  if (!ret.empty())
	    ret += ' ';
	  ret += "remote=";
	  ret += re;
    ...
    protected:
      virtual void async_connect_post_open(const protocol_type& protocol, skvpn_io::error_code& ec) override
      {
	if (bind_local_addr.defined())
	  {
	    set_option(skvpn_io::socket_base::reuse_address(true), ec);
	    if (ec)
	      return;
	    bind(skvpn_io::ip::tcp::endpoint(bind_local_addr.to_asio(), bind_local_port), ec);
	  }
      }
