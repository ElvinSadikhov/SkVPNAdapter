...
#ifndef SKVPN_ASIO_ASIOSTOP_H
#define SKVPN_ASIO_ASIOSTOP_H
...
namespace skvpn {
  ...
  {
  public:
    AsioStopScope(skvpn_io::io_context& io_context,
		  Stop* stop,
  ...
  private:
    static std::function<void()> post_method(skvpn_io::io_context& io_context, std::function<void()>&& method)
    {
      return [&io_context, method=std::move(method)]()
	{
	  skvpn_io::post(io_context, std::move(method));
	};
    }
  };
