...
#ifndef SKVPN_OPENSSL_UTIL_PEM_H
#define SKVPN_OPENSSL_UTIL_PEM_H
...
out:
	...
	if (!PEM_read_bio(bio, &name_read, &header_read, &data_read,
			  &data_read_len))
	{
	  SKVPN_LOG("PEM decode failed");
	  goto out;
	}

	if (key_name.compare(std::string(name_read)))
	{
	  SKVPN_LOG("unexpected PEM name (got '" << name_read <<
		      "', expected '" << key_name << "')");
	  goto out;
	}
...
#endif /* SKVPN_OPENSSL_UTIL_PEM_H */
