...
// that it can be used as part of the crypto layer of the SkVPN core.

#ifndef SKVPN_OPENSSL_CRYPTO_CIPHER_H
#define SKVPN_OPENSSL_CRYPTO_CIPHER_H
...
#include <skvpn/openssl/compat.hpp>
...
namespace skvpn {
  namespace OpenSSLCrypto {
    ...
    public:
      SKVPN_SIMPLE_EXCEPTION(openssl_cipher_mode_error);
      SKVPN_SIMPLE_EXCEPTION(openssl_cipher_uninitialized);
      SKVPN_EXCEPTION(openssl_cipher_error);
	...
	switch (alg)
	  ...
	  case CryptoAlgs::BF_CBC:
	    return EVP_bf_cbc();
	  default:
	    SKVPN_THROW(openssl_cipher_error, CryptoAlgs::name(alg) << ": not usable");
	  }
      }
