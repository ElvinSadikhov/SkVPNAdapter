...
// so that it can be used as the SSL layer by the SkVPN core.

#ifndef SKVPN_OPENSSL_SSL_SSLCTX_H
#define SKVPN_OPENSSL_SSL_SSLCTX_H
...
#include <skvpn/common/uniqueptr.hpp>
...
#include <skvpn/pki/cclist.hpp>
#include <skvpn/pki/epkibase.hpp>
#include <skvpn/ssl/kuparse.hpp>
#include <skvpn/ssl/nscert.hpp>
#include <skvpn/ssl/tlsver.hpp>
#include <skvpn/ssl/tls_remote.hpp>
#include <skvpn/ssl/verify_x509_name.hpp>
#include <skvpn/ssl/sslconsts.hpp>
#include <skvpn/ssl/sslapi.hpp>
#include <skvpn/ssl/ssllog.hpp>
#include <skvpn/ssl/sni_handler.hpp>
#include <skvpn/ssl/iana_ciphers.hpp>
#include <skvpn/openssl/util/error.hpp>
#include <skvpn/openssl/pki/extpki.hpp>
#include <skvpn/openssl/pki/x509.hpp>
#include <skvpn/openssl/pki/crl.hpp>
#include <skvpn/openssl/pki/pkey.hpp>
#include <skvpn/openssl/pki/dh.hpp>
#include <skvpn/openssl/pki/x509store.hpp>
#include <skvpn/openssl/pki/x509certinfo.hpp>
#include <skvpn/openssl/bio/bio_memq_stream.hpp>
#include <skvpn/openssl/ssl/sess_cache.hpp>
#include <skvpn/openssl/ssl/tlsver.hpp>


#ifdef HAVE_JSON
#include <skvpn/common/jsonhelper.hpp>
#endif
...
namespace skvpn {
  ...
  public:
	...
	return TLSVersion::V1_0;
      ...
      Mode mode;
      CertCRLList ca;                   // from SkVPN "ca" and "crl-verify" option
      OpenSSLPKI::X509 cert;            // from SkVPN "cert" option
      OpenSSLPKI::X509List extra_certs; // from SkVPN "extra-certs" option
	...
	if (status < 0)
	  {
	    if (status == -1 && BIO_should_retry(ssl_bio))
	      ...
	      {
		mark_no_cache();
		SKVPN_THROW(OpenSSLException, "OpenSSLContext::SSL::write_cleartext: BIO_write failed, size=" << size << " status=" << status);
	      }
	  }
	else
	  return status;
	...
	if (!overflow)
	  {
		...
		if (status == -1 && BIO_should_retry(ssl_bio))
		  ...
		  {
		    mark_no_cache();
		    SKVPN_THROW(OpenSSLException, "OpenSSLContext::SSL::read_cleartext: BIO_read failed, cap=" << capacity << " status=" << status);
		  }
	      }
	    else
	      return status;
	...
	try {
	  ...
	  if (ctx.config->mode.is_server())
	    ...
	    SKVPN_THROW(ssl_context_error, "OpenSSLContext::SSL: unknown client/server mode");
    ...
    {
      // SkVPN 2.x accepts IANA ciphers instead in the cipher list, we need
      // to do the same
      ...
      while(std::getline(cipher_list_ss, ciphersuite, ':'))
	{
	  ...
	  if (pair)
		...
		{
		  SKVPN_LOG_SSL("OpenSSLContext: Deprecated cipher suite name '"
  ...
  private:
      ...
      try
	{
	  ...
	  if (config->mode.is_server())
		...
		SKVPN_THROW(ssl_context_error, "OpenSSLContext: DH not defined");
...
#define OPENSSL_SERVER_SNI
...
#else
		  SKVPN_THROW(ssl_context_error, "OpenSSLContext: server-side SNI requires OpenSSL 1.1 or higher");
#endif
		}
	    }
	    ...
	    SKVPN_THROW(ssl_context_error, "OpenSSLContext: unknown config->mode");
	...
	if (config->tls_version_min > TLSVersion::V1_0)
	  ...
	  if (!config->tls_ciphersuite_list.empty())
		...
		SKVPN_THROW(ssl_context_error, "OpenSSLContext: SSL_CTX_set_ciphersuites_list failed");
	    }
#endif
	  ...
	  if (!SSL_CTX_set_cipher_list(ctx, tls_cipher_list))
	      SKVPN_THROW(ssl_context_error, "OpenSSLContext: SSL_CTX_set_cipher_list failed");
	  ...
	  case TLSCertProfile::UNDEF:
	    SKVPN_THROW(ssl_context_error,
			  "OpenSSLContext: undefined tls-cert-profile");
	    break;
#ifdef SKVPN_USE_TLS_MD5
	  ...
	  if (config->local_cert_enabled)
		...
		SKVPN_THROW(ssl_context_error, "OpenSSLContext: cert not defined");
	      ...
	      else
		{
		  if (!config->pkey.defined())
		    SKVPN_THROW(ssl_context_error, "OpenSSLContext: private key not defined");
	  ...
	  if (config->ca.certs.defined())
	    ...
	    SKVPN_THROW(ssl_context_error, "OpenSSLContext: CA not defined");
  ...
  private:
      ...
      while (std::getline(groups_ss, group, ':'))
	{
	  ...
	  if (nid != 0)
	    ...
	    {
	      SKVPN_LOG_SSL("OpenSSL -- warning ignoring unknown group '"
				<< group << "' in tls-groups");
	    }
	}

      if (!SSL_CTX_set1_groups(ctx, glist.get(), glistlen))
	SKVPN_THROW(ssl_context_error, "OpenSSLContext: SSL_CTX_set1_groups failed");
	...
	SKVPN_LOG_SSL(cert_status_line(preverify_ok, depth, X509_STORE_CTX_get_error(ctx),
				 signature, subject));
      ...
      if (depth == 0)
	{
	  ...
	  if (self->ns_cert_type_defined() && !self->verify_ns_cert_type(current_cert))
	    {
	      SKVPN_LOG_SSL("VERIFY FAIL -- bad ns-cert-type in leaf certificate");
	      preverify_ok = false;
	    }
	  ...
	  if (self->x509_cert_ku_defined() && !self->verify_x509_cert_ku(current_cert))
	    {
	      SKVPN_LOG_SSL("VERIFY FAIL -- bad X509 key usage in leaf certificate");
	      preverify_ok = false;
	    }
	  ...
	  if (self->x509_cert_eku_defined() && !self->verify_x509_cert_eku(current_cert))
	    {
	      SKVPN_LOG_SSL("VERIFY FAIL -- bad X509 extended key usage in leaf certificate");
	      preverify_ok = false;
	    }
	  ...
	  if (verify_x509.get_mode() != VerifyX509Name::VERIFY_X509_NONE)
	  {
	    ...
	    if (!preverify_ok)
	    {
	      SKVPN_LOG_SSL("VERIFY FAIL -- verify-x509-name failed");
	    }
	  }
	  ...
	  if (!self->config->tls_remote.empty())
		...
		{
		  SKVPN_LOG_SSL("VERIFY FAIL -- tls-remote match failed");
		  preverify_ok = false;
		}
	    }
	}
    ...
    static int verify_callback_server(int preverify_ok, X509_STORE_CTX *ctx)
    {
      ...
      // log subject
      if (self->config->flags & SSLConst::LOG_VERIFY_STATUS)
	SKVPN_LOG_SSL(cert_status_line(preverify_ok, depth, err,
	...
	{
	  SKVPN_LOG_SSL("SSL state (" << (where & SSL_ST_CONNECT ? "connect" : where & SSL_ST_ACCEPT ? "accept" : "undefined") << "): " << SSL_state_string_long(s));
	}
      else if (where & SSL_CB_ALERT)
	{
	  SKVPN_LOG_SSL("SSL alert (" << (where & SSL_CB_READ ? "read" : "write") << "): " << SSL_alert_type_string_long(ret) << ": " << SSL_alert_desc_string_long(ret));
	}
    }
      ...
      if (enc)
	{
	  ...
	  switch (t->create_session_ticket_key(name, key))
	    ...
	    case TLSSessionTicketBase::TICKET_EXPIRING: // doesn't really make sense for enc==1?
	      ...
	      //SKVPN_LOG("tls_ticket_key_callback: create: no ticket or expiring ticket");
		...
		return -1;
	      ...
	      //SKVPN_LOG("tls_ticket_key_callback: created ticket");
	      return 1;
	    default:
	      //SKVPN_LOG("tls_ticket_key_callback: create: bad ticket");
	      return -1;
	    }
	}
      else
	{
	  ...
	  switch (t->lookup_session_ticket_key(name, key))
	    ...
	    case TLSSessionTicketBase::TICKET_AVAILABLE:
	      if (!tls_ticket_init_cipher_hmac(key, iv, ctx, hctx, enc))
		return -1;
	      //SKVPN_LOG("tls_ticket_key_callback: found ticket");
	      return 1;
	    case TLSSessionTicketBase::TICKET_EXPIRING:
	      if (!tls_ticket_init_cipher_hmac(key, iv, ctx, hctx, enc))
		return -1;
	      //SKVPN_LOG("tls_ticket_key_callback: expiring ticket");
	      return 2;
	    case TLSSessionTicketBase::NO_TICKET:
	      //SKVPN_LOG("tls_ticket_key_callback: lookup: no ticket");
	      return 0;
	    default:
	      //SKVPN_LOG("tls_ticket_key_callback: lookup: bad ticket");
	      return -1;
	    }
	}
    }
	...
	if (!sni_name.empty())
	  {
		...
		try {
		  ...
		  {
		    SKVPN_LOG("SNI HANDLER ERROR: " << e.what());
	...
	{
	  SKVPN_LOG("SNI exception in OpenSSLContext, SNI=" << sni_name << " : " << e.what());
...
#if OPENSSL_VERSION_NUMBER < 0x10100000L && defined(SKVPN_NO_EXTERN)
