...
#include <skvpn/common/msfind.hpp>
...
namespace skvpn {
  ...
  public:
    typedef RCPtr<OpenSSLSessionCache> Ptr;
      ...
      SKVPN_EXCEPTION(openssl_sess_cache_error);
	...
	: key(key_arg),
      ...
      {
	//SKVPN_LOG("OpenSSLSessionCache::Key CONSTRUCT key=" << key);
      }
	...
	if (mi)
	  {
	    ...
	    //SKVPN_LOG("OpenSSLSessionCache::Key::commit ADD=" << ins.second << " key=" << key);
	  }
	else
	  {
	    //SKVPN_LOG("OpenSSLSessionCache::Key::commit CREATE key=" << key);
    ...
    void extract(const std::string& key, FUNC func)
    {
      ...
      if (mi)
	{
	  //SKVPN_LOG("OpenSSLSessionCache::Key::lookup EXISTS key=" << key);
	...
	{
	  //SKVPN_LOG("OpenSSLSessionCache::Key::lookup NOT_FOUND key=" << key);
	}
    }
