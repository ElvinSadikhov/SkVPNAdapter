...
namespace skvpn {
  namespace WS {
	...
	if (out_state == S_DEFERRED && (!outbuf || outbuf->empty()))
	  ...
	  SKVPN_THROW(http_exception, "http_content_out_finish: no deferred state=" << http_out_state_string(out_state) << " outbuf_size=" + (std::to_string(outbuf ? int(outbuf->size()) : -1)) << " halt=" << halt << " ready=" << ready << " async_out=" << async_out << " websock=" << websocket);
      }
		...
		if (ssl_sess)
		  {
		    ...
		    if (actual >= 0)
		      {
#if defined(SKVPN_DEBUG_HTTP)
			BufferAllocated tmp(outbuf->c_data(), actual, 0);
			SKVPN_LOG("OUT: " << buf_to_string(tmp));
#endif
...
#if defined(SKVPN_DEBUG_HTTP)
		    SKVPN_LOG("OUT: " << buf_to_string(buf));
#endif
...
#if defined(SKVPN_DEBUG_HTTP)
	SKVPN_LOG("IN: " << buf_to_string(buf));
#endif
