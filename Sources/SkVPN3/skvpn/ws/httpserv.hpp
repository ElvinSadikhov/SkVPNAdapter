...
#include <skvpn/http/request.hpp>
...
#include <skvpn/server/listenlist.hpp>

#ifdef SKVPN_POLYSOCK_SUPPORTS_ALT_ROUTING
#include <skvpn/kovpn/sock_mark.hpp>
#endif

// include acceptors for different protocols
#include <skvpn/proxy/listener.hpp>
#include <skvpn/acceptor/base.hpp>
#include <skvpn/acceptor/tcp.hpp>
#if defined(SKVPN_PLATFORM_WIN)
#include <skvpn/acceptor/namedpipe.hpp>
#endif
#ifdef ASIO_HAS_LOCAL_SOCKETS
#include <skvpn/acceptor/unix.hpp>
#endif

#ifndef SKVPN_HTTP_SERV_RC
#define SKVPN_HTTP_SERV_RC RC<thread_unsafe_refcount>
#endif
...
namespace skvpn {
  namespace WS {
    namespace Server {
      ...
      SKVPN_EXCEPTION(http_server_exception);
	...
	class Client;
      ...
      private:
	typedef WS::HTTPBase<Client, Config, Status, HTTP::RequestType, ContentInfo, content_len_t, SKVPN_HTTP_SERV_RC> Base;
	...
	public:
	  ...
	  class Initializer
	  {
	    ...
	    Initializer(skvpn_io::io_context& io_context_arg,
			Listener* parent_arg,
	  ...
	  struct Factory : public SKVPN_HTTP_SERV_RC
	  {
	...
	private:
	  ...
	  void stop(const bool remove_self_from_map, const bool shutdown)
	  {
	    ...
	    if (remove_self_from_map)
	      skvpn_io::post(io_context, [self=Ptr(this), parent=Listener::Ptr(parent)]() mutable
			 {
		...
		if (!timeout_coarse.similar(next))
		  {
		    ...
		    timeout_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
                                             {
					       if (!error)
	  ...
	  void timeout_callback(const skvpn_io::error_code& e)
	  {
	    if (halt || e)
	      return;
	...
	Listener(skvpn_io::io_context& io_context_arg,
	...
	virtual void start() override
	{
	  ...
	  for (const auto &listen_item : listen_list)
		...
		case Protocol::TCPv6:
		  {
		    ...
		    SKVPN_LOG("HTTP" << ((ssl_mode == Acceptor::Item::SSLOn) ? "S" : "") << " Listen: " << listen_item.to_string());
...
#ifdef SKVPN_DEBUG_ACCEPT
		    SKVPN_LOG("ACCEPTOR BIND " << a->local_endpoint);
#endif
		...
		case Protocol::NamedPipe:
		  {
		    SKVPN_LOG("HTTP Listen: " << listen_item.to_string());
	...
	void throttle_timer_wait()
	{
	  ...
	  throttle_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
				    {
				      if (self->halt || error)
					return;
	...
	virtual void handle_accept(AsioPolySock::Base::Ptr sock, const skvpn_io::error_code& error) override
	{
	  if (halt)
	      return;
	  ...
	  try {
...
#ifdef SKVPN_DEBUG_ACCEPT
		SKVPN_LOG("ACCEPT from " << sock->remote_endpoint_str());
#endif
	    ...
	    {
	      SKVPN_LOG("exception in handle_accept: " << e.what());
	    }
