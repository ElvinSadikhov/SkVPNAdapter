...
#include <skvpn/common/complog.hpp>
#include <skvpn/time/asiotimersafe.hpp>
#include <skvpn/buffer/buflist.hpp>
#include <skvpn/buffer/bufstr.hpp>
#include <skvpn/buffer/zlib.hpp>
#include <skvpn/random/randapi.hpp>
#include <skvpn/http/urlparse.hpp>
#include <skvpn/http/headredact.hpp>
#include <skvpn/ws/httpcli.hpp>
#include <skvpn/ws/resolver_results.hpp>

#ifndef SKVPN_HTTP_CLISET_RC
#define SKVPN_HTTP_CLISET_RC RC<thread_unsafe_refcount>
#endif
...
namespace skvpn {
  namespace WS {
	...
	void construct(skvpn_io::io_context& io_context,
	...
	std::string format_status() const
	{
	  ...
	  if (status == WS::Client::Status::E_SUCCESS)
	    {
	      ret += skvpn::to_string(reply.status_code);
	      ret += ' ';
      ...
      {
      private:
	// optionally contains an skvpn_io::io_context for
	...
	size_t index = 0;
      ...
      ClientSet(skvpn_io::io_context& io_context_arg)
	...
	if (!io_context)
	  io_context.reset(new skvpn_io::io_context(1));
	ClientSet::Ptr cs;
	try {
	...
	if (sps)
      ...
      static void run_synchronous(Function<void(ClientSet::Ptr)> job,
				  ...
				  RandomAPI* prng=nullptr)
      {
	std::unique_ptr<skvpn_io::io_context> io_context(new skvpn_io::io_context(1));
	ClientSet::Ptr cs;
	try {
	  AsioStopScope scope(*io_context, stop, [&]() {
	      if (cs)
    ...
    private:
      ...
      class Client : public SKVPN_HTTP_CLISET_RC
      {
      public:
	...
	{
	  skvpn_io::post(parent->io_context, [id=client_id, parent=ClientSet::Ptr(parent)]()
		     {
	...
	void reconnect_schedule(const bool error_retry)
	{
	  ...
	  reconnect_timer.async_wait([self=Ptr(this), error_retry](const skvpn_io::error_code& error)
				     {
				       if (!error && !self->halt)
	...
	void http_mutate_resolver_results(HTTPDelegate& hd, skvpn_io::ip::tcp::resolver::results_type& results)
	{
	...
	{
	  skvpn_io::post(parent->io_context, [self=Ptr(this)]()
			   {
	...
	void http_keepalive_close(HTTPDelegate& hd, const int status, const std::string& description)
	{
	  ...
	  //SKVPN_LOG("http_keepalive_close " << WS::Client::Status::error_str(status) << " description=" << description << " http_status=" << std::to_string(hd.reply().status_code) << " http_text=" << hd.reply().status_text);
	}
