...
//   * GET, POST, etc.
//   * any SkVPN SSL module (OpenSSL, MbedTLS)
//   * server CA bundle
//   * client certificate
//   * HTTP basic auth
...
#include <skvpn/client/async_resolve.hpp>
#endif

#include <skvpn/common/platform.hpp>
#include <skvpn/common/base64.hpp>
#include <skvpn/common/olong.hpp>
#include <skvpn/common/arraysize.hpp>
#include <skvpn/common/hostport.hpp>
#include <skvpn/common/base64.hpp>
#include <skvpn/addr/ip.hpp>
#include <skvpn/asio/asiopolysock.hpp>
#include <skvpn/asio/asioresolverres.hpp>
#include <skvpn/common/to_string.hpp>
#include <skvpn/error/error.hpp>
#include <skvpn/buffer/bufstream.hpp>
#include <skvpn/http/reply.hpp>
#include <skvpn/time/asiotimersafe.hpp>
#include <skvpn/time/coarsetime.hpp>
#include <skvpn/transport/tcplink.hpp>
#include <skvpn/transport/client/transbase.hpp>
#include <skvpn/ws/httpcommon.hpp>
#include <skvpn/ws/httpcreds.hpp>
#include <skvpn/ws/websocket.hpp>
...
#include <skvpn/common/periodic_fail.hpp>
#endif
...
namespace skvpn {
  namespace WS {
    namespace Client {
      ...
      SKVPN_EXCEPTION(http_client_exception);
	...
	static std::string error_str(const int status)
	{
	  ...
	  if (status >= 0 && status < N_ERRORS)
	    ...
	    return "E_?/" + skvpn::to_string(status);
	}
	...
	HTTPCore(skvpn_io::io_context& io_context_arg,
	...
	void start_request()
	{
	  ...
	  skvpn_io::post(io_context, [self=Ptr(this)]()
		     {
		       self->handle_request();
		     });
	}
	...
	void start_request_after(const Time::Duration dur)
	{
	  ...
	  req_timer->async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
				{
				  if (!error)
				    self->handle_request();
				});
	...
	virtual void http_mutate_resolver_results(skvpn_io::ip::tcp::resolver::results_type& results)
	{
	}
	...
	bool inject_fault(const char *caller)
	{
	  if (periodic_fail.trigger("httpcli", SIMULATE_HTTPCLI_FAILURES))
	    {
	      SKVPN_LOG("HTTPCLI BOGON on " << host.host_port_str() << " (" << caller << ')');
	...
	void activity(const bool init)
	{
	  ...
	  if (general_timeout_duration.defined())
	      ...
	      if (init || !general_timeout_coarse.similar(next))
		{
		  ...
		  general_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
					   {
					     if (!error)
	...
	void handle_request() // called by Asio
	{
	  ...
	  try {
	    ...
	    if (host.port == "unix")
	      {
		skvpn_io::local::stream_protocol::endpoint ep(host.host_transport());
		AsioPolySock::Unix* s = new AsioPolySock::Unix(io_context, 0);
		socket.reset(s);
		s->socket.async_connect(ep,
					[self=Ptr(this)](const skvpn_io::error_code& error)
					{
		...
		if (!Win::Handle::defined(h))
		  {
		    ...
		    SKVPN_THROW(http_client_exception, "failed to open existing named pipe: " << ht << " : " << err.message());
		  }
		socket.reset(new AsioPolySock::NamedPipe(skvpn_io::windows::stream_handle(io_context, h), 0));
		do_connect(true);
		set_connect_timeout(config->connect_timeout);
		return;
	      }
		...
		resolver.async_resolve(host.host_transport(), host.port,
				       [self=Ptr(this)](const skvpn_io::error_code& error, skvpn_io::ip::tcp::resolver::results_type results)
				       {
	...
	void resolve_callback(const skvpn_io::error_code& error, // called by Asio
			      skvpn_io::ip::tcp::resolver::results_type results)
	{
	  if (halt)
	    return;
	  ...
	  try {
	    ...
	    if (results.empty())
	      SKVPN_THROW_EXCEPTION("no results");
	    ...
	    if (config->debug_level >= 2)
	      SKVPN_LOG("TCP HTTP CONNECT to " << s->remote_endpoint_str() << " res=" << asio_resolver_results_to_string(results));

	    skvpn_io::async_connect(s->socket, std::move(results),
				[self=Ptr(this)](const skvpn_io::error_code& error, const skvpn_io::ip::tcp::endpoint& endpoint)
				{
	...
	void handle_tcp_connect(const skvpn_io::error_code& error, // called by Asio
				const skvpn_io::ip::tcp::endpoint& endpoint)
	{
	  if (halt)
	    return;
...
#ifdef ASIO_HAS_LOCAL_SOCKETS
	void handle_unix_connect(const skvpn_io::error_code& error) // called by Asio
	{
	  if (halt)
	    return;
	...
	void alt_routing_connect(AltRouting::Shim::Ptr shim)
	{
	  ...
	  skvpn_io::ip::tcp::resolver::results_type results =
	    skvpn_io::ip::tcp::resolver::results_type::create(skvpn_io::ip::tcp::endpoint(addr.to_asio(),
											      port),
								host.host,
								"");

	  if (config->debug_level >= 2)
	    SKVPN_LOG("ALT_ROUTING HTTP CONNECT to " << s->remote_endpoint_str() << " res=" << asio_resolver_results_to_string(results));
	  ...
	  if (config->connect_timeout)
	    {
	      connect_timer.expires_after(Time::Duration::seconds(to.connect >= 0
								  ? to.connect
								  : connect_timeout));
	      connect_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
				       {
					 if (!error)
	...
	void bind_local_addr(AsioPolySock::TCP* s)
	{
	  ...
	  if (!host.local_addr.empty())
	    {
#if defined(SKVPN_POLYSOCK_SUPPORTS_BIND) || defined(SKVPN_POLYSOCK_SUPPORTS_ALT_ROUTING)
...
#else
	      throw Exception("httpcli must be built with SKVPN_POLYSOCK_SUPPORTS_BIND or SKVPN_POLYSOCK_SUPPORTS_ALT_ROUTING to support local bind");
#endif
	    }
	}
	...
	void schedule_keepalive_timer()
	{
	  if (config->keepalive_timeout || to.keepalive >= 0)
		...
		keepalive_timer.reset(new AsioTimerSafe(io_context));
	      ...
	      keepalive_timer->async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
	              {
	...
	void general_timeout_handler(const skvpn_io::error_code& e) // called by Asio
	{
	  if (!halt && !e)
	    error_handler(Status::E_GENERAL_TIMEOUT, "General timeout");
	}

	void connect_timeout_handler(const skvpn_io::error_code& e) // called by Asio
	{
	  if (!halt && !e)
	...
	void asio_error_handler(int errcode, const char *func_name, const skvpn_io::error_code& error)
	{
...
#ifndef USE_ASYNC_RESOLVE
	skvpn_io::ip::tcp::resolver resolver;
#endif
	Host host;
	...
	PeriodicFail periodic_fail;
      ...
      {
      public:
	SKVPN_EXCEPTION(http_delegate_error);

	typedef RCPtr<HTTPDelegate> Ptr;

	HTTPDelegate(skvpn_io::io_context& io_context,
