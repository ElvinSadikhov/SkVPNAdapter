...
#ifndef SKVPN_SSL_MSSPARMS_H
#define SKVPN_SSL_MSSPARMS_H
...
namespace skvpn {
  struct MSSParms
  {
      ...
      if (o)
	{
	  ...
	  if (val == nullptr)
	    {
	      if (nothrow)
		{
		  SKVPN_LOG("Missing mssfix value, mssfix functionality disabled");
		  return;
		}
	      else
	  ...
	  if (!status)
	      ...
	      if (nothrow)
		{
		  ...
		  if (*val != "0")
		    {
		      SKVPN_LOG("Invalid mssfix value " << *val << ", mssfix functionality disabled");
		    }
		}
	      else
    ...
    unsigned int mssfix;  // standard SkVPN mssfix parm
