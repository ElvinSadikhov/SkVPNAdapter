...
#ifndef SKVPN_SSL_PEERINFO_H
#define SKVPN_SSL_PEERINFO_H
...
namespace skvpn {
  namespace PeerInfo {
    ...
    SKVPN_EXCEPTION(peer_info_error);
...
#else
	    SKVPN_THROW(peer_info_error, fn << ": JSON library not available");
#endif
	  }
	else
	  parse_csv(src, dest);
	...
	if (!string::is_empty(src))
	  {
	    if (string::is_multiline(src))
	      SKVPN_THROW(peer_info_error, "key/value list must be a single line: " << Unicode::utf8_printable(src, 256));
		...
		if (kv.size() == 2)
		  ...
		  SKVPN_THROW(peer_info_error, "key/value must be in the form K=V, not: " << Unicode::utf8_printable(kvstr, 256));
	      }
	  }
      }
      ...
      {
	if (!src.isObject())
	  SKVPN_THROW(peer_info_error, title << ": top level JSON object must be a dictionary");
	auto m = src.map();
	for (auto &e : m)
	  {
