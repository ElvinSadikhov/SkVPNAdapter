...
#ifndef SKVPN_SSL_PROTOSTACK_H
#define SKVPN_SSL_PROTOSTACK_H
...
#include <skvpn/common/usecount.hpp>
#include <skvpn/buffer/buffer.hpp>
#include <skvpn/time/time.hpp>
#include <skvpn/log/sessionstats.hpp>
#include <skvpn/reliable/relrecv.hpp>
#include <skvpn/reliable/relsend.hpp>
#include <skvpn/reliable/relack.hpp>
...
// but not limited to SkVPN) to run over SSL, where the underlying transport
// layer is unreliable, such as UDP.  The SkVPN protocol implementation in
...
namespace skvpn {
  ...
  public:
    ...
    SKVPN_SIMPLE_EXCEPTION(proto_stack_invalidated);
    SKVPN_SIMPLE_EXCEPTION(unknown_status_from_ssl_layer);
    ...
    void export_key_material(SkVPNStaticKey& key) const
    {
      if (!ssl_->export_keying_material("EXPORTER-SkVPN-datakeys", key.raw_alloc(),
