...
// ProtoContext, the fundamental SkVPN protocol implementation.
// It can be used by SkVPN clients, servers, or unit tests.

#ifndef SKVPN_SSL_PROTO_H
#define SKVPN_SSL_PROTO_H
...
#include <skvpn/buffer/bufcomposed.hpp>
#include <skvpn/ip/ip4.hpp>
#include <skvpn/ip/ip6.hpp>
#include <skvpn/ip/udp.hpp>
#include <skvpn/ip/tcp.hpp>
#include <skvpn/time/time.hpp>
#include <skvpn/time/durhelper.hpp>
#include <skvpn/frame/frame.hpp>
#include <skvpn/random/randapi.hpp>
#include <skvpn/crypto/cryptoalgs.hpp>
#include <skvpn/crypto/cryptodc.hpp>
#include <skvpn/crypto/cipher.hpp>
#include <skvpn/crypto/ovpnhmac.hpp>
#include <skvpn/crypto/tls_crypt.hpp>
#include <skvpn/crypto/tls_crypt_v2.hpp>
#include <skvpn/crypto/packet_id.hpp>
#include <skvpn/crypto/static_key.hpp>
#include <skvpn/crypto/bs64_data_limit.hpp>
#include <skvpn/log/sessionstats.hpp>
#include <skvpn/ssl/protostack.hpp>
#include <skvpn/ssl/psid.hpp>
#include <skvpn/ssl/tlsprf.hpp>
#include <skvpn/ssl/datalimit.hpp>
#include <skvpn/ssl/mssparms.hpp>
#include <skvpn/transport/mssfix.hpp>
#include <skvpn/transport/protocol.hpp>
#include <skvpn/tun/layer.hpp>
#include <skvpn/tun/tunmtu.hpp>
#include <skvpn/compress/compress.hpp>
#include <skvpn/ssl/proto_context_options.hpp>
#include <skvpn/ssl/peerinfo.hpp>
#include <skvpn/ssl/ssllog.hpp>
#include <skvpn/crypto/crypto_aead.hpp>


#if SKVPN_DEBUG_PROTO >= 1
#define SKVPN_LOG_PROTO(x) SKVPN_LOG(x)
#define SKVPN_LOG_STRING_PROTO(x) SKVPN_LOG_STRING(x)
#else
#define SKVPN_LOG_PROTO(x)
#define SKVPN_LOG_STRING_PROTO(x)
#endif

#if SKVPN_DEBUG_PROTO >= 2
#define SKVPN_LOG_PROTO_VERBOSE(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_PROTO_VERBOSE(x)
#endif

/*

ProtoContext -- SkVPN protocol implementation
...
namespace skvpn {
  ...
  protected:
    ...
    enum iv_proto_flag: unsigned int
      {
      // See ssl.h in skvpn2 for detailed documentation of IV_PROTO
  ...
  public:
    SKVPN_EXCEPTION(proto_error);
    SKVPN_EXCEPTION(process_server_push_error);
    SKVPN_EXCEPTION_INHERIT(option_error, proto_option_error);
    ...
    public:
      ...
      // If relay mode is enabled, connect to a special SkVPN
      ...
      SkVPNStaticKey tls_key; // leave this undefined to disable tls_auth/crypt
      ...
      // For compatibility with skvpn2 we send initial options on rekeying,
	  ...
	  if (o)
	      ...
	      if (o->size() >= 2)
		{
		  ...
		  if (meth == CompressContext::NONE)
		    SKVPN_THROW(proto_option_error, "Unknown compressor: '" << meth_name << '\'');
	...
	// data channel
	{
	  ...
	  try {
	    ...
	    {
	      SKVPN_THROW(process_server_push_error, "Problem accepting server-pushed cipher '" << new_cipher << "': " << e.what());
	    }
	  ...
	  try {
	    ...
	    {
	      SKVPN_THROW(process_server_push_error, "Problem accepting server-pushed digest '" << new_digest << "': " << e.what());
	    }
	  ...
	  try {
	      ...
	      if (o)
		{
		  ...
		  if (key_method == "tls-ekm")
		    ...
		    SKVPN_THROW(process_server_push_error, "Problem accepting key-derivation method '" << key_method << "'");
		}
	      else
		dc.set_key_derivation(CryptoAlgs::KeyDerivation::SKVPN_PRF);
	    }
	  catch (const std::exception& e)
	    {
	      SKVPN_THROW(process_server_push_error, "Problem accepting key-derivation method '" << key_method << "': " << e.what());
	    }
	}
	// compression
	std::string new_comp;
	try {
		  ...
		  if (pco.is_comp())
		  ...
		  else
		    {
		      ...
		      if (!comp_ctx.is_any_stub(meth))
		        {
			  SKVPN_LOG("Server has pushed compressor "
				      << comp_ctx.str()
	  ...
	  {
	    SKVPN_THROW(process_server_push_error, "Problem accepting server-pushed compressor '" << new_comp << "': " << e.what());
	  }
	...
	try {
	  ...
	  {
	    SKVPN_THROW(process_server_push_error, "Problem accepting server-pushed peer-id: " << e.what());
	  }
	...
	try {
	  ...
	  {
	    SKVPN_THROW(process_server_push_error, "Problem accepting server-pushed parameter: " << e.what());
	  }

	// show negotiated options
	SKVPN_LOG_STRING_PROTO(show_options());
      }
	...
	/*
	 * SkVPN3 allows to be pushed any cipher that it supports as it
	...
	SKVPN_LOG_PROTO("Peer Info:" << std::endl << ret);
	return ret;
      }
  ...
  protected:
    ...
    public:
      ...
      SKVPN_SIMPLE_EXCEPTION(tls_crypt_unwrap_wkc_error);
	...
	return current_event != KEV_NONE;
      ...
      // our Key ID in the SkVPN protocol
	...
	send_data_channel_message(proto_context_private::keepalive_message,
      ...
      {
#ifndef SKVPN_DISABLE_EXPLICIT_EXIT // explicit exit should always be enabled in production
	...
	try {
	  ...
	  switch (proto.tls_wrap_mode)
	  {
	      ...
	      if (opcode_extract(recv[0]) == CONTROL_HARD_RESET_CLIENT_V3)
		{
		  ...
		  SKVPN_LOG_PROTO_VERBOSE("SKIPPING VALIDATION OF HARD_RESET_V3");
		  return true;
		}
	      /* no break */
	    case TLS_CRYPT:
	  ...
	  {
	    SKVPN_LOG_PROTO_VERBOSE("validate() exception: " << e.what());
	  }
	return false;
      }
	...
	tlsprf->erase();
	SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " KEY " << CryptoAlgs::name(proto.config->dc.key_derivation())
	...
	dck.swap(data_channel_key);
      ...
      // Initialize the components of the SkVPN data channel protocol
	...
	const unsigned int key_dir = proto.is_server() ? SkVPNStaticKey::INVERSE : SkVPNStaticKey::NORMAL;
	const SkVPNStaticKey& key = data_channel_key->key;

	// special data limits for 64-bit block-size ciphers (CVE-2016-6329)
	if (is_bs64_cipher(c.dc.cipher()))
	  {
	    DataLimit::Parameters dp;
	    dp.encrypt_red_limit = SKVPN_BS64_DATA_LIMIT;
	    dp.decrypt_red_limit = SKVPN_BS64_DATA_LIMIT;
	    SKVPN_LOG_PROTO("Per-Key Data Limit: " << dp.encrypt_red_limit << '/' << dp.decrypt_red_limit);
	...
	if (crypto_flags & CryptoDCInstance::CIPHER_DEFINED)
	  crypto->init_cipher(key.slice(SkVPNStaticKey::CIPHER | SkVPNStaticKey::ENCRYPT | key_dir),
			      key.slice(SkVPNStaticKey::CIPHER | SkVPNStaticKey::DECRYPT | key_dir));

	if (crypto_flags & CryptoDCInstance::HMAC_DEFINED)
	  crypto->init_hmac(key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::ENCRYPT | key_dir),
			    key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::DECRYPT | key_dir));
	...
	if (c.mss_parms.mssfix != 0)
	  {
	    SKVPN_LOG_PROTO("MTU mssfix=" << c.mss_parms.mssfix <<
	...
	remote_peer_id = proto.config->remote_peer_id;
      ...
      {
	SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " KeyContext[" << key_id_ << "] " << state_string(state) << " -> " << state_string(newstate));
	state = newstate;
      }

      void set_event(const EventType current)
      {
	SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " KeyContext[" << key_id_ << "] " << event_type_string(current));
	current_event = current;
      }

      void set_event(const EventType current, const EventType next, const Time& next_time)
      {
	SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " KeyContext[" << key_id_ << "] " << event_type_string(current) << " -> " << event_type_string(next) << '(' << seconds_until(next_time) << ')');
	current_event = current;
	next_event = next;
	...
	if (!key_limit_renegotiation_fired && state >= ACTIVE && !invalidated())
	  {
	    SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " SCHEDULE KEY LIMIT RENEGOTIATION");
	...
	if (state > DataLimit::None)
      ...
      {
	SKVPN_LOG_PROTO_VERBOSE(proto.debug_prefix() << " DATA LIMIT " << DataLimit::mode_str(mode) << ' ' << DataLimit::state_str(state) << " key_id=" << key_id_);
	...
	if (!proto.is_server())
	  {
	    SKVPN_LOG_PROTO("Tunnel Options:" << options);
	...
	if (proto.config->debug_level >= 1)
	  SKVPN_LOG_SSL("SSL Handshake: " << Base::ssl_handshake_details());
	...
	if (plaintext.size() < SkVPNStaticKey::KEY_SIZE)
	  {
	    proto.stats->error(Error::DECRYPT_ERROR);
	    if (proto.is_tcp())
	      invalidate(Error::DECRYPT_ERROR);
	...
	SkVPNStaticKey client_key;
	plaintext.read(client_key.raw_alloc(), SkVPNStaticKey::KEY_SIZE);
  ...
  public:
    ...
    {
    public:
      SKVPN_SIMPLE_EXCEPTION(tls_auth_pre_validate);
	...
	if (c.key_direction >= 0)
	  {
	    ...
	    const unsigned int key_dir = c.key_direction ? SkVPNStaticKey::INVERSE : SkVPNStaticKey::NORMAL;
	    ta_hmac_recv->init(c.tls_key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::DECRYPT | key_dir));
	  }
	else
	  {
	    // key-direction bidirectional mode
	    ta_hmac_recv->init(c.tls_key.slice(SkVPNStaticKey::HMAC));
	  }
      }
    ...
    {
    public:
      SKVPN_SIMPLE_EXCEPTION(tls_crypt_pre_validate);
	...
	const unsigned int key_dir = server ? SkVPNStaticKey::NORMAL : SkVPNStaticKey::INVERSE;
	tls_crypt_recv->init(c.tls_key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::DECRYPT | key_dir),
			     c.tls_key.slice(SkVPNStaticKey::CIPHER | SkVPNStaticKey::DECRYPT | key_dir));
    ...
    {
        public:
          SKVPN_SIMPLE_EXCEPTION(tls_crypt_v2_pre_validate);
    ...
    SKVPN_SIMPLE_EXCEPTION(select_key_context_error);
	...
	return primary->get_tls_warnings();
      ...
      SKVPN_LOG("TLS: primary key context uninitialized. Can't retrieve TLS warnings");
      return 0;
    }
    ...
    void reset_tls_crypt(const Config& c, const SkVPNStaticKey& key)
    {
      ...
      unsigned int key_dir = is_server() ?
			     SkVPNStaticKey::NORMAL :
			     SkVPNStaticKey::INVERSE;

      tls_crypt_send->init(key.slice(SkVPNStaticKey::HMAC |
				     SkVPNStaticKey::ENCRYPT | key_dir),
			   key.slice(SkVPNStaticKey::CIPHER |
				     SkVPNStaticKey::ENCRYPT | key_dir));
      tls_crypt_recv->init(key.slice(SkVPNStaticKey::HMAC |
				     SkVPNStaticKey::DECRYPT | key_dir),
			   key.slice(SkVPNStaticKey::CIPHER |
				     SkVPNStaticKey::DECRYPT | key_dir));
    }
    ...
    void reset_tls_crypt_server(const Config& c)
    {
      ...
      tls_crypt_server->init(c.tls_key.slice(SkVPNStaticKey::HMAC),
			     c.tls_key.slice(SkVPNStaticKey::CIPHER));
      ...
      switch (tls_wrap_mode)
	{
	  ...
	  case TLS_AUTH:
	    ...
	    if (c.key_direction >= 0)
	      {
	        // key-direction is 0 or 1
	        key_dir = c.key_direction ? SkVPNStaticKey::INVERSE : SkVPNStaticKey::NORMAL;
	        ta_hmac_send->init(c.tls_key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::ENCRYPT | key_dir));
	        ta_hmac_recv->init(c.tls_key.slice(SkVPNStaticKey::HMAC | SkVPNStaticKey::DECRYPT | key_dir));
	      }
	    else
	      {
	        // key-direction bidirectional mode
	        ta_hmac_send->init(c.tls_key.slice(SkVPNStaticKey::HMAC));
	        ta_hmac_recv->init(c.tls_key.slice(SkVPNStaticKey::HMAC));
	      }
      ...
      SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " New KeyContext PRIMARY id=" << primary->key_id());
    ...
    {
      //SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " DATA ENCRYPT size=" << in_out.size());
      if (!primary)
    ...
    bool data_decrypt(const PacketType& type, BufferAllocated& in_out)
    {
      ...
      //SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " DATA DECRYPT key_id=" << select_key_context(type, false).key_id() << " size=" << in_out.size());
  ...
  private:
    ...
    // than the immediate rollover practiced by SkVPN 2.x.
    KeyContext& select_control_send_context()
    {
      SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " CONTROL SEND");
      if (!primary)
    ...
    void new_secondary_key(const bool initiator)
    {
      ...
      SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " New KeyContext SECONDARY id=" << secondary->key_id() << (initiator ? " local-triggered" : " remote-triggered"));
    }
	...
	secondary->prepare_expire();
      SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " PRIMARY_SECONDARY_SWAP");
    }
      ...
      if (ev != KeyContext::KEV_NONE)
	{
	  ...
	  switch (ev)
	    ...
	    case KeyContext::KEV_ACTIVE:
	      SKVPN_LOG_PROTO_VERBOSE(debug_prefix() << " SESSION_ACTIVE");
    ...
    {
      std::string ret = skvpn::to_string(now_->raw());
      ret += is_server() ? " SERVER[" : " CLIENT[";
      if (primary)
	ret += skvpn::to_string(primary->key_id());
      if (secondary)
	{
	  ret += '/';
	  ret += skvpn::to_string(secondary->key_id());
	}
      ret += ']';
      return ret;
    }
