...
// SkVPN protocol implementation for client-instance object on server

#ifndef SKVPN_SERVER_SERVPROTO_H
#define SKVPN_SERVER_SERVPROTO_H
...
#include <skvpn/transport/server/transbase.hpp>
#include <skvpn/tun/server/tunbase.hpp>
#include <skvpn/server/manage.hpp>

#ifdef SKVPN_DEBUG_SERVPROTO
#define SKVPN_LOG_SERVPROTO(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_SERVPROTO(x)
#endif
...
namespace skvpn {
  ...
  public:
    ...
    public:
      ...
      Factory(skvpn_io::io_context& io_context_arg,
	      const Base::Config& c)
    ...
    class Session : Base,                  // SkVPN protocol implementation
	...
	if (!halt)
      ...
      // called with SkVPN-encapsulated packets from transport layer
	...
	if (!Base::primary_defined())
	  return false;
	try {
	  SKVPN_LOG_SERVPROTO("Transport RECV[" << buf.size() << "] " << client_endpoint_render() << ' ' << Base::dump_packet(buf));
	  ...
	  if (pt.is_data())
	      ...
	      if (buf.size())
		{
		  ...
		  if (true) // fixme: was tun
		    {
		      SKVPN_LOG_SERVPROTO("TUN SEND[" << buf.size() << ']');
		      // fixme -- code me
		    }
		}
	...
	return !halt && TransportLink::send;
      ...
      {
	SKVPN_LOG_SERVPROTO("Transport SEND[" << net_buf.size() << "] " << client_endpoint_render() << ' ' << Base::dump_packet(net_buf));
	if (TransportLink::send)
	  {
	...
	if (msg == "PUSH_REQUEST")
	  ...
	  {
	    SKVPN_LOG("Unrecognized client request: " << msg);
	  }
      }
	...
	SKVPN_LOG("Disconnect: " << ts << ' ' << reason);
