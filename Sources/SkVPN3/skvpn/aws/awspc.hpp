...
#include <skvpn/aws/awscreds.hpp>
#include <skvpn/ws/httpcliset.hpp>
#include <skvpn/common/jsonhelper.hpp>
#include <skvpn/common/hexstr.hpp>
#include <skvpn/common/enumdir.hpp>
#include <skvpn/common/file.hpp>
#include <skvpn/random/devurand.hpp>
#include <skvpn/frame/frame_init.hpp>
#include <skvpn/openssl/sign/verify.hpp>
#include <skvpn/openssl/sign/pkcs7verify.hpp>
...
namespace skvpn {
  namespace AWS {
    ...
    public:
      ...
      SKVPN_EXCEPTION(awspc_query_error);
	...
	// transaction #1
	{
	  ...
	  root["host"] = Json::Value(skvpn_io::ip::host_name());
	...
	try {
	  ...
	  // verify reply signature
	  {
	    ...
	    if (debug_level >= 3)
	      SKVPN_LOG("LINE TO SIGN: " << line_to_sign);
	...
	return std::string(
      ...
      // The SkVPN Tech. lambda web cert
	...
	return std::string(
      ...
      // The SkVPN Tech. lambda response signing cert
