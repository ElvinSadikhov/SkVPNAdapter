...
#include <skvpn/common/xmlhelper.hpp>
#include <skvpn/aws/awshttp.hpp>
#include <skvpn/aws/awspc.hpp>
#include <skvpn/aws/awsrest.hpp>
...
namespace skvpn {
  namespace AWS {
    ...
    {
    public:
      SKVPN_EXCEPTION(aws_route_error);
	...
	Info(Context& ctx)
	{
	  ...
	  // first request -- get the AWS network interface
	  {
		...
		SKVPN_THROW(aws_route_error, "DescribeNetworkInterfaces: " << t.format_status(*ctx.ts));
	      ...
	      const tinyxml2::XMLElement* item = Xml::find(&doc,
							   ...
							   "item");
	      if (!item)
		SKVPN_THROW(aws_route_error, "DescribeNetworkInterfaces: cannot locate <item> tag in returned XML:\n" << reply);
		...
		SKVPN_THROW(aws_route_error, "DescribeNetworkInterfaces: cannot locate one of networkInterfaceId, vpcId, or subnetId in returned XML:\n" << reply);
	    }
	  }
	  ...
	  // second request -- get the VPC routing table
	  {
	    ...
	    // process reply
	    {
	      ...
	      // check the reply status
	      if (!t.http_status_success())
		SKVPN_THROW(aws_route_error, "DescribeRouteTables: " << t.format_status(*ctx.ts) << '\n' << reply);
		...
		SKVPN_THROW(aws_route_error, "DescribeRouteTables: cannot locate routeTableId in returned XML:\n" << reply);
	    }
	  }
	}
	...
	// process reply
	{
	  ...
	  // check the reply status
	  if (!t.http_status_success())
	    SKVPN_THROW(aws_route_error, "DescribeNetworkInterfaceAttribute: " << t.format_status(*ctx.ts) << '\n' << reply);
	...
	// process reply
	{
	  ...
	  // check the reply status
	  if (!t.http_status_success())
	    SKVPN_THROW(aws_route_error, "ModifyNetworkInterfaceAttribute: " << t.format_status(*ctx.ts) << '\n' << reply);
	  ...
	  const std::string retval = Xml::find_text(&doc,
						    ...
						    "return");
	  if (retval != "true")
	    SKVPN_THROW(aws_route_error, "ModifyNetworkInterfaceAttribute: returned failure status: " << '\n' << reply);

	  SKVPN_LOG("AWS EC2 ModifyNetworkInterfaceAttribute " << network_interface_id << " SourceDestCheck.Value=" << sdc);
	}
      }
	...
	// process reply
	{
	  ...
	  // check the reply status
	  if (!t.http_status_success())
	    SKVPN_THROW(aws_route_error, "DeleteRoute: " << t.format_status(*ctx.ts) << '\n' << reply);
	  ...
	  const std::string retval = Xml::find_text(&doc,
						    ...
						    "return");
	  if (retval != "true")
	    SKVPN_THROW(aws_route_error, "DeleteRoute: returned failure status: " << '\n' << reply);

	  SKVPN_LOG("AWS EC2 DeleteRoute " << cidr << " -> table " << route_table_id);
	}
      }
	...
	// process reply
	{
	  ...
	  if (!t.comm_status_success())
	    SKVPN_THROW(aws_route_error, "ReplaceRoute: " << t.format_status(*ctx.ts) << '\n' << reply);
	  ...
	  if (t.request_status_success())
	    {
	      ...
	      if (retval == "true")
		{
		  SKVPN_LOG("AWS EC2 ReplaceRoute " << route << " -> table " << route_table_id);
		  return;
		}
	    }
	}
	...
	// process reply
	{
	  ...
	  // check the reply status
	  if (!t.http_status_success())
	    SKVPN_THROW(aws_route_error, "CreateRoute: " << t.format_status(*ctx.ts) << '\n' << reply);
	  ...
	  const std::string retval = Xml::find_text(&doc,
						    ...
						    "return");
	  if (retval != "true")
	    SKVPN_THROW(aws_route_error, "CreateRoute: returned failure status: " << '\n' << reply);

	  SKVPN_LOG("AWS EC2 CreateRoute " << route << " -> table " << route_table_id);
	}
      }
