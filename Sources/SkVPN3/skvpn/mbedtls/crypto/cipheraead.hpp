...
namespace skvpn {
  namespace MbedTLSCrypto {
    ...
    public:
      SKVPN_EXCEPTION(mbedtls_aead_error);
	...
	if (cid == MBEDTLS_CIPHER_NONE)
		SKVPN_THROW(mbedtls_aead_error, CryptoAlgs::name(alg) << ": not usable");
	...
	if (unlikely(status))
	  SKVPN_THROW(mbedtls_aead_error, "mbedtls_gcm_crypt_and_tag failed with status=" << status);
      }
