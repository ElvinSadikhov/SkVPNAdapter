...
#ifndef SKVPN_MBEDTLS_SSL_SSLCTX_H
#define SKVPN_MBEDTLS_SSL_SSLCTX_H
...
#include <skvpn/common/binprefix.hpp>
#include <skvpn/common/string.hpp>
#include <skvpn/frame/memq_stream.hpp>
#include <skvpn/pki/cclist.hpp>
#include <skvpn/pki/pkcs1.hpp>
...
#include <skvpn/mbedtls/pki/x509cert.hpp>
#include <skvpn/mbedtls/pki/x509certinfo.hpp>
#include <skvpn/mbedtls/pki/x509crl.hpp>
#include <skvpn/mbedtls/pki/dh.hpp>
#include <skvpn/mbedtls/pki/pkctx.hpp>
#include <skvpn/mbedtls/util/error.hpp>
...
#         endif
	...
	try {
	  ...
	  /* SkVPN 2.x disables cbc_record_splitting by default, therefore
	...
	while(std::getline(cipher_list_ss, ciphersuite, ':'))
	  {
	    ...
	    if (pair && pair->iana_name != ciphersuite)
	      {
		SKVPN_LOG_SSL("mbed TLS -- Deprecated cipher suite name '"
		...
		i++;
	      ...
	      {
	        /* SkVPN 2.x ignores silently ignores unknown cipher suites with
	         * mbed TLS. We warn about them in SkVPN 3.x */
		SKVPN_LOG_SSL("mbed TLS -- warning ignoring unknown cipher suite '"
	...
	while(std::getline(groups_ss, group, ':'))
	  {
		...
		i++;
	      ...
	      {
		SKVPN_LOG_SSL("mbed TLS -- warning ignoring unknown group '"
				  << group << "' in tls-groups");
	      }
	  }
	...
	if (level <= self->config->ssl_debug_level)
	  SKVPN_LOG_NTNL("mbed TLS[" << filename << ":" << linenum << " "<< level << "]: " << text);
      }
  ...
  protected:
	...
	SKVPN_LOG_SSL(status_string(cert, depth, flags));
	...
	*flags |= MBEDTLS_X509_BADCERT_OTHER;
      ...
      if (depth == 1) // issuer cert
	{
	  ...
	  if (ssl->authcert)
		...
		{
		  SKVPN_LOG_SSL("VERIFY FAIL -- SHA1 calculation failed.");
		  fail = true;
		}
#endif
	    }
  ...
  private:
    ...
    {
      SKVPN_LOG_SSL("MbedTLSContext::epki_decrypt is unimplemented, mode=" << mode
	...
	if (mode == MBEDTLS_RSA_PRIVATE)
	  {
	    ...
	    case MBEDTLS_MD_SHA512:
	      ...
	      break;
	    default:
	      SKVPN_LOG_SSL("MbedTLSContext::epki_sign unrecognized hash_id, mode=" << mode
	  ...
	  {
	    SKVPN_LOG_SSL("MbedTLSContext::epki_sign unrecognized parameters, mode=" << mode
			    << " md_alg=" << md_alg << " hashlen=" << hashlen);
	    return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
	  }
      }
	...
	{
	  SKVPN_LOG("MbedTLSContext::epki_sign exception: " << e.what());
	  return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
	}
    }
