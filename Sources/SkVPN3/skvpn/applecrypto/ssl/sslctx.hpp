...
#ifndef SKVPN_APPLECRYPTO_SSL_SSLCTX_H
#define SKVPN_APPLECRYPTO_SSL_SSLCTX_H
...
#include <skvpn/apple/cf/cfsec.hpp>
...
namespace skvpn {
  ...
  public:
	...
	if (!identity())
	  SKVPN_THROW(ssl_context_error, "AppleSSLContext: identity '" << subject_match << "' undefined");
      }
    ...
    private:
      ...
      {
	SKVPN_LOG("AppleSSL: " << funcname << " not implemented");
      }
	...
	return "[AppleSSL not implemented]";
      ...
      {
	SKVPN_THROW(ssl_context_error, "AppleSSL::SSL: auth_cert() not implemented");
      }
	...
	try {
	  ...
	  if (ctx.mode().is_server())
	    ...
	    SKVPN_THROW(ssl_context_error, "AppleSSLContext::SSL: unknown client/server mode");
	  if (ssl == nullptr)
    ...
    {
      SKVPN_THROW(ssl_context_error, "AppleSSLContext: ssl session with CommonName and/or SubjectAltName verification not implemented");
    }
  ...
  private:
    ...
    {
      if (!config_->identity())
	SKVPN_THROW(ssl_context_error, "AppleSSLContext: identity undefined");
    }
...
#endif // SKVPN_APPLECRYPTO_SSL_SSLCTX_H
