...
// so that it can be used as part of the crypto layer of the SkVPN core.

#ifndef SKVPN_APPLECRYPTO_CRYPTO_DIGEST_H
#define SKVPN_APPLECRYPTO_CRYPTO_DIGEST_H
...
#include <skvpn/apple/cf/error.hpp>

#define SKVPN_DIGEST_CONTEXT(TYPE) CC_##TYPE##_CTX TYPE##_ctx

#define SKVPN_DIGEST_ALG_CLASS(TYPE) \
...
#define SKVPN_DIGEST_ALG_DECLARE(TYPE) const DigestAlgorithm##TYPE alg_##TYPE;

#define SKVPN_DIGEST_INFO_DECLARE(TYPE) const DigestInfo info_##TYPE(CryptoAlgs::TYPE, &alg_##TYPE, kCCHmacAlg##TYPE)

#define SKVPN_DIGEST_INFO_DECLARE_NO_HMAC(TYPE) const DigestInfo info_##TYPE(CryptoAlgs::TYPE, &alg_##TYPE, DigestInfo::NO_HMAC_ALG)
...
namespace skvpn {
  namespace AppleCrypto {
    ...
    struct DigestCTX {
      union {
	SKVPN_DIGEST_CONTEXT(MD4);
	SKVPN_DIGEST_CONTEXT(MD5);
	SKVPN_DIGEST_CONTEXT(SHA1);
	SKVPN_DIGEST_CONTEXT(SHA224);
	SKVPN_DIGEST_CONTEXT(SHA256);
	SKVPN_DIGEST_CONTEXT(SHA384);
	SKVPN_DIGEST_CONTEXT(SHA512);
      } u;
    };
    ...
    SKVPN_DIGEST_ALG_CLASS(MD4);
    SKVPN_DIGEST_ALG_CLASS(MD5);
    SKVPN_DIGEST_ALG_CLASS(SHA1);
    SKVPN_DIGEST_ALG_CLASS(SHA224);
    SKVPN_DIGEST_ALG_CLASS(SHA256);
    SKVPN_DIGEST_ALG_CLASS(SHA384);
    SKVPN_DIGEST_ALG_CLASS(SHA512);
    ...
    SKVPN_DIGEST_ALG_DECLARE(MD4);
    SKVPN_DIGEST_ALG_DECLARE(MD5);
    SKVPN_DIGEST_ALG_DECLARE(SHA1);
    SKVPN_DIGEST_ALG_DECLARE(SHA224);
    SKVPN_DIGEST_ALG_DECLARE(SHA256);
    SKVPN_DIGEST_ALG_DECLARE(SHA384);
    SKVPN_DIGEST_ALG_DECLARE(SHA512);

    // instantiate individual digest info class instances (each is a DigestInfo),
    // naming convention is info_TYPE
    SKVPN_DIGEST_INFO_DECLARE_NO_HMAC(MD4);
    SKVPN_DIGEST_INFO_DECLARE(MD5);
    SKVPN_DIGEST_INFO_DECLARE(SHA1);
    SKVPN_DIGEST_INFO_DECLARE(SHA224);
    SKVPN_DIGEST_INFO_DECLARE(SHA256);
    SKVPN_DIGEST_INFO_DECLARE(SHA384);
    SKVPN_DIGEST_INFO_DECLARE(SHA512);
    ...
    public:
      ...
      SKVPN_SIMPLE_EXCEPTION(apple_digest_uninitialized);
      SKVPN_SIMPLE_EXCEPTION(apple_digest_final_overflow);
      SKVPN_EXCEPTION(apple_digest_error);
	...
	switch (alg)
	  ...
	  case CryptoAlgs::SHA512:
	    return &info_SHA512;
	  default:
	    SKVPN_THROW(apple_digest_error, CryptoAlgs::name(alg) << ": not usable");
	  }
      }
...
#undef SKVPN_DIGEST_CONTEXT
#undef SKVPN_DIGEST_ALG_CLASS
#undef SKVPN_DIGEST_ALG_DECLARE
#undef SKVPN_DIGEST_INFO_DECLARE
