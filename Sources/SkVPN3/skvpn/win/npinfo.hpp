...
#include <skvpn/win/secattr.hpp>

namespace skvpn {
  namespace Win {
    struct NamedPipeImpersonate
    {
      SKVPN_EXCEPTION(named_pipe_impersonate);
	...
	if (!::ImpersonateNamedPipeClient(pipe))
	  {
	    ...
	    SKVPN_THROW(named_pipe_impersonate, "ImpersonateNamedPipeClient failed: " << err.message());
	  }
      }
	...
	if (!::RevertToSelf())
	  {
	    SKVPN_LOG("NamedPipeImpersonate: RevertToSelf failed, must abort");
	    std::abort();
	  }
      }
    };
	...
	if (!proc.defined())
	  {
	    const Win::LastError err;
	    SKVPN_THROW(npinfo_error, "OpenProcess failed: " << err.message());
	  }
	return proc;
      }
	...
	if (!::GetSecurityDescriptorDacl(sa.sa.lpSecurityDescriptor,
	  ...
	  {
	    const Win::LastError err;
	    SKVPN_THROW(npinfo_error, "allow_client_query: GetSecurityDescriptorDacl failed: " << err.message());
	  }
	if (!bDaclPresent)
	  SKVPN_THROW(npinfo_error, "allow_client_query: missing DACL");
	...
	if (ssi_status != ERROR_SUCCESS)
	  {
	    ...
	    SKVPN_THROW(npinfo_error, "allow_client_query: SetSecurityInfo failed: " << err.message());
	  }
      }
	...
	if (client)
	  {
	    if (!::GetNamedPipeClientProcessId(np_handle, &pid))
	      {
		const Win::LastError err;
		SKVPN_THROW(npinfo_error, "GetNamedPipeClientProcessId failed: " << err.message());
	      }
	  }
	else
	  {
	    if (!::GetNamedPipeServerProcessId(np_handle, &pid))
	      {
		const Win::LastError err;
		SKVPN_THROW(npinfo_error, "GetNamedPipeServerProcessId failed: " << err.message());
	      }
	  }
	return pid;
      }
	...
	if (!::QueryFullProcessImageNameW(proc, 0, exe, &exe_size))
	  {
	    const Win::LastError err;
	    SKVPN_THROW(npinfo_error, "QueryFullProcessImageNameW failed: " << err.message());
	  }
