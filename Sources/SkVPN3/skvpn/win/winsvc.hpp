...
namespace skvpn {
  namespace Win {
	...
	if (!scmgr.defined())
	  {
	    ...
	    SKVPN_THROW(winsvc_error, "OpenSCManagerW failed: " << err.message());
	  }
	...
	if (!svc.defined())
	  {
	    const Win::LastError err;
	    SKVPN_THROW(winsvc_error, "CreateServiceW failed: " << err.message());
	  }
	...
	if (!svc.defined())
	  {
	    const Win::LastError err;
	    SKVPN_THROW(winsvc_error, "OpenServiceW failed: " << err.message());
	  }
	...
	if (!::DeleteService(svc()))
	  {
	    const Win::LastError err;
	    SKVPN_THROW(winsvc_error, "DeleteService failed: " << err.message());
	  }
      }
	...
	if (!::StartServiceCtrlDispatcherW(dispatch_table))
	  {
	    const Win::LastError err;
	    SKVPN_THROW(winsvc_error, "StartServiceCtrlDispatcherW failed: " << err.message());
	  }
      }
	...
	try {
	  ...
	  if (!status_handle)
	    {
	      const Win::LastError err;
	      SKVPN_THROW(winsvc_error, "RegisterServiceCtrlHandlerW failed: " << err.message());
	    }
	  ...
	  {
	    SKVPN_LOG("service exception: " << e.what());
	    report_service_status(SERVICE_STOPPED, NO_ERROR, 0);
	  }
      }
	...
	switch (dwCtrl)
	  ...
	  case SERVICE_CONTROL_STOP:
	    ...
	    try {
	      ...
	      {
		SKVPN_LOG("service stop exception: " << e.what());
	      }
