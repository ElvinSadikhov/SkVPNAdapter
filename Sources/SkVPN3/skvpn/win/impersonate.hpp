...
namespace skvpn {
  namespace Win {
	...
	if (as_local_system)
	  {
	    if (local_system_)
	      SKVPN_LOG("ImpersonateAsSystem: running under SYSTEM account, no need to impersonate");
	    else
	...
	else
	  {
	    if (local_system_)
	      ...
	      SKVPN_LOG("ImpersonateAsUser: running under user account, no need to impersonate");
	  }
      }
	...
	if (impersonated)
	  {
		...
		SKVPN_LOG("Impersonate: RevertToSelf() failed: " << err.message());
	      }
	  }
      }
	...
	if (sessId == 0xFFFFFFFF)
	  {
	    const Win::LastError err;
	    SKVPN_LOG("ImpersonateAsUser: WTSGetActiveConsoleSessionId() failed: " << err.message());
	    return;
	  }
	...
	if (!WTSQueryUserToken(sessId, &hToken))
	  {
	    const Win::LastError err;
	    SKVPN_LOG("ImpersonateAsUser: WTSQueryUserToken() failed: " << err.message());
	    return;
	  }
	...
	if (!ImpersonateLoggedOnUser(hToken))
	  {
	    ...
	    const Win::LastError err;
	    SKVPN_LOG("ImpersonateAsUser: ImpersonateLoggedOnUser() failed: " << err.message());
	    return;
	  }
	...
	SKVPN_LOG("ImpersonateAsUser: impersonated as " << uname);
      }
