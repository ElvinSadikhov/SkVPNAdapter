...
#ifndef SKVPN_PKI_CCLIST_H
#define SKVPN_PKI_CCLIST_H
...
namespace skvpn {
  ...
  {
  public:
    SKVPN_EXCEPTION(parse_cert_crl_error);
      ...
      while (std::getline(in, line))
	{
	  ...
	  if (state == S_OUTSIDE)
		...
		{
		  if (!cert_list)
		    SKVPN_THROW(parse_cert_crl_error, title << ":" << line_num << " : not expecting a CERT");
		  state = S_IN_CERT;
		}
	      else if (line == crl_start)
		{
		  if (!crl_list)
		    SKVPN_THROW(parse_cert_crl_error, title << ":" << line_num << " : not expecting a CRL");
		  state = S_IN_CRL;
		}
	    }
	  if (state != S_OUTSIDE)
	  ...
	  if (state == S_IN_CERT && line == cert_end)
		...
		{
		  SKVPN_THROW(parse_cert_crl_error, title << ":" << line_num << " : error parsing CERT: " << e.what());
		}
	      state = S_OUTSIDE;
	      item = "";
	    }
	  if (state == S_IN_CRL && line == crl_end)
		...
		{
		  SKVPN_THROW(parse_cert_crl_error, title << ":" << line_num << " : error parsing CRL: " << e.what());
		}
	      state = S_OUTSIDE;
	      item = "";
	    }
      ...
      if (state != S_OUTSIDE)
	SKVPN_THROW(parse_cert_crl_error, title << " : CERT/CRL content ended unexpectedly without END marker");
    }
    ...
    static void from_file(const std::string& filename, CertList* cert_list, CRLList* crl_list = nullptr)
    {
      ...
      if (!ifs)
	SKVPN_THROW(open_file_error, "cannot open CERT/CRL file " << filename);
      from_istream(ifs, filename, cert_list, crl_list);
      if (ifs.bad())
	SKVPN_THROW(open_file_error, "cannot read CERT/CRL file " << filename);
    }
