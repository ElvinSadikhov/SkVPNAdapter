...
#include <skvpn/common/stringize.hpp>
// VERSION version can be passed on build command line
#ifdef VERSION
#define HTTP_SERVER_VERSION SKVPN_STRINGIZE(VERSION)
#else
...
#define OVPNAGENT_NAME_STRING SKVPN_STRINGIZE(OVPNAGENT_NAME)
#else
#define OVPNAGENT_NAME_STRING "ovpnagent"
#endif

#include <skvpn/log/logbase.hpp>
...
#include <skvpn/common/path.hpp>
#include <skvpn/common/file.hpp>
#include <skvpn/common/splitlines.hpp>
#include <skvpn/common/wstring.hpp>
#include <skvpn/log/logbasesimple.hpp>
#include <skvpn/buffer/buflist.hpp>
#include <skvpn/buffer/bufhex.hpp>
#include <skvpn/init/initprocess.hpp>
#include <skvpn/ssl/sslchoose.hpp>
#include <skvpn/ws/httpserv.hpp>
#include <skvpn/win/winerr.hpp>
#include <skvpn/client/win/agentconfig.hpp>
#include <skvpn/win/scoped_handle.hpp>
#include <skvpn/win/winsvc.hpp>
#include <skvpn/win/logfile.hpp>
#include <skvpn/tun/win/client/tunsetup.hpp>
#include <skvpn/win/npinfo.hpp>
#include <skvpn/win/handlecomm.hpp>

void log_version()
{
  SKVPN_LOG("SkVPN Agent " HTTP_SERVER_VERSION " [" SSL_LIB_NAME "] built on " __DATE__ " " __TIME__);
}
...
#ifdef SKVPN_AGENT_START_PROCESS
...
public:
  ...
  {
    SKVPN_LOG(skvpn::Error::name(err_type));
  }

  std::string dump() const
  {
    std::ostringstream os;
    os << "SkVPN Agent Stats" << std::endl;
    return os.str();
  }
};
...
public:
				  ...
				  bool wintun)
  {
    ...
    auto th = tun->establish(tbc, skvpn_app_path, stop, os, ring_buffer);
  ...
  void set_client_process(Win::ScopedHANDLE&& proc)
  {
    ...
    client_process.async_wait([self=Ptr(this)](const skvpn_io::error_code& error) {
	if (!error)
	  {
	    {
	      std::ostringstream os;
	      ...
	      SKVPN_LOG_NTNL("remove bypass route (failsafe)\n" << os.str());
	    }
		...
		std::ostringstream os;
		self->destroy_tun(os);
		SKVPN_LOG_NTNL("TUN CLOSE (failsafe)\n" << os.str());
	      }
	  }
      });
  }
  ...
  void set_client_confirm_event(const std::string& confirm_handle_hex)
  {
    ...
    if (!::DuplicateHandle(get_client_process(),
      ...
      {
	const Win::LastError err;
	SKVPN_THROW_EXCEPTION("set_client_confirm_event: DuplicateHandle failed: " << err.message());
      }
	...
	default:
	  SKVPN_THROW_EXCEPTION("set_client_confirm_event: WaitForSingleObject failed: " << err.message());
	}
    }
    ...
    client_confirm_event.async_wait([self=Ptr(this)](const skvpn_io::error_code& error) {
	if (!error)
	  {
	    self->remote_tap_handle_hex.clear();
	    SKVPN_LOG_STRING("TUN CONFIRM\n");
	  }
      });
  }
  ...
  void set_client_destroy_event(const std::string& event_handle_hex)
  {
    ...
    if (!::DuplicateHandle(get_client_process(),
      ...
      {
	const Win::LastError err;
	SKVPN_THROW_EXCEPTION("set_client_destroy_event: DuplicateHandle failed: " << err.message());
      }
	...
	default:
	  SKVPN_THROW_EXCEPTION("set_client_destroy_event: WaitForSingleObject failed: " << err.message());
	}
    }

    // normal event-based tun close processing
    client_destroy_event.async_wait([self=Ptr(this)](const skvpn_io::error_code& error) {
	if (!error)
	  {
	    {
	      std::ostringstream os;
	      self->remove_cmds_bypass_hosts.execute(os);
	      self->remove_cmds_bypass_hosts.clear();
	      SKVPN_LOG_NTNL("remove bypass route (event)\n" << os.str());
	    }
		...
		std::ostringstream os;
		self->destroy_tun(os);
		SKVPN_LOG_NTNL("TUN CLOSE (event)\n" << os.str());
	      }
	  }
      });
  }
...
#ifdef SKVPN_AGENT_START_PROCESS
  void start_skvpn_process(HANDLE client_pipe,
			     ...
			     const bool log_append)
  {
...

    {
      ...
      if (res == 0)
	{
	  const skvpn_io::error_code err(::GetLastError(), skvpn_io::error::get_system_category());
	  SKVPN_THROW_EXCEPTION("failed to open thread token: " << err.message());
	}
      res = DuplicateTokenEx(imp_token, 0, NULL, SECURITY_IMPERSONATION_LEVEL::SecurityAnonymous, TokenPrimary, &pri_token);
      if (res == 0)
	{
	  const skvpn_io::error_code err(::GetLastError(), skvpn_io::error::get_system_category());
	  SKVPN_THROW_EXCEPTION("failed to duplicate token: " << err.message());
	}

      // create pipe which is used to write password to skvpn process's stdin
	...
	{
	  const skvpn_io::error_code err(::GetLastError(), skvpn_io::error::get_system_category());
	  SKVPN_THROW_EXCEPTION("failed to set up pipe: " << err.message());
	}

      // create command line for skvpn process
	...
	{
	  const skvpn_io::error_code err(::GetLastError(), skvpn_io::error::get_system_category());
	  SKVPN_THROW_EXCEPTION("failed to create skvpn process: " << err.message());
	}
...
private:
  virtual bool allow_client(AsioPolySock::Base& sock) override
  {
	...
	SKVPN_LOG("connection from " << client_exe);
	if (Agent::valid_pipe(client_exe, config.server_exe))
	  return true;
	SKVPN_LOG(client_exe << " not recognized as a valid client");
#else
	return true;
#endif
      }
    else
      SKVPN_LOG("only named pipe clients are allowed");
    return false;
  }

  TunWin::Setup::Ptr tun;
  skvpn_io::windows::object_handle client_process;
  skvpn_io::windows::object_handle client_confirm_event;
  skvpn_io::windows::object_handle client_destroy_event;
  std::string remote_tap_handle_hex;
  skvpn_io::io_context& io_context_;
...
public:
  ...
  {
    //SKVPN_LOG("INSTANCE START");
  }

  virtual ~MyClientInstance()
  {
    //SKVPN_LOG("INSTANCE DESTRUCT");
  }
...
private:
  ...
  virtual void http_request_received() override
  {
    ...
    try {
      ...
      SKVPN_LOG("HTTP request received from " << sock->remote_endpoint_str() << '\n' << req.to_string());
      ...
      if (req.method == "POST")
	{
	  ...
	  if (req.uri == "/tun-setup")
	  ...
	  else if (req.uri == "/start")
	    {
	      ...
	      parent()->start_skvpn_process(client_pipe, config_file, config_dir, exit_event_name,
  ...
  virtual bool http_stop(const int status, const std::string& description) override
  {
    if (status != WS::Server::Status::E_SUCCESS)
      {
	SKVPN_LOG("INSTANCE STOP : " << WS::Server::Status::error_str(status) << " : " << description);
	return false;
      }
    else
      return true;
...
public:
  ...
  virtual void service_work(DWORD argc, LPWSTR *argv) override
  {
    ...
    io_context.reset(new skvpn_io::io_context(1)); // concurrency hint=1
...
private:
  static Config config()
  {
    ...
    c.display_name = "SkVPN Agent " OVPNAGENT_NAME_STRING;
  ...
  std::unique_ptr<skvpn_io::io_context> io_context;
  MyListener::Ptr listener;
  LogBase::Ptr log;
};

SKVPN_SIMPLE_EXCEPTION(usage);
