...
#include <skvpn/ws/httpcliset.hpp>
...
namespace skvpn {
  ...
  private:
    ...
    {
    public:
      SetupClient(skvpn_io::io_context& io_context,
	...
	ts->post_connect = [host, client_exe, cb=std::move(cb)](WS::ClientSet::TransactionSet& ts, AsioPolySock::Base& sock) {
	  ...
	  if (np)
	  {
	    ...
	    if (!Agent::valid_pipe(client_exe, server_exe))
	      SKVPN_THROW(ovpnagent, host << " server running from " << server_exe << " could not be validated");
	...
	if (service_process.is_open())
	  {
	    service_process.async_wait([handler=std::move(handler)](const skvpn_io::error_code& error) {
		if (!error)
		  handler();
	      });
	  }
	...
	if (!reader->parse(content.c_str(), content.c_str() + content.size(), &jres, &err))
	  {
	    os << content;
	    SKVPN_THROW(ovpnagent, "error parsing returned JSON: " << err);
	  }
	return jres;
      }

      Config::Ptr config;
      skvpn_io::windows::object_handle service_process;
      Win::Event confirm_event;
      Win::DestroyEvent destroy_event;
    };

    virtual TunWin::SetupBase::Ptr new_setup_obj(skvpn_io::io_context& io_context, bool wintun) override
    {
      if (config)
	{
