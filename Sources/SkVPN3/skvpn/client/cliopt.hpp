...
// create an SkVPN client session.  The basic idea here is to look at both
...
#ifndef SKVPN_CLIENT_CLIOPT_H
#define SKVPN_CLIENT_CLIOPT_H
...
#include <skvpn/frame/frame_init.hpp>
#include <skvpn/pki/epkibase.hpp>
#include <skvpn/crypto/cryptodcsel.hpp>
#include <skvpn/ssl/mssparms.hpp>
#include <skvpn/tun/tunmtu.hpp>
#include <skvpn/tun/ipv6_setting.hpp>
#include <skvpn/netconf/hwaddr.hpp>

#include <skvpn/transport/socket_protect.hpp>
#include <skvpn/transport/reconnect_notify.hpp>
#include <skvpn/transport/client/udpcli.hpp>
#include <skvpn/transport/client/tcpcli.hpp>
#include <skvpn/transport/client/httpcli.hpp>
#include <skvpn/transport/altproxy.hpp>
#include <skvpn/transport/dco.hpp>
#include <skvpn/client/cliproto.hpp>
#include <skvpn/client/cliopthelper.hpp>
#include <skvpn/client/optfilt.hpp>
#include <skvpn/client/clilife.hpp>
...
#if defined(SKVPN_PLATFORM_ANDROID)
#include <skvpn/client/cliemuexr.hpp>
#endif

#if defined(SKVPN_EXTERNAL_TRANSPORT_FACTORY)
#include <skvpn/transport/client/extern/config.hpp>
#include <skvpn/transport/client/extern/fw.hpp>
#endif

#if defined(SKVPN_EXTERNAL_TUN_FACTORY)
// requires that client implements ExternalTun::Factory::new_tun_factory
#include <skvpn/tun/extern/config.hpp>
#elif defined(USE_TUN_BUILDER)
#include <skvpn/tun/builder/client.hpp>
#elif defined(SKVPN_PLATFORM_LINUX) && !defined(SKVPN_FORCE_TUN_NULL)
#include <skvpn/tun/linux/client/tuncli.hpp>
#ifdef SKVPN_COMMAND_AGENT
#include <skvpn/client/unix/cmdagent.hpp>
#endif
#elif defined(SKVPN_PLATFORM_MAC) && !defined(SKVPN_FORCE_TUN_NULL)
#include <skvpn/tun/mac/client/tuncli.hpp>
#include <skvpn/apple/maclife.hpp>
#ifdef SKVPN_COMMAND_AGENT
#include <skvpn/client/unix/cmdagent.hpp>
#endif
#elif defined(SKVPN_PLATFORM_WIN) && !defined(SKVPN_FORCE_TUN_NULL)
#include <skvpn/tun/win/client/tuncli.hpp>
#ifdef SKVPN_COMMAND_AGENT
#include <skvpn/client/win/cmdagent.hpp>
#endif
#else
#include <skvpn/tun/client/tunnull.hpp>
#endif

#ifdef PRIVATE_TUNNEL_PROXY
#include <skvpn/pt/ptproxy.hpp>
#endif

#if defined(ENABLE_KOVPN) || defined(ENABLE_OVPNDCO)
#include <skvpn/dco/dcocli.hpp>
#endif

#ifndef SKVPN_UNUSED_OPTIONS
#define SKVPN_UNUSED_OPTIONS "UNUSED OPTIONS"
#endif
...
namespace skvpn {
  ...
  public:
...
#if (defined(ENABLE_KOVPN) || defined(ENABLE_OVPNDCO)) && !defined(SKVPN_FORCE_TUN_NULL) && !defined(SKVPN_EXTERNAL_TUN_FACTORY)
      if (config.dco)
	...
	throw option_error("DCO not enabled in this build");
      ...
      // SkVPN Protocol context (including SSL)
      ...
      else
	{
#if defined(SKVPN_EXTERNAL_TUN_FACTORY)
	  {
	    ...
	    if (!tun_factory)
	      throw option_error("SKVPN_EXTERNAL_TUN_FACTORY: no tun factory");
	  }
#elif defined(USE_TUN_BUILDER)
	  {
	    ...
	    tunconf->tun_prop.remote_list = remote_list;
	    tun_factory = tunconf;
#if defined(SKVPN_PLATFORM_IPHONE)
	...
	// IPv6
	{
      ...
      // show unused options
      opt.show_unused_options(SKVPN_UNUSED_OPTIONS);
    }
...
#endif // SKVPN_EXTERNAL_TRANSPORT_FACTORY
