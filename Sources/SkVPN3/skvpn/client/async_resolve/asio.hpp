...
#ifndef SKVPN_CLIENT_ASYNC_RESOLVE_ASIO_H
#define SKVPN_CLIENT_ASYNC_RESOLVE_ASIO_H
...
namespace skvpn {
  ...
  private:
    ...
    private:
      ...
      ResolveThread(skvpn_io::io_context &io_context_arg,
	...
	return detached.load(std::memory_order_relaxed);
      ...
      void post_callback(typename RESOLVER_TYPE::results_type results,
			 skvpn_io::error_code error)
      {
	skvpn_io::post(io_context, [self=Ptr(this), results, error]()
	{
  ...
  public:
    AsyncResolvable(skvpn_io::io_context& io_context_arg)
      : io_context(io_context_arg)
    {
    }
    ...
    virtual void resolve_callback(const skvpn_io::error_code& error,
...
#endif /* SKVPN_CLIENT_ASYNC_RESOLVE_ASIO_H */
