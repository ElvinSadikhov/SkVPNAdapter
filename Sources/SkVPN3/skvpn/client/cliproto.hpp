...
#ifndef SKVPN_CLIENT_CLIPROTO_H
#define SKVPN_CLIENT_CLIPROTO_H

// This is a middle-layer object in the SkVPN client protocol stack.
// It is above the general SkVPN protocol implementation in
...
//
// This layer deals with setting up an SkVPN client connection:
//
...
// 4. manages the underlying SkVPN protocol object (class ProtoContext)
// 5. handles timers on behalf of the underlying SkVPN protocol object
...
#include <skvpn/ip/ptb.hpp>
#include <skvpn/tun/client/tunbase.hpp>
#include <skvpn/transport/client/transbase.hpp>
#include <skvpn/transport/client/relay.hpp>
#include <skvpn/options/continuation.hpp>
#include <skvpn/options/sanitize.hpp>
#include <skvpn/client/clievent.hpp>
#include <skvpn/client/clicreds.hpp>
#include <skvpn/client/cliconstants.hpp>
#include <skvpn/client/clihalt.hpp>
#include <skvpn/time/asiotimer.hpp>
#include <skvpn/time/coarsetime.hpp>
#include <skvpn/time/durhelper.hpp>
#include <skvpn/error/excode.hpp>

#include <skvpn/ssl/proto.hpp>

#ifdef SKVPN_DEBUG_CLIPROTO
#define SKVPN_LOG_CLIPROTO(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_CLIPROTO(x)
#endif
...
namespace skvpn {
  namespace ClientProto {
    ...
    public:
      ...
      SKVPN_EXCEPTION(client_exception);
      SKVPN_EXCEPTION(client_halt_restart);
      SKVPN_EXCEPTION(tun_exception);
      SKVPN_EXCEPTION(transport_exception);
      SKVPN_EXCEPTION(max_pushed_options_exceeded);
      SKVPN_SIMPLE_EXCEPTION(session_invalidated);
      SKVPN_SIMPLE_EXCEPTION(authentication_failed);
      SKVPN_SIMPLE_EXCEPTION(inactive_timer_expired);
      SKVPN_SIMPLE_EXCEPTION(relay_event);

      SKVPN_EXCEPTION(proxy_exception);
	...
	bool autologin_sessions = false;
      ...
      Session(skvpn_io::io_context& io_context_arg,
	      const Config& config,
	...
	: Base(config.proto_context_config, config.cli_stats),
      ...
      {
#ifdef SKVPN_PACKET_LOG
	packet_log.open(SKVPN_PACKET_LOG, std::ios::binary);
	if (!packet_log)
	  SKVPN_THROW(open_file_error, "cannot open packet log for output: " << SKVPN_PACKET_LOG);
#endif
	Base::update_now();
	Base::reset();
	//Base::enable_strict_skvpn_2x();
	...
	if (!halt)
      ...
      void stop_on_signal(const skvpn_io::error_code& error, int signal_number)
      {
	stop(true);
      }
    ...
    private:
      virtual bool transport_is_skvpn_protocol()
      {
	return true;
      }
      ...
      {
	try {
	  SKVPN_LOG_CLIPROTO("Transport RECV " << server_endpoint_render() << ' ' << Base::dump_packet(buf));
	  ...
	  if (pt.is_data())
	      ...
	      if (buf.size())
		{
		  ...
		  if (tun)
		    {
		      SKVPN_LOG_CLIPROTO("TUN send, size=" << buf.size());
		      tun->tun_send(buf);
		    }
		}
      ...
      {
	try {
	  SKVPN_LOG_CLIPROTO("TUN recv, size=" << buf.size());
	  ...
	  if (buf.size())
	      ...
	      else
		{
		  ...
		  if (buf.size())
		  {
		    ...
		    SKVPN_LOG_CLIPROTO("Transport SEND " << server_endpoint_render() << ' ' << Base::dump_packet(buf));
	...
	cli_events->add_event(std::move(ev));
      ...
      {
	try {
	  SKVPN_LOG("Connecting to " << server_endpoint_render());
	...
	if (notify_callback)
	  {
	    SKVPN_LOG("Transport Error: " << err_text);
	    stop(true);
	  }
	else
	...
	if (notify_callback)
	  {
	    SKVPN_LOG("Proxy Error: " << err_text);
	    stop(true);
	  }
	else
	...
	// auth-token
	{
	  ...
	  if (o)
		...
		{
		  if (!username.empty())
		    SKVPN_LOG("Session user: " << username);
#ifdef SKVPN_SHOW_SESSION_TOKEN
		  SKVPN_LOG("Session token: " << sess_id);
#else
		  SKVPN_LOG("Session token: [redacted]");
#endif
      ...
      {
	SKVPN_LOG_CLIPROTO("Transport SEND " << server_endpoint_render() << ' ' << Base::dump_packet(net_buf));
	...
	//SKVPN_LOG("SERVER: " << sanitize_control_message(msg));
	...
	if (!received_options.complete() && string::starts_with(msg, "PUSH_REPLY,"))
	  {
	    ...
	    if (received_options.complete())
	      {
		// show options
		SKVPN_LOG("OPTIONS:" << std::endl << render_options_sanitized(received_options, Option::RENDER_PASS_FMT|Option::RENDER_NUMBER|Option::RENDER_BRACKET));
		...
		check_proto_warnings();
	      ...
	      SKVPN_LOG("Options continuation...");
	  }
	else if (received_options.complete() && string::starts_with(msg, "PUSH_REPLY,"))
	{
		...
		if (string::starts_with(msg, "AUTH_PENDING,"))
		  {
			...
			try
			  ...
			  {
			    SKVPN_LOG("could not parse AUTH_PENDING timeout: " << timeout_str);
			  }
		      }
		  }
	...
	else if (msg == "RELAY")
	  {
	    ...
	    if (notify_callback)
	      {
		SKVPN_LOG(Error::name(fatal_) << ' ' << fatal_reason_);
		stop(true);
	      }
	    else
	      throw relay_event();
	...
	cli_events->add_event(std::move(ev));
      ...
      {
	SKVPN_LOG("Connected via " + tun->tun_name());
	...
	try {
	  ...
	  {
	    SKVPN_LOG("exception parsing client-ip: " << e.what());
	  }
	...
	try {
	  if (!e && !halt && !received_options.partial())
	      ...
	      if (!sent_push_request)
		{
	      ...
	      SKVPN_LOG("Sending PUSH_REQUEST to server...");
	...
	if (!received_options.partial())
	  {
	    ...
	    push_request_timer.async_wait([self=Ptr(this), dur](const skvpn_io::error_code& error)
                                          {
                                            SKVPN_ASYNC_HANDLER;
	...
	if (comp_type != CompressContext::NONE
      ...
      virtual void active()
      {
	SKVPN_LOG("Session is ACTIVE");
	check_tls_warnings();
	schedule_push_request_callback(Time::Duration::seconds(0));
      }

      void housekeeping_callback(const skvpn_io::error_code& e)
      {
	try {
	  if (!e && !halt)
	    {
	      ...
	      if (Base::invalidated())
		{
		  if (notify_callback)
		    {
		      SKVPN_LOG("Session invalidated: " << Error::name(Base::invalidation_reason()));
		      stop(true);
		    }
		  else
	...
	if (!housekeeping_schedule.similar(next))
	  {
		...
		housekeeping_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
                                              {
                                                SKVPN_ASYNC_HANDLER;
	...
	try {
	  ...
	  {
	    SKVPN_LOG("exception parsing inactive: " << e.what());
	  }
      }
	...
	inactive_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
                                  {
                                    SKVPN_ASYNC_HANDLER;
      ...
      void inactive_callback(const skvpn_io::error_code& e) // fixme for DCO
      {
	try {
	  if (!e && !halt)
	    {
	      ...
	      //SKVPN_LOG("*** INACTIVE SAMPLE " << delta << ' ' << inactive_bytes);
	      if (delta <= count_t(inactive_bytes))
		{
		  ...
		  if (notify_callback)
		    {
		      SKVPN_LOG("inactive timer expired");
		      stop(true);
		    }
		  else
	...
	if (notify_callback)
	  {
	    SKVPN_LOG("Client exception in " << method_name << ": " << e.what());
	    stop(true);
	  }
	else
	...
	if (notify_callback)
	  {
	    SKVPN_LOG("Client halt/restart: " << ch.render());
	    stop(true);
	  }
	else
	...
	info_hold_timer.async_wait([self=Ptr(this)](const skvpn_io::error_code& error)
                                  {
                                    SKVPN_ASYNC_HANDLER;
      ...
      void info_hold_callback(const skvpn_io::error_code& e)
      {
	try {
	  if (!e && !halt)
	    {
