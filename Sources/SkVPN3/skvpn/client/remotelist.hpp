...
// These classes handle parsing and representation of SkVPN "remote" directives,
// and the list of IP addresses that they resolve to.
// <connection> blocks are supported.

#ifndef SKVPN_CLIENT_REMOTELIST_H
#define SKVPN_CLIENT_REMOTELIST_H
...
#include <skvpn/addr/addrlist.hpp>
...
#if SKVPN_DEBUG_REMOTELIST >= 1
#define SKVPN_LOG_REMOTELIST(x) SKVPN_LOG(x)
#else
#define SKVPN_LOG_REMOTELIST(x)
#endif
...
namespace skvpn {
  ...
  public:
	...
	SKVPN_LOG_REMOTELIST("*** RemoteList::Item endpoint SET " << to_string());
      }
	...
	if (res_addr_list && index < res_addr_list->size())
	  {
	    ...
	    SKVPN_LOG_REMOTELIST("*** RemoteList::Item endpoint GET[" << index << "] " << endpoint << ' ' << to_string());
	    return true;
	  }
	else
	  return false;
  ...
  public:
    ...
    SKVPN_EXCEPTION(remote_list_error);
	...
	virtual void pre_resolve_done() = 0;
      ...
      PreResolve(skvpn_io::io_context& io_context_arg,
		...
		if (sitem)
		  {
		    ...
		    SKVPN_LOG_REMOTELIST("*** PreResolve USED CACHE for " << item.server_host);
		    item.res_addr_list = sitem->res_addr_list;
		  }
		else
		  {
		    SKVPN_LOG_REMOTELIST("*** PreResolve RESOLVE on " << item.server_host << " : " << item.server_port);
	...
	if (notify_callback && index < remote_list->list.size())
	  {
		...
		item.set_endpoint_range(results, remote_list->rng.get());
	      ...
	      {
		// resolve failed
		SKVPN_LOG("DNS pre-resolve error on " << item.server_host << ": " << error.message());
		if (stats)
  ...
  private:
	...
	SKVPN_LOG("NOTE: " << option << " directive is not currently supported in <connection> blocks");
    }
