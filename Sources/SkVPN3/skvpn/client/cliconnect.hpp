...
// This file implements the top-level connection logic for an SkVPN client
...
//      The top level object in an SkVPN client connection.
// 2. class ClientProto::Session --
//      The SkVPN client protocol object.
...
//      layer, but isn't specific to SkVPN per-se.

#ifndef SKVPN_CLIENT_CLICONNECT_H
#define SKVPN_CLIENT_CLICONNECT_H
...
#include <skvpn/asio/asiowork.hpp>
#include <skvpn/error/excode.hpp>
#include <skvpn/time/asiotimer.hpp>
#include <skvpn/client/cliopt.hpp>
...
namespace skvpn {
  ...
  public:
    ...
    SKVPN_SIMPLE_EXCEPTION(client_connect_unhandled_exception);

    ClientConnect(skvpn_io::io_context& io_context_arg,
      ...
      if (!halt)
	{
	  if (seconds < 0)
	    seconds = 0;
	  SKVPN_LOG("Client terminated, reconnecting in " << seconds << "...");
	  server_poll_timer.cancel();
	  client_options->remote_reset_cache_item();
	  restart_wait_timer.expires_after(Time::Duration::seconds(seconds));
	  restart_wait_timer.async_wait([self=Ptr(this), gen=generation](const skvpn_io::error_code& error)
                                        {
                                          SKVPN_ASYNC_HANDLER;
    ...
    {
      if (!halt)
	skvpn_io::post(io_context, [self=Ptr(this), reason]()
		   {
		     SKVPN_ASYNC_HANDLER;
		     self->pause(reason);
		   });
    ...
    {
      if (!halt)
	skvpn_io::post(io_context, [self=Ptr(this), seconds]()
		   {
		     SKVPN_ASYNC_HANDLER;
    ...
    {
      if (!halt)
	skvpn_io::post(io_context, [self=Ptr(this), msg=std::move(msg)]()
		   {
		     SKVPN_ASYNC_HANDLER;
  ...
  private:
    ...
    void restart_wait_callback(unsigned int gen, const skvpn_io::error_code& e)
    {
    ...
    void server_poll_callback(unsigned int gen, const skvpn_io::error_code& e)
    {
      if (!e && gen == generation && !halt && !client->first_packet_received())
	{
	  SKVPN_LOG("Server poll timeout, trying next remote entry...");
	  new_client();
	}
    }

    void conn_timer_callback(unsigned int gen, const skvpn_io::error_code& e)
    {
      if (!e && !halt)
	{
      ...
      if (!conn_timer_pending && timeout > 0)
	{
	  ...
	  conn_timer.async_wait([self=Ptr(this), gen=generation](const skvpn_io::error_code& error)
                                {
                                  SKVPN_ASYNC_HANDLER;
    ...
    {
      SKVPN_LOG("Client terminated, restarting in " << delay_ms << " ms...");
      server_poll_timer.cancel();
      interim_finalize();
      client_options->remote_reset_cache_item();
      ...
      if (conn_timer_pending)
	{
	  ...
	  if(timer_left < timeout)
	    {
	      SKVPN_LOG("Extending connection timeout from " << timer_left  << " to " << timeout << " for pending authentification");
	      conn_timer.cancel();
	      conn_timer_pending = false;
      ...
      if (client_options->server_poll_timeout_enabled())
	{
	  ...
	  server_poll_timer.async_wait([self=Ptr(this), gen=generation](const skvpn_io::error_code& error)
                                       {
                                         SKVPN_ASYNC_HANDLER;
