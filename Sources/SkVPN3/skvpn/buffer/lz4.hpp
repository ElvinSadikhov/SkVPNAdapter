...
#include <skvpn/common/socktypes.hpp> // for ntohl/htonl
#include <skvpn/buffer/buffer.hpp>

namespace skvpn {
  namespace LZ4 {
    SKVPN_EXCEPTION(lz4_error);
	...
	SKVPN_THROW(lz4_error, "compress buffer size=" << src.size() << " exceeds LZ4_MAX_INPUT_SIZE=" << LZ4_MAX_INPUT_SIZE);
	...
	dest->write(&size, sizeof(size));
      ...
      if (comp_size <= 0)
	SKVPN_THROW(lz4_error, "LZ4_compress_default returned error status=" << comp_size);
	...
	SKVPN_THROW(lz4_error, "decompress buffer size=" << src.size() << " is too small");
      std::uint32_t size;
	...
	SKVPN_THROW(lz4_error, "decompress expansion size=" << size << " is too large (must be <= " << max_decompressed_size << ')');
      ...
      const int decomp_size = LZ4_decompress_safe((const char *)src.c_data(), (char *)dest->data(),
						  (int)src.size(), size);
      if (decomp_size <= 0)
	SKVPN_THROW(lz4_error, "LZ4_decompress_safe returned error status=" << decomp_size);
      if (static_cast<unsigned int>(decomp_size) != size)
	SKVPN_THROW(lz4_error, "decompress size inconsistency expected_size=" << size << " actual_size=" << decomp_size);
