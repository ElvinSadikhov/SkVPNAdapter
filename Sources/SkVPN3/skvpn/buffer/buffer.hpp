...
// SkVPN core.  Normally SkVPN uses buffers of unsigned chars, but the
...
#ifndef SKVPN_BUFFER_BUFFER_H
#define SKVPN_BUFFER_BUFFER_H
...
#include <skvpn/common/abort.hpp>
#include <skvpn/common/exception.hpp>
#include <skvpn/common/rc.hpp>
#include <skvpn/buffer/bufclamp.hpp>

#ifdef SKVPN_BUFFER_ABORT
#define SKVPN_BUFFER_THROW(exc) { std::abort(); }
#else
#define SKVPN_BUFFER_THROW(exc) { throw BufferException(BufferException::exc); }
#endif
...
namespace skvpn {
  ...
  public:
	...
	SKVPN_BUFFER_THROW(buffer_headroom);
      offset_ = headroom;
      size_ = 0;
    }
	...
	SKVPN_BUFFER_THROW(buffer_offset);
      offset_ = offset;
      size_ = size;
    }
    ...
    {
      if (size > max_size())
	SKVPN_BUFFER_THROW(buffer_set_size);
      size_ = size;
    }
    ...
    {
      if (!offset_)
	SKVPN_BUFFER_THROW(buffer_push_front_headroom);
      --offset_;
      ++size_;
      *data() = value;
    }
    ...
    {
      if (!size_)
	SKVPN_BUFFER_THROW(buffer_pop_back);
      return *(data()+(--size_));
    }
    ...
    {
      if (index >= size_)
	SKVPN_BUFFER_THROW(buffer_index);
      return data()[index];
    }
    ...
    {
      if (index >= size_)
	SKVPN_BUFFER_THROW(buffer_const_index);
      return c_data()[index];
    }
...
#ifndef SKVPN_NO_IO
    // return a skvpn_io::mutable_buffer object used by
    // asio read methods, starting from data()
    skvpn_io::mutable_buffer mutable_buffer(const size_t tailroom = 0)
    {
      return skvpn_io::mutable_buffer(data(), max_size_tailroom(tailroom));
    }

    // return a skvpn_io::mutable_buffer object used by
    // asio read methods, starting from data_end()
    skvpn_io::mutable_buffer mutable_buffer_append(const size_t tailroom = 0)
    {
      return skvpn_io::mutable_buffer(data_end(), remaining(tailroom));
    }

    // return a skvpn_io::const_buffer object used by
    // asio write methods.
    skvpn_io::const_buffer const_buffer() const
    {
      return skvpn_io::const_buffer(c_data(), size());
    }

    // clamped versions of mutable_buffer(), mutable_buffer_append(),
    // and const_buffer()

    skvpn_io::mutable_buffer mutable_buffer_clamp(const size_t tailroom = 0)
    {
      return skvpn_io::mutable_buffer(data(), buf_clamp_read(max_size_tailroom(tailroom)));
    }

    skvpn_io::mutable_buffer mutable_buffer_append_clamp(const size_t tailroom = 0)
    {
      return skvpn_io::mutable_buffer(data_end(), buf_clamp_read(remaining(tailroom)));
    }

    skvpn_io::const_buffer const_buffer_clamp() const
    {
      return skvpn_io::const_buffer(c_data(), buf_clamp_write(size()));
    }

    skvpn_io::const_buffer const_buffer_limit(const size_t limit) const
    {
      return skvpn_io::const_buffer(c_data(), std::min(buf_clamp_write(size()), limit));
    }
#endif
	...
	SKVPN_BUFFER_THROW(buffer_underflow);
    }
  ...
  protected:
    ...
    {
      SKVPN_BUFFER_THROW(buffer_no_reset_impl);
    }
