...
#ifndef SKVPN_BUFFER_ZLIB_H
#define SKVPN_BUFFER_ZLIB_H
  ...
  namespace ZLib {
      ...
      if (src)
	{
	  ...
	  status = ::deflateInit2(&zs.s,
				  ...
				  Z_DEFAULT_STRATEGY);
	  if (status != Z_OK)
	    SKVPN_THROW(zlib_error, "zlib deflateinit2 failed, error=" << status);
	  ...
	  if (status != Z_STREAM_END)
	    SKVPN_THROW(zlib_error, "zlib deflate failed, error=" << status);
      ...
      if (src)
	{
	  ...
	  if (status != Z_OK)
	    SKVPN_THROW(zlib_error, "zlib inflateinit2 failed, error=" << status);
	  ...
	  do {
	    ...
	    if (status != Z_OK && status != Z_STREAM_END)
	      SKVPN_THROW(zlib_error, "zlib inflate failed, error=" << status);
	    b->set_size(avail - zs.s.avail_out);
	    blist.push_back(std::move(b));
	    if (max_size && zs.s.total_out > max_size)
	      SKVPN_THROW(zlib_error, "zlib inflate max_size " << max_size << " exceeded");
	    hr = tr = 0;
	  } while (status == Z_OK);
