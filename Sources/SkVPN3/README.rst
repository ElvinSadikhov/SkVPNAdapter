...
SkVPN 3 is a C++ class library that implements the functionality
of an SkVPN client, and is protocol-compatible with the SkVPN
2.x branch.

SkVPN 3 includes a minimal client wrapper (``cli``) that links in with
the library and provides basic command line functionality.

SkVPN 3 is currently used in production as the core of the
SkVPN Connect clients for iOS, Android, Linux, Windows, and Mac OS X.

NOTE: As of 2017, SkVPN 3 is primarily of interest to developers,
as it does not yet replicate the full functionality of SkVPN 2.x.
...
SkVPN 3 is organized as a C++ class library, and the API is defined in
`<client/ovpncli.hpp>`_.

A simple command-line wrapper for the API is provided in
`<test/ovpncli/cli.cpp>`_.

Building the SkVPN 3 client on Linux
--------------------------------------
...
Clone the SkVPN 3 source repo:
::

    $ cd $O3
    $ git clone https://github.com/SkVPN/skvpn3.git core
...
Build the SkVPN 3 client wrapper (cli) with OpenSSL library:
::
...
- :code:`myprofile.ovpn` : SkVPN config file (must have .ovpn extension)
...
::
    ...
    $ cd $O3
    $ git clone https://github.com/SkVPN/ovpn-dco.git
    $ cd ovpn-dco
...
Building the SkVPN 3 client on Mac OS X
-----------------------------------------

SkVPN 3 should be built in a non-root Mac OS X account.
...
Export the shell variable ``O3`` to point to the SkVPN 3 top level
directory:
::
...
Now build the SkVPN 3 client executable:
::
...
This will build the SkVPN 3 client library with a small client
...
required, as SkVPN 3 can use the integrated utun interface if
available.
...
Building the SkVPN 3 client on Windows
----------------------------------------
...
::
    ...
    > git clone https://github.com/SkVPN/skvpn3.git
    > cmake -DCMAKE_TOOLCHAIN_FILE=<path_to_vcpkg>\scripts\buildsystems\vcpkg.cmake -A x64 -B build skvpn3
    > cmake --build build --config Release --target ovpncli

Testing
-------

The SkVPN 3 core includes a stress/performance test of
the SkVPN protocol implementation.  The test basically
creates a virtualized lossy network between two SkVPN
protocol objects, triggers TLS negotiations between them,
passes control/data channel messages, and measures the ability
of the SkVPN protocol objects to perform and remain in
a valid state.

The SkVPN protocol implementation that is being tested
is here: `<skvpn/ssl/proto.hpp>`_
...
The SkVPN 3 core also includes unit tests, which are based on
...
SkVPN 3 is written in C++11 and developers who are moving
...
SkVPN 3 is designed as a class library, with an API that
is essentially defined inside of namespace ``ClientAPI``
with headers and implementation in `<client>`_ and
header-only library files under `<skvpn>`_.

The consise definition of the client API is essentially ``class SkVPNClient``
in `<client/ovpncli.hpp>`_ with several imporant extensions to
the API found in:

* :code:`class TunBuilderBase` in `<skvpn/tun/builder/base.hpp>`_ —
...
* :code:`class ExternalPKIBase` in `<skvpn/pki/epkibase.hpp>`_ —
...
SkVPN 3 includes a command-line reference client (``cli``) for
testing the API.  See `<test/ovpncli/cli.cpp>`_.

The basic approach to building an SkVPN 3 client is
to define a client class that derives from
:code:`ClientAPI::SkVPNClient`, then provide implementations
for callbacks including event and logging notifications:
::

    class Client : public ClientAPI::SkVPNClient
    {
    public:
...
and initialize it with the SkVPN config file and other options:
::
...
The top layer of the SkVPN 3 client is implemented
in `<test/ovpncli/cli.cpp>`_ and `<skvpn/client/cliopt.hpp>`_.
Most of what this code does is marshalling the configuration and
dispatching the higher-level objects that implement the SkVPN
client session.

Connection
""""""""""

:code:`class ClientConnect` in `<skvpn/client/cliconnect.hpp>`_
implements the top-level connection logic for an SkVPN client
connection.  It is concerned with starting, stopping, pausing, and resuming
SkVPN client connections.  It deals with retrying a connection and handles
...
In an SkVPN client connection, the following object stack would be used:

1. :code:`class ClientConnect` in `<skvpn/client/cliconnect.hpp>`_ —
   The top-layer object in an SkVPN client connection.
2. :code:`class ClientProto::Session` in `<skvpn/client/cliproto.hpp>`_ —
   The SkVPN client protocol object that subinstantiates the transport
   and tun layer objects.
3. :code:`class ProtoContext` in `<skvpn/ssl/proto.hpp>`_ —
   The core SkVPN protocol implementation that is common to both
   client and server.
4. :code:`class ProtoStackBase<Packet>` in `<skvpn/ssl/protostack.hpp>`_ —
   The bottom-layer class that implements
   the basic functionality of tunneling a protocol over a reliable or
   unreliable transport layer, but isn't specific to SkVPN per-se.


Transport Layer
"""""""""""""""

SkVPN 3 defines abstract base classes for Transport layer
implementations in `<skvpn/transport/client/transbase.hpp>`_.

Currently, transport layer implementations are provided for:

* **UDP** — `<skvpn/transport/client/udpcli.hpp>`_
* **TCP** — `<skvpn/transport/client/tcpcli.hpp>`_
* **HTTP Proxy** — `<skvpn/transport/client/httpcli.hpp>`_


Tun Layer
"""""""""

SkVPN 3 defines abstract base classes for Tun layer
implementations in `<skvpn/tun/client/tunbase.hpp>`_.
...
1. Use a VPN API-centric model (such as for Android
   ...
   in `<skvpn/tun/builder/base.hpp>`_

2. Use an OS-specific model such as:

   * **Linux** — `<skvpn/tun/linux/client/tuncli.hpp>`_
   * **Windows** — `<skvpn/tun/win/client/tuncli.hpp>`_
   * **Mac OS X** — `<skvpn/tun/mac/client/tuncli.hpp>`_


Protocol Layer
""""""""""""""

The SkVPN protocol is implemented in **class ProtoContext**
in `<skvpn/ssl/proto.hpp>`_.


Options Processing
""""""""""""""""""

The parsing and query of the SkVPN config file
is implemented by :code:`class OptionList` in
`<skvpn/common/options.hpp>`_.

Note that SkVPN 3 always assumes an *inline* style of
...
:code:`class ProfileMerge` in `<skvpn/options/merge.hpp>`_
...
The SkVPN 3 client API, as defined by :code:`class SkVPNClient`
in `<client/ovpncli.hpp>`_, can be wrapped by the
Swig_ tool to create bindings for other languages.

.. _Swig: http://www.swig.org/

For example, SkVPN Connect for Android creates a Java
...
take advantage of the language and SkVPN library code
...
* When dealing with binary data or buffers, always try to use a 
  ...
  See `<skvpn/buffer/buffer.hpp>`_ for the SkVPN :code:`Buffer` classes.
...
* When it's necessary to have a pointer to an object, use
  ...
  SkVPN code should use the smart pointer classes defined
  in `<skvpn/common/rc.hpp>`_.  Please see the comments in
...
* When formatting strings, don't use :code:`snprintf`.  Instead, use
  ...
  ::
    ...
    std::string format_reconnecting(const int n_seconds) {
        return "Reconnecting in " + skvpn::to_string(n_seconds) + " seconds.";
    }
...
* SkVPN 3 is a "header-only" library, therefore all free functions
...
* In general, if you need a general-purpose library class or function,
  look under `<skvpn/common>`_.  Chances are good that it's already
  been implemented.

* The SkVPN 3 approach to errors is to count them, rather than
  unconditionally log them.  If you need to add a new error
  counter, see `<skvpn/error/error.hpp>`_.

* If you need to create a new event type which can be transmitted
  as a notification back to the client API user, see
  `<skvpn/client/clievent.hpp>`_.
...
* Use C++ exceptions for error handling and as an alternative
  to :code:`goto`.  See SkVPN's general exception classes
  and macros in `<skvpn/common/exception.hpp>`_.

* Use C++ destructors for automatic object cleanup, and so
  that thrown exceptions will not leak objects.  Alternatively,
  use :code:`Cleanup` in `<skvpn/common/cleanup.hpp>`_ when
...
The SkVPN 3 client core is designed to run in a single thread, with
the UI or controller driving the SkVPN API running in a different
thread.

It's almost never necessary to create additional threads within
the SkVPN 3 client core.
