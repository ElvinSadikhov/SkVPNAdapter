...
// Implementation file for SkVPNClient API defined in ovpncli.hpp.
...
// SKVPN_CORE_API_VISIBILITY_HIDDEN is defined
#if defined(__GNUC__)
#define SKVPN_CLIENT_EXPORT
#ifndef SKVPN_CORE_API_VISIBILITY_HIDDEN
...
//#define SKVPN_DUMP_CONFIG          // dump parsed configuration (comment out)
//#define SKVPN_DEBUG_CLIPROTO       // shows packets in/out (comment out)
#define SKVPN_DEBUG_PROTO   1        // increases low-level protocol verbosity (1)
//#define SKVPN_DEBUG_PROTO_DUMP     // dump hex of transport-layer packets, requires SKVPN_DEBUG_CLIPROTO (comment out)
//#define SKVPN_DEBUG_VERBOSE_ERRORS // verbosely log Error::Type errors (comment out)
#define SKVPN_DEBUG_TUN     2        // debug level for tun object (2)
#define SKVPN_DEBUG_UDPLINK 2        // debug level for UDP link object (2)
#define SKVPN_DEBUG_TCPLINK 2        // debug level for TCP link object (2)
#define SKVPN_DEBUG_COMPRESS 1       // debug level for compression objects (1)
#define SKVPN_DEBUG_REMOTELIST 0     // debug level for RemoteList object (0)
#define SKVPN_DEBUG_TUN_BUILDER 0    // debug level for tun/builder/client.hpp (0)
//#define SKVPN_SHOW_SESSION_TOKEN   // show server-pushed auth-token (comment out)
//#define SKVPN_DEBUG_TAPWIN           // shows Windows TAP driver debug logging (comment out)

// enable assertion checks (can safely be disabled in production)
//#define SKVPN_ENABLE_ASSERT

// force null tun device (useful for testing)
//#define SKVPN_FORCE_TUN_NULL

// log cleartext tunnel packets to file for debugging/analysis
//#define SKVPN_PACKET_LOG "pkt.log"

#ifndef SKVPN_LOG
// log thread settings
#define SKVPN_LOG_CLASS skvpn::ClientAPI::LogReceiver
#define SKVPN_LOG_INFO  skvpn::ClientAPI::LogInfo
#include <skvpn/log/logthread.hpp>    // should be included early
#endif

// log SSL handshake messages
#define SKVPN_LOG_SSL(x) SKVPN_LOG(x)

// on Android and iOS, use TunBuilderBase abstraction
#include <skvpn/common/platform.hpp>
#if (defined(SKVPN_PLATFORM_ANDROID) || defined(SKVPN_PLATFORM_IPHONE)) && !defined(SKVPN_FORCE_TUN_NULL) && !defined(SKVPN_EXTERNAL_TUN_FACTORY)
#define USE_TUN_BUILDER
#endif

#include <skvpn/init/initprocess.hpp>
#include <skvpn/common/bigmutex.hpp>
#include <skvpn/common/size.hpp>
#include <skvpn/common/platform_string.hpp>
#include <skvpn/common/count.hpp>
#include <skvpn/asio/asiostop.hpp>
#include <skvpn/time/asiotimer.hpp>
#include <skvpn/client/cliconnect.hpp>
#include <skvpn/client/cliopthelper.hpp>
#include <skvpn/options/merge.hpp>
#include <skvpn/error/excode.hpp>
#include <skvpn/crypto/selftest.hpp>

// copyright
#include <skvpn/legal/copyright.hpp>

namespace skvpn {
  namespace ClientAPI {

    SKVPN_SIMPLE_EXCEPTION(app_expired);
    ...
    public:
      ...
      MySessionStats(SkVPNClient* parent_arg)
	: parent(parent_arg)
      {
	std::memset(errors, 0, sizeof(errors));
#ifdef SKVPN_DEBUG_VERBOSE_ERRORS
...
#ifdef SKVPN_DEBUG_VERBOSE_ERRORS
	    if (text)
	      SKVPN_LOG("ERROR: " << Error::name(err) << " : " << *text);
	    else
	      SKVPN_LOG("ERROR: " << Error::name(err));
#endif
	    ++errors[err];
	  }
      }
    ...
    public:
      ...
      MyClientEvents(SkVPNClient* parent_arg) : parent(parent_arg) {}
    ...
    public:
      ...
      void set_parent(SkVPNClient* parent_arg)
      {
	parent = parent_arg;
      }
	...
	if (parent)
	  {
#if defined(SKVPN_COMMAND_AGENT) && defined(SKVPN_PLATFORM_WIN)
	    return rg_local ? true : WinCommandAgent::add_bypass_route(endpoint);
#elif defined(SKVPN_COMMAND_AGENT) && defined(SKVPN_PLATFORM_MAC)
    ...
    private:
      SkVPNClient* parent = nullptr;
    };

    class MyClockTick
    {
    public:
      MyClockTick(skvpn_io::io_context& io_context,
		  SkVPNClient* parent_arg,
	...
	timer.async_wait([this](const skvpn_io::error_code& error)
			 {
			   if (!parent || error)
			     return;
			   try {
	...
	void attach(SkVPNClient* parent,
		    skvpn_io::io_context* io_context,
		    Stop* async_stop_global)
	{
	  ...
	  if (io_context)
	    ...
	    {
	      io_context_ = new skvpn_io::io_context(1); // concurrency hint=1
	...
	skvpn_io::io_context* io_context_ = nullptr;
    ...
    SKVPN_CLIENT_EXPORT SkVPNClient::SkVPNClient()
    {
#ifndef SKVPN_NORESET_TIME
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::parse_config(const Config& config, EvalConfig& eval, OptionList& options)
    {
      try {
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::parse_extras(const Config& config, EvalConfig& eval)
    {
      try {
...
#else
	  throw Exception("client not built with SKVPN_GREMLIN");
#endif
	  }
    ...
    SKVPN_CLIENT_EXPORT long SkVPNClient::max_profile_size()
    {
      return ProfileParseLimits::MAX_PROFILE_SIZE;
    }

    SKVPN_CLIENT_EXPORT MergeConfig SkVPNClient::merge_config_static(const std::string& path,
    ...
    SKVPN_CLIENT_EXPORT MergeConfig SkVPNClient::merge_config_string_static(const std::string& config_content)
    {
    ...
    SKVPN_CLIENT_EXPORT MergeConfig SkVPNClient::build_merge_config(const ProfileMerge& pm)
    {
      MergeConfig ret;
    ...
    SKVPN_CLIENT_EXPORT EvalConfig SkVPNClient::eval_config_static(const Config& config)
    {
      EvalConfig eval;
      OptionList options;
    ...
    SKVPN_CLIENT_EXPORT EvalConfig SkVPNClient::eval_config(const Config& config)
    {
    ...
    SKVPN_CLIENT_EXPORT Status SkVPNClient::provide_creds(const ProvideCreds& creds)
    {
      Status ret;
      try {
    ...
    SKVPN_CLIENT_EXPORT bool SkVPNClient::socket_protect(int socket, std::string remote, bool ipv6)
    {
      return true;
    }

    SKVPN_CLIENT_EXPORT bool SkVPNClient::parse_dynamic_challenge(const std::string& cookie, DynamicChallenge& dc)
    {
      try {
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::process_epki_cert_chain(const ExternalPKICertRequest& req)
    {
    ...
    SKVPN_CLIENT_EXPORT Status SkVPNClient::connect()
    {
#if !defined(SKVPN_OVPNCLI_SINGLE_THREAD)
      skvpn_io::detail::signal_blocker signal_blocker; // signals should be handled by parent thread
#endif
#if defined(SKVPN_LOG_LOGTHREAD_H) && !defined(SKVPN_LOG_LOGBASE_H)
...
#error ovpn3 core logging object only supports thread-local scope
      ...
      SKVPN_LOG(ClientAPI::SkVPNClient::platform());

      return do_connect();
    }

    SKVPN_CLIENT_EXPORT Status SkVPNClient::do_connect()
    {
      Status status;
      bool session_started = false;
      try {
	connect_attach();
#if defined(SKVPN_OVPNCLI_ASYNC_SETUP)
	skvpn_io::post(*state->io_context(), [this]() {
	    do_connect_async();
	  });
#else
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::do_connect_async()
    {
      enum StopType {
	NONE,
	SESSION,
	...
	connect_session_stop();
#ifdef SKVPN_IO_REQUIRES_STOP
      if (stop_type == EXPLICIT)
	state->io_context()->stop();
#endif
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::connect_setup(Status& status, bool& session_started)
    {
	...
	cc.remote_override = &state->remote_override;
      ...
      cc.builder = this;
#endif
#if defined(SKVPN_EXTERNAL_TUN_FACTORY)
      cc.extern_tun_factory = this;
#endif
#if defined(SKVPN_EXTERNAL_TRANSPORT_FACTORY)
    ...
    SKVPN_CLIENT_EXPORT Status SkVPNClient::status_from_exception(const std::exception& e)
    {
      Status ret;
      ret.error = true;
      ret.message = Unicode::utf8_printable<std::string>(e.what(), 256);
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::connect_attach()
    {
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::connect_pre_run()
    {
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::connect_run()
    {
      state->io_context()->run();
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::connect_session_stop()
    {
      state->session->stop();     // On exception, stop client...
      state->io_context()->poll();  //   and execute completion handlers.
    }

    SKVPN_CLIENT_EXPORT ConnectionInfo SkVPNClient::connection_info()
    {
      ConnectionInfo ci;
    ...
    SKVPN_CLIENT_EXPORT bool SkVPNClient::session_token(SessionToken& tok)
    {
      if (state->is_foreign_thread_access())
	{
    ...
    SKVPN_CLIENT_EXPORT Stop* SkVPNClient::get_async_stop()
    {
      return nullptr;
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::external_pki_error(const ExternalPKIRequestBase& req, const size_t err_type)
    {
      if (req.error)
	{
	  if (req.invalidAlias)
    ...
    SKVPN_CLIENT_EXPORT bool SkVPNClient::sign(const std::string& data, std::string& sig, const std::string& algorithm)
    {
    ...
    SKVPN_CLIENT_EXPORT bool SkVPNClient::remote_override_enabled()
    {
      return false;
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::remote_override(RemoteOverride&)
    {
    }

    SKVPN_CLIENT_EXPORT int SkVPNClient::stats_n()
    {
      return (int)MySessionStats::combined_n();
    }

    SKVPN_CLIENT_EXPORT std::string SkVPNClient::stats_name(int index)
    {
      return MySessionStats::combined_name(index);
    }

    SKVPN_CLIENT_EXPORT long long SkVPNClient::stats_value(int index) const
    {
      if (state->is_foreign_thread_access())
	{
    ...
    SKVPN_CLIENT_EXPORT std::vector<long long> SkVPNClient::stats_bundle() const
    {
    ...
    SKVPN_CLIENT_EXPORT InterfaceStats SkVPNClient::tun_stats() const
    {
      InterfaceStats ret;
      if (state->is_foreign_thread_access())
	{
    ...
    SKVPN_CLIENT_EXPORT TransportStats SkVPNClient::transport_stats() const
    {
      TransportStats ret;
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::stop()
    {
      if (state->is_foreign_thread_access())
	state->trigger_async_stop_local();
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::pause(const std::string& reason)
    {
      if (state->is_foreign_thread_access())
	{
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::resume()
    {
      if (state->is_foreign_thread_access())
	{
	  ClientConnect* session = state->session.get();
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::reconnect(int seconds)
    {
      if (state->is_foreign_thread_access())
	{
	  ClientConnect* session = state->session.get();
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::post_cc_msg(const std::string& msg)
    {
      if (state->is_foreign_thread_access())
	{
	  ClientConnect* session = state->session.get();
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::clock_tick()
    {
    }

    SKVPN_CLIENT_EXPORT void SkVPNClient::on_disconnect()
    {
      state->on_disconnect();
    }

    SKVPN_CLIENT_EXPORT std::string SkVPNClient::crypto_self_test()
    {
      return SelfTest::crypto_self_test();
    }

    SKVPN_CLIENT_EXPORT int SkVPNClient::app_expire()
    {
    ...
    SKVPN_CLIENT_EXPORT void SkVPNClient::check_app_expired()
    {
#ifdef APP_EXPIRE_TIME
    ...
    SKVPN_CLIENT_EXPORT std::string SkVPNClient::copyright()
    {
      return skvpn_copyright;
    }

    SKVPN_CLIENT_EXPORT std::string SkVPNClient::platform()
    {
