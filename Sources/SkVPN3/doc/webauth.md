...
==========================
This document describes various web based protocols that are used together with SkVPN. 
They are not part of core protocol of SkVPN but are extremely closely related.
Interoperability between products building on SkVPN is expected, so products
...
describes a simpler REST based interface that SkVPN Connect Client and Access Server use to
download profiles to the client with minimal user interaction.

SkVPN web auth protocol during Connect
========================================

This document describes the assumption and what client and server should implement
to facilitate a web based second factor login for SkVPN.
...
https://github.com/SkVPN/skvpn/blob/master/doc/management-notes.txt
...
(see `--auth-token` and `--auth-gen-token` in the SkVPN man page as well as 
...
designed to work with SkVPN. Therefore this feature must be implemented strictly as opt-in.
...
the URL in the default browser of the user that has no specific SkVPN support. On the server
side any identity provider should be able to be used. Unless the IdP is tightly integrated/designed
for SkVPN web auth, it will not support the State API. 
...
browser the SkVPN profile should be offered as download with be content-type  
`application/x-skvpn-profile`.
...
a `HEAD` or `GET` request to `https://servername/skvpn-api/profile`. If the response
...
To start the web based method the client should load the url
  ...
  https://servername/skvpn-api/profile
...
- `appVersion`	Version of SkVPN client 
...
This is currently mainly implemented by SkVPN Access Server API and 
...
in contrast to the right type `application/x-skvpn-profile`). Credentials 
...
To get the Autologin configuration using curl (from a client without tls-cryptv2 support):
    ...
    $ curl -u USERNAME:PASSWORD https://asdemo.skvpn.net/rest/GetAutologin

To get the Userlogin (requiring authentication) configuration using curl, indicating
that the client supports tls crypt v2

    $ curl -u USERNAME:PASSWORD https://asdemo.skvpn.net/rest/GetUserlogin?tls-cryptv2=1&action=import
...
Internally SkVPN Access server used XMLRPC when this and the current 
...
of 'mypass".  Get the SkVPN config file:
