...
public class Client implements SkVPNClientThread.EventReceiver {
    private SkVPNClientThread client_thread;
    ...
    // Load SkVPN core (implements ClientAPI_SkVPNClient) from shared library 
    static {
	System.loadLibrary("ovpncli");
	String test = ClientAPI_SkVPNClient.crypto_self_test();
	...
	client_thread = new SkVPNClientThread();
	...
	if (ec.getError())
	    throw new ConfigError("SkVPN config file parse error: " + ec.getMessage());
	...
	if (!ec.getAutologin())
		...
		if (username.length() > 0)
		    ...
		    throw new CredsUnspecifiedError("SkVPN config file requires username/password but none provided");
	    }
    ...
    @Override
    public SkVPNClientThread.TunBuilder tun_builder_new()
    {
	return null;
    }
 }
