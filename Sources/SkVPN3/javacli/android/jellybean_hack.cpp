...
#ifndef SKVPN_PACKAGE_ID
#error SKVPN_PACKAGE_ID must be defined
#endif

#define MAKE_SYM2(pkg_id, suffix) Java_ ## pkg_id ## _JellyBeanHack_ ## suffix
#define MAKE_SYM(pkg_id, suffix) MAKE_SYM2(pkg_id, suffix)

#define RSA_SIGN_INIT MAKE_SYM(SKVPN_PACKAGE_ID, rsa_1sign_1init)
#define RSA_SIGN      MAKE_SYM(SKVPN_PACKAGE_ID, rsa_1sign)
#define PKEY_RETAIN   MAKE_SYM(SKVPN_PACKAGE_ID, pkey_1retain)
  ...
  if (pkey && CRYPTO_add_lock)
    {
      ...
      __android_log_print(ANDROID_LOG_DEBUG, "skvpn", "pkey_retain ref=%d", newref);
    }
}
