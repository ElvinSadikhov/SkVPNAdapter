...
@testable import SkVPNAdapter

class SkVPNAdapterTests: XCTestCase {
    
    private class CustomFlow: NSObject, SkVPNAdapterPacketFlow {
    ...
    private class AdapterDelegate: NSObject, SkVPNAdapterDelegate {
        
        let settingsHandler: (NEPacketTunnelNetworkSettings?) -> Void
        let errorHandler: (Error) -> Void
        let eventHandler: (SkVPNAdapterEvent) -> Void
        
        init(
            settingsHandler: @escaping (NEPacketTunnelNetworkSettings?) -> Void,
            errorHandler: @escaping (Error) -> Void,
            eventHandler: @escaping (SkVPNAdapterEvent) -> Void
        ) {
        ...
        func skVPNAdapter(
            _ skVPNAdapter: SkVPNAdapter, handleEvent event: SkVPNAdapterEvent, message: String?
        ) {
            eventHandler(event)
        }
    ...
    func testVPNConnection() {
        ...
        let preEvaluation: SkVPNConfigurationEvaluation
        do {
            preEvaluation = try SkVPNAdapter.evaluate(configuration: configuration)
        } catch {
        ...
        let finalEvaluation: SkVPNConfigurationEvaluation
        do {
            finalEvaluation = try adapter.apply(configuration: configuration)
        } catch {
            XCTFail("Failed to apply SkVPN configuration due to error: \(error)")
            return
        }
