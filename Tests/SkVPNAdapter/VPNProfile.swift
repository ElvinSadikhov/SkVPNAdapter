...
extension VPNProfile {
    ...
    static let live: VPNProfile = {
        let profileName: String = <#SKVPN_PROFILE_NAME#>

        let configuration: String = <#SKVPN_CONFIGURATION#>

        let cert: String? = <#SKVPN_CERT#>
        let key: String? = <#SKVPN_KEY#>

        let keyPassword: String? = <#PRIVATE_KEY_PASSWORD#>
        
        let username: String? = <#SKVPN_USERNAME#>
        let password: String? = <#SKVPN_PASSWORD#>

        let settings: [String: String]? = <#SKVPN_ADDITIONAL_SETTINGS#>
