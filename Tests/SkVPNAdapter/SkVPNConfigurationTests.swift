...
class SkVPNConfigurationTests: XCTestCase {
    ...
    func testEvaluateEmptyConfig() {
        ...
        do {
            let _ = try SkVPNAdapter.evaluate(configuration: configuration)
            XCTFail("We shouldn't be here, evaluation should fail.")
        } catch {
            guard let fatal = (error as NSError).userInfo[SkVPNAdapterErrorFatalKey] as? Bool else {
                XCTFail("Error should contain SkVPNAdapterErrorFatalKey.")
                return
            }
    ...
    func testEvaluateManuallyPopulatedConfig() {
        ...
        do {
            evaluation = try SkVPNAdapter.evaluate(configuration: configuration)
        } catch {
