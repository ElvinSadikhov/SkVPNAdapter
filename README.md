...
## !!!ATTENTION!!!
Due to the lack of time and motivation the support of this project was dropped. If you need a solution for SkVPN please take a look at alternatives such as [TunnelKit](https://github.com/passepartoutvpn/tunnelkit).

## Overview
SkVPNAdapter is an Objective-C framework that allows to easily configure and establish VPN connection using SkVPN protocol. It is based on the original [skvpn3](https://github.com/SkVPN/skvpn3) library so it has every feature the library has.
...
### Carthage
To install SkVPNAdapter with Carthage, add the following line to your `Cartfile`.

```
github "ss-abramchuk/SkVPNAdapter"
```
...
### Cocoapods
To install SkVPNAdapter with Cocoapods, add the following lines to your `Podfile`.
...
Add `SkVPNAdapter` package to your project using File > Swift Packages > Add Package Dependency menu. Xcode 11 will automatically retrieve all necessary dependencies. In addition to that you need to add `SystemConfiguration` framework to the Frameworks and Libraries. If you work on iOS project add `UIKit` as well.
...
self.providerManager?.loadFromPreferences(completionHandler: { (error) in
    ...
    // extension target.
    tunnelProtocol.providerBundleIdentifier = "com.example.skvpn-client.tunnel-provider"
    ...
    self.providerManager?.localizedDescription = "SkVPN Client"
...
Packet Tunnel Provider extension uses [`NEPacketTunnelProvider`](https://developer.apple.com/documentation/networkextension/nepackettunnelprovider) subclass to configure and establish VPN connection. Therefore, that class is the right place to configure SkVPNAdapter. The following example shows how you can setup it:

```swift
import NetworkExtension
import SkVPNAdapter

// Extend NEPacketTunnelFlow to adopt SkVPNAdapterPacketFlow protocol so that
// `self.packetFlow` could be sent to `completionHandler` callback of SkVPNAdapterDelegate
// method skVPNAdapter(skVPNAdapter:configureTunnelWithNetworkSettings:completionHandler).
...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(
        ...
        // There are many ways to provide SkVPN settings to the tunnel provider. For instance,
        ...
        // of the SkVPN configuration file. Other options related to the tunnel
        ...
        let evaluation: SkVPNConfigurationEvaluation
        do {
...
extension PacketTunnelProvider: SkVPNAdapterDelegate {
    ...
    // SkVPNAdapter calls this delegate method to configure a VPN tunnel.
    // `completionHandler` callback requires an object conforming to `SkVPNAdapterPacketFlow`
    // protocol if the tunnel is configured without errors. Otherwise send nil.
    // `SkVPNAdapterPacketFlow` method signatures are similar to `NEPacketTunnelFlow` so
    // you can just extend that class to adopt `SkVPNAdapterPacketFlow` protocol and
    // send `self.packetFlow` to `completionHandler` callback.
    func skVPNAdapter(
        _ skVPNAdapter: SkVPNAdapter, 
    ...
    // Process events returned by the SkVPN library
    func skVPNAdapter(
        _ skVPNAdapter: SkVPNAdapter, 
        handleEvent event: 
        SkVPNAdapterEvent, message: String?
    ) {
        switch event {
        case .connected:
            if reasserting {
    ...
    // Handle errors thrown by the SkVPN library
    func skVPNAdapter(_ skVPNAdapter: SkVPNAdapter, handleError error: Error) {
        // Handle only fatal errors
        guard let fatal = (error as NSError).userInfo[SkVPNAdapterErrorFatalKey] as? Bool, 
    ...
    // Use this method to process any log message returned by SkVPN library.
...
## License
SkVPNAdapter is available under the AGPLv3 license. See the [LICENSE](LICENSE) file for more info. Also this project has a few dependencies:
- [skvpn3](https://github.com/SkVPN/skvpn3)
